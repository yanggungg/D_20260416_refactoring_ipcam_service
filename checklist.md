...
- [ ] 외부 API 시그니처 인벤토리: `include/nf_api_ipcam.h` 공개 함수 목록 + 외부 호출자 18+ 파일 매핑 표 작성 (`docs/phase0_external_api.md` 형태)
- [ ] 빌드 절차 문서화: 개발서버 접속 → 빌드 명령 → 결과 확인까지의 단계(사용자 안내)
- [ ] 수동 검증 시나리오 정의 (phase별 공통 회귀 체크): CCTV 모드 카메라 연결, OPEN 모드 카메라 연결, 듀얼랜 모드 전환, 카메라 디스커버리 1회, 펌업/이벤트 수신 1회 — 최소 시나리오 합의
- [ ] 모드 플래그 직접/간접 참조 18 파일 목록 확정 (Phase 1 변경 범위 매핑)
- [ ] Phase 0 완료 — 빌드 1회 통과 확인 (변경 없음, 베이스라인)
