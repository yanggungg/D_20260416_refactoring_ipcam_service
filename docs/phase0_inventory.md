...
## 6. 회귀 검증 시나리오와 테스트 셋업(2026-05-13 합의)

### 6.1 테스트 셋업

- **자사 카메라 N대**(NVR 내장 PoE에 직결, 듀얼랜 ON 시 내부망)
- **외부 벤더 카메라 1대**(외부망 LAN, ONVIF 또는 manual)
- 32CH 변형은 CCTV 모드 미지원(`-DNO_PND=1`). 검증 가능 모드 = OPEN 단일랜 + OPEN+듀얼랜.

### 6.2 매 phase 완료 시 돌릴 회귀 시나리오 4종

각 시나리오의 정밀 임계값(시간/카메라 수 등)은 phase 진행 중 보정. 본 절은 수행 단계와 통과/실패 신호의 초안.

#### A. 듀얼랜 ON/OFF 전환 — 핵심 phase: 1, 2, 5

- **단계**: sysdb `cam.install.dual_lan` 토글(또는 UI 설치 모드 변경) → 시스템 재시작/적용 → 내부망 PoE 카메라 채널 출현/해제 확인.
- **통과**: 토글 후 내부망 자사 카메라가 채널에 정상 등장(ON) 또는 사라짐(OFF). 로그에 모드 변경 메시지 정상. 외부망 카메라는 양 모드 모두 유지.
- **실패**: 토글이 반영되지 않음, 채널 매핑 깨짐, 모드 전환 후 라이브 영상 정지/검은 화면.

#### B. 카메라 디스커버리 — 핵심 phase: 6, 안전망(모든 phase)

- **단계**: 디스커버리 트리거 → 외부망(ONVIF) + 내부망(PoE 자사) 두 경로 검색 결과 확인.
- **통과**: 두 경로 모두에서 예상 카메라 후보가 검색 목록에 나타남. ITX 디스커버리 / ONVIF / DHCPD 경로 중 활성 경로가 실제로 응답.
- **실패**: 한 경로가 누락, 검색 결과가 비거나 잘못된 IP, 디스커버리 무한 대기.

#### C. 라이브 영상 1채널 표시 — 핵심 phase: 모든 phase(기본 안전망)

- **단계**: 자사 카메라 1채널 + 외부 카메라 1채널 라이브 화면 열기.
- **통과**: 두 채널 모두 정상 영상 표시. 1분 이상 안정. 코덱/해상도 자동 협상 동작.
- **실패**: 검은 화면, 타임아웃, 디코딩 에러 로그, 한쪽만 표시.

#### D. 카메라 재연결 — 핵심 phase: 5(FSM), 6(디스커버리)

- **단계**: 카메라 1대(자사 또는 외부) 네트워크 끊기 → 잠시 대기 → 다시 연결.
- **통과**: 끊김 표시(vloss) → 자동으로 라이브 복구. recovery 함수(`pnd_recovery_func`/`ipx_vloss_handler_func`/`nf_openmode_recovery`)가 동작.
- **실패**: 자동 복구 안 됨(수동 재기동 필요), 복구 후 다른 채널까지 영향, recovery 무한 루프 로그.

### 6.3 phase별 시나리오 실행 권장

| Phase | 필수 | 선택 |
|---|---|---|
| 0 베이스라인 | A·B·C·D 모두 1회 | — |
| 1 명명 부채 | A·C | B |
| 2 enum 도입 | A·C | B |
| 3 도메인 split | A·B·C | D |
| 4 드라이버 추상화 | B·C·D | A |
| 5 채널 FSM | A·C·D | B |
| 6 통합 디스커버리 | B·C | A·D |
| 7 런타임 capability | C | A·B·D |

→ 매 phase 평균 2-3개 시나리오만 돌리면 됨. 한 phase 검증 부담 추정 10-20분.

---

## 7. 본 인벤토리가 phase 결정에 미치는 영향
