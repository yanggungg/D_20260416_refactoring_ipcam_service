...
#define NULL_PTR_ERROR			0x0001  //	파라미터에 NULL 포인터가 입력됨.
...
static void	SEED_SetAlgInfo( DWORD ModeID, DWORD PadType, BYTE *IV, SEED_ALG_INFO	*AlgInfo);
static unsigned short CalcCRC16(char *data , int len);

static RET_VAL ECB_EncFinal(SEED_ALG_INFO *AlgInfo, BYTE *CipherTxt, DWORD *CipherTxtLen);
static RET_VAL CBC_EncFinal(SEED_ALG_INFO *AlgInfo, BYTE *CipherTxt, DWORD *CipherTxtLen);
static RET_VAL OFB_EncFinal(SEED_ALG_INFO *AlgInfo, BYTE *CipherTxt, DWORD *CipherTxtLen);
static RET_VAL CFB_EncFinal(SEED_ALG_INFO *AlgInfo, BYTE *CipherTxt, DWORD *CipherTxtLen);
static RET_VAL PaddSet(	BYTE *pbOutBuffer, DWORD dRmdLen, DWORD	dBlockLen, DWORD dPaddingType);

static RET_VAL ECB_DecUpdate(SEED_ALG_INFO *AlgInfo, BYTE *CipherTxt, DWORD CipherTxtLen, BYTE *PlainTxt, DWORD *PlainTxtLen);
static RET_VAL CBC_DecUpdate(SEED_ALG_INFO *AlgInfo, BYTE *CipherTxt, DWORD CipherTxtLen, BYTE *PlainTxt, DWORD *PlainTxtLen);
static RET_VAL OFB_DecUpdate(SEED_ALG_INFO *AlgInfo, BYTE *CipherTxt, DWORD CipherTxtLen, BYTE *PlainTxt, DWORD *PlainTxtLen);
static RET_VAL CFB_DecUpdate(SEED_ALG_INFO *AlgInfo, BYTE *CipherTxt, DWORD CipherTxtLen, BYTE *PlainTxt, DWORD *PlainTxtLen);
	
static RET_VAL	SEED_EncUpdate(SEED_ALG_INFO *AlgInfo, BYTE *PlainTxt, DWORD PlainTxtLen, BYTE *CipherTxt, DWORD *CipherTxtLen);
static RET_VAL	SEED_EncFinal(SEED_ALG_INFO *AlgInfo, BYTE *CipherTxt, DWORD *CipherTxtLen);
static RET_VAL SEED_KeySchedule(BYTE *UserKey, DWORD UserKeyLen, SEED_ALG_INFO	*AlgInfo);
...
static RET_VAL CBC_EncUpdate(SEED_ALG_INFO *AlgInfo, BYTE *PlainTxt, DWORD PlainTxtLen, BYTE *CipherTxt, DWORD *CipherTxtLen)
{
	int i;
	DWORD		*ScheduledKey=AlgInfo->RoundKey;
	DWORD		BlockLen=SEED_BLOCK_LEN, BufLen=AlgInfo->BufLen;
...
static RET_VAL OFB_EncUpdate(SEED_ALG_INFO *AlgInfo, BYTE *PlainTxt, DWORD PlainTxtLen, BYTE *CipherTxt, DWORD *CipherTxtLen)
{
	int i;
	DWORD		*ScheduledKey=AlgInfo->RoundKey;
...
static RET_VAL CFB_EncUpdate( SEED_ALG_INFO *AlgInfo, BYTE *PlainTxt, DWORD PlainTxtLen, BYTE *CipherTxt, DWORD *CipherTxtLen)
{
	int i;
	DWORD		*ScheduledKey=AlgInfo->RoundKey;
	DWORD		BlockLen=SEED_BLOCK_LEN;
...
/* ******************************************************************************** */
/* ***********************************  Decoding   ******************************** */
static RET_VAL	SEED_DecUpdate(	SEED_ALG_INFO	*AlgInfo, BYTE	*CipherTxt, DWORD CipherTxtLen,	BYTE *PlainTxt,	DWORD *PlainTxtLen)	
{
	switch( AlgInfo->ModeID ) {
...
static RET_VAL ECB_DecFinal(SEED_ALG_INFO *AlgInfo, BYTE *PlainTxt, DWORD *PlainTxtLen)
{
	DWORD		*ScheduledKey=AlgInfo->RoundKey;
	DWORD		BlockLen=SEED_BLOCK_LEN, BufLen=AlgInfo->BufLen;
	RET_VAL		ret;
...
static RET_VAL CBC_DecFinal(SEED_ALG_INFO *AlgInfo, BYTE *PlainTxt, DWORD *PlainTxtLen)
{
	DWORD		*ScheduledKey=AlgInfo->RoundKey;
	DWORD		BlockLen=SEED_BLOCK_LEN, BufLen=AlgInfo->BufLen;
	RET_VAL		ret;
...
static RET_VAL OFB_DecFinal(SEED_ALG_INFO *AlgInfo, BYTE *PlainTxt, DWORD *PlainTxtLen)
{
	DWORD		*ScheduledKey=AlgInfo->RoundKey;
...
static RET_VAL CFB_DecFinal(SEED_ALG_INFO *AlgInfo, BYTE *PlainTxt, DWORD *PlainTxtLen)
{
	int i;
	DWORD		*ScheduledKey=AlgInfo->RoundKey;
	DWORD		BufLen=AlgInfo->BufLen;
...
static RET_VAL	SEED_DecFinal(SEED_ALG_INFO *AlgInfo, BYTE *PlainTxt, DWORD *PlainTxtLen)
{
	switch( AlgInfo->ModeID ) {
...
static int SeedExecute(DWORD EncType, DWORD ModeType, DWORD PadType, BYTE* lpSrc, DWORD *SLen, BYTE* lpDst, DWORD *DLen)
{
	int i;
	unsigned char	UserKey[SEED_USER_KEY_LEN] = {0x0F,0x31,0x45,0x67,0x89,0xAB,0xCD,0xEF,0xCE,0x34,0x56,0x68,0x93,0xBC,0xDE,0xF7};
	unsigned char	IV[SEED_BLOCK_LEN] = {0x4E, 0xEC, 0xAA, 0x88, 0x66, 0x44, 0x22, 0x11, 0xDE, 0xEC, 0xAA, 0x98, 0x76, 0x54, 0x32, 0x10};
	...
	if(SrcData==NULL) return NULL_PTR_ERROR; 
...
static RET_VAL SEED_KeySchedule(
...
{
	if( !lpDst || (*DLen==0) || !lpSrc || (*SLen ==0)) return NULL_PTR_ERROR;

	return SeedExecute( ENC_TYPE, AI_CBC, AI_PKCS_PADDING , lpSrc, SLen, lpDst, DLen );
}
...
static int DecodeData(BYTE* lpDst, DWORD *DLen, BYTE* lpSrc, DWORD *SLen)
{
	...
	return SeedExecute( DEC_TYPE, AI_CBC, AI_PKCS_PADDING , lpSrc, SLen, lpDst, DLen );
}
...
static unsigned short CalcCRC16(char *data , int len)
{
...
#endif
	int packet_length;
	unsigned short nRetCRC = 0;

	unsigned char  	Ch, TF, TS;
	unsigned short  wCRC;
	...
	do {
		...
		Ch = (Ch^(Ch<<4));
		wCRC = (wCRC>>8)^((unsigned short)Ch<<8)^((unsigned short)Ch<<3)^((unsigned short)Ch>>4);
	}	while(--packet_length);
	...
	((unsigned char*)&nRetCRC)[0] = TF;
	((unsigned char*)&nRetCRC)[1] = TS;
	return nRetCRC;
}


#define S1_DDNS_HEADER_LEN 			7
#define S1_DDNS_CMD_BUFF_POS		7
#define S1_DDNS_DCRC_LEN			2

#define S1_DDNS_DEBUG 	1
...
int _s1_register_write_data(const int sock, const int portnumber, const int rtsp_port, const unsigned char* mac_addr)
{
	...
	unsigned char encryption_buffer[1024]={0,};
	unsigned char decryption_buffer[1024]={0,};
	char tmp[4]={0,};
	int num;
	int ret, crc, len;
	int encryption_buffer_len;
	unsigned short  encryption_buffer_crc16;

	//big endian
	/* ---------------------------------- data --------------------------------------- */
	//Command. 1byte
	packet[S1_DDNS_CMD_BUFF_POS] = 0x21;
...
*/
	...
	//IP: 1byte 0x00=use peer IP, 0x01=dont use peer IP. use below IP
	packet[48] = 0x00;
...
{	
	...
	nf_netif_get_info( &ret_info);		
	tmp_addr = ntohl(ret_info.ipaddr);	
	
	memcpy( &packet[49], &tmp_addr, 4);						
}

#if 0	// single port

	//Number of Port: 2byte. maximum 10. 1 ~ n
	packet[53] = 0x00;//upper byte
	packet[54] = 0x01;//lower byte

	//1st port number : 2byte
	packet[55] = (portnumber>>8)&0xff;//upper byte
	packet[56] = (portnumber)&0xff;//lower byte


	//mac address : 6byte
	sprintf(tmp, "%c%c", mac_addr[0], mac_addr[1]);
	sscanf(tmp, "%x", &num);
	packet[57] = num;

	sprintf(tmp, "%c%c", mac_addr[2], mac_addr[3]);
	sscanf(tmp, "%x", &num);
	packet[58] = num;

	sprintf(tmp, "%c%c", mac_addr[4], mac_addr[5]);
	sscanf(tmp, "%x", &num);
	packet[59] = num;

	sprintf(tmp, "%c%c", mac_addr[6], mac_addr[7]);
	sscanf(tmp, "%x", &num);
	packet[60] = num;

	sprintf(tmp, "%c%c", mac_addr[8], mac_addr[9]);
	sscanf(tmp, "%x", &num);
	packet[61] = num;

	sprintf(tmp, "%c%c", mac_addr[10], mac_addr[11]);
	sscanf(tmp, "%x", &num);
	packet[62] = num;

	encryption_buffer[0]=0x00;
	encryption_buffer_len = sizeof(encryption_buffer);
	len = 56;
	...
	//1st port number : 2byte
	packet[57] = (rtsp_port>>8)&0xff;//upper byte
	packet[58] = (rtsp_port)&0xff;//lower byte
...
/*
	SEED가 적용되면, SEED하는 데이터 필드는 Command~Data까지임.
	SEED가 적용되면, SEED결과를 가지고 [DataCRC16]을 계산함.	
*/
	if( EncodeData( encryption_buffer, &encryption_buffer_len, 
				(BYTE*)(packet+S1_DDNS_HEADER_LEN), &len)!= CTR_SUCCESS ) 
	{
		return -1; //encryption fail
	}

	if(encryption_buffer_len<0 || encryption_buffer_len>300 )
	{
		return -1; //encryption fail
	}
	
	//data CRC16: 2byte. command ~ mac addr
	encryption_buffer_crc16 = CalcCRC16(encryption_buffer,encryption_buffer_len);
	memcpy(packet+7,encryption_buffer, encryption_buffer_len);
	packet[7+encryption_buffer_len] = (encryption_buffer_crc16>>8)&0xff;//upper byte
	packet[7+encryption_buffer_len+1] = (encryption_buffer_crc16)&0xff;//lower byte

	/* ---------------------------------- header --------------------------------------- */
	//Length : 2byte. Command ~ Data CRC16
	packet[0] = ((encryption_buffer_len+S1_DDNS_DCRC_LEN)>>8)&0xff;//upper byte
	packet[1] = ((encryption_buffer_len+S1_DDNS_DCRC_LEN))&0xff;//65-7. lower byte
	//Major Version
	packet[2] = 0x01;
	//Minor Version
	packet[3] = 0x00;
	//SEED : 0x00=not used. 0x01=used
	packet[4] = 0x01;
	//Header CRC16 : 2byte. Length ~ SEED
	crc = CalcCRC16(packet, 5);
	packet[5] = (crc>>8)&0xff;//upper byte
	packet[6] = crc&0xff;//lower byte
	
	len = S1_DDNS_HEADER_LEN+encryption_buffer_len+S1_DDNS_DCRC_LEN;

#ifdef S1_DDNS_DEBUG	
	printf("%s write_len[%d]\n", __FUNCTION__, len);
	nf_debug_hexdump( packet, len);	
#endif	
	if(write(sock, packet, len) < 0)  // 73
	{
		return -1;
	}
...
int _s1_register_read_data(const int sock)
{
	...
	int crc;
	int data_len;
	int decryption_buffer_len;
	int count;
	int len;

	len = read(sock, packet, PACKET_SIZE);
	if(len != 25 )  // s1 read packet size 25 byte
		return -1;

	decryption_buffer[0] = 0x00;
	decryption_buffer_len = 1024;

	crc = CalcCRC16(packet, 5);
	if( (((crc>>8)&0xff) != (packet[5]&0xff)) || ((crc&0xff) != (packet[6]&0xff)) )
	{
		return -1;
	}
	data_len = 0;
	data_len = (((packet[0])<<8)&0xff00)|(packet[1]&0xff)-2;
	if(data_len<0 || data_len>300)
	{
		return -1;
	}
	if(DecodeData( decryption_buffer, &decryption_buffer_len, packet+S1_DDNS_HEADER_LEN, &data_len) != CTR_SUCCESS )
	{
		return -1;
	}

#ifdef S1_DDNS_DEBUG	
	printf("len[0x%04x] cmd[0x%02x] err_up[0x%02x] err_lo[0x%02x]\n", 
				decryption_buffer_len,
				decryption_buffer[0],
				decryption_buffer[1],
				decryption_buffer[2]);
	nf_debug_hexdump( decryption_buffer, decryption_buffer_len);	
#endif
				
	if( (decryption_buffer_len!=9) || 	//data length	: 9byte
			(decryption_buffer[0]!=0x21)|| 	//Command code	: 0x21
			(decryption_buffer[1]!=0x00)||	//Error Code	: Upper byte
			(decryption_buffer[2]!=0x00)	//Error Code	: Lower byte
	  )
	{
		return -1;
	}
...
int _s1_query_write_data(const int sock, const unsigned char* mac_addr)
{
	...
	encryption_buffer_len = sizeof(encryption_buffer);
	len = 7;
	if( EncodeData( encryption_buffer, &encryption_buffer_len, (BYTE*)(packet+S1_DDNS_HEADER_LEN), &len)!= CTR_SUCCESS ) //56byte
	{
		return -1; //encryption fail
	}
...
int _s1_query_read_data(const int sock, unsigned int *ipv4_addr)
{
	...
	if(DecodeData( decryption_buffer, &decryption_buffer_len, packet+7, &data_len) != CTR_SUCCESS )
	{
		return -1;
	}
	...
	if( (decryption_buffer[0]!=0x41)|| 	//Command code	: 0x21
			(decryption_buffer[1]!=0x00)||	//Error Code	: Upper byte
			(decryption_buffer[2]!=0x00)	//Error Code	: Lower byte
	  )
	{
	...
	*ipv4_addr = *(unsigned int *)&decryption_buffer[44]; // peer_ip
