...
#elif defined(ENABLE_AI_ALARM_AUDIO)
...
#if defined(ENABLE_AI_ALARM_AUDIO_DSP) || defined(ENABLE_AI_ALARM_AUDIO)
#if defined(ENABLE_AI_ALARM_AUDIO_DSP)
	...
	for(ch=0; ch<NUM_ACTIVE_CH; ch++) {
		...
		if(*is_playing == FALSE) {
			...
			if((*mask_ai_event_test >> ch) & 0x1) {		// Test!!
				char *filename=NULL;
				char *convert_src=NULL;
				char convert_tar[256] = {0,};
				char tmp_cmd[300] = {0,};

				convert_src=ai_aud->ai_evt_aud_test_filename;
				sprintf(convert_tar,"%s",convert_src);

				filename = strstr(convert_tar, ".wav");
				if(filename == NULL)
					goto nf_rec_aud_send_fail3_ai;
				else {
					strncpy(filename, ".ulaw", sizeof(convert_tar));
					filename = convert_tar;
				}

				if(access(filename, F_OK) == -1) {
					memset(tmp_cmd,0x00,sizeof(tmp_cmd));
					strcpy(tmp_cmd, "cp -a \"");
					strcat(tmp_cmd,convert_src);
					strcat(tmp_cmd, "\" /opt/jr695_aud.wav");
					system(tmp_cmd);

					memset(tmp_cmd,0x00,sizeof(tmp_cmd));
					strcpy(tmp_cmd, "sox /opt/jr695_aud.wav -r 8000 -c 1 -t ul /opt/jr695_aud.ulaw");
					system(tmp_cmd);

					memset(tmp_cmd,0x00,sizeof(tmp_cmd));
					strcpy(tmp_cmd, "cp -a /opt/jr695_aud.ulaw \"");
					strcat(tmp_cmd,convert_tar);
					strcat(tmp_cmd,"\"");
					system(tmp_cmd);

					memset(tmp_cmd,0x00,sizeof(tmp_cmd));
					strcpy(tmp_cmd, "rm -rf /opt/jr695_aud.*");
					system(tmp_cmd);
				}
				...
				*is_playing=TRUE;
				...
				for(ai_evt_cnt=0; ai_evt_cnt < (gint)ai_evt_total; ai_evt_cnt++) {
					...
					if(*mask_ai_event[ch]) {
						...
						for(ai_evt_type=0; ai_evt_type<NF_AUDIO_AI_EVENT_NR; ai_evt_type++) {
							...
							if((*mask_ai_event[ch] >> ai_evt_type) & 0x1) {
								...
								convert_src=nf_rec_audio_ai_get_sysdb_alarm((gint)ch, ai_evt_rule, ai_evt_type);
								sprintf(convert_tar,"%s",convert_src);
	...
	for(ai_evt_cnt=0; ai_evt_cnt < (gint)ai_evt_total; ai_evt_cnt++) {
		for(ai_evt_type=0; ai_evt_type<AI_IVCA_NR; ai_evt_type++) {
			if(pevt->type & AI_IVCA_ET_DIR_POS) {
				mask_ai_evt |= (1<< NF_AUDIO_AI_EVENT_DIR_POS);
			} else if(pevt->type & AI_IVCA_ET_DIR_NEG) {
				mask_ai_evt |= (1 << NF_AUDIO_AI_EVENT_DIR_NEG);
			} else if(pevt->type & AI_IVCA_ET_ENTER) {
				mask_ai_evt |= (1 << NF_AUDIO_AI_EVENT_ENTER);
			} else if(pevt->type & AI_IVCA_ET_EXIT) {
				mask_ai_evt |= (1 << NF_AUDIO_AI_EVENT_EXIT);
			} else if(pevt->type & AI_IVCA_ET_STOPPED) {
				mask_ai_evt |= (1 << NF_AUDIO_AI_EVENT_STOPPED);
			} else if(pevt->type & AI_IVCA_ET_REMOVED) {
				mask_ai_evt |= (1 << NF_AUDIO_AI_EVENT_REMOVED);
			} else if(pevt->type & AI_IVCA_ET_INTRUSION) {
				mask_ai_evt |= (1 << NF_AUDIO_AI_EVENT_INTRUSION);
			} else if(pevt->type & AI_IVCA_ET_LOITERED) {
				mask_ai_evt |= (1 << NF_AUDIO_AI_EVENT_LOITERED);
			} else {
				/*
				 *	Do Nothing
				*/
