...
#define DBG_LEVEL		9
#define DBG_MODULE		"VW_DEEPLEARNING_GENERIC_ANIMATE_POPUP"


#define MAX_ANIMATE_CNT				(10)

#define EVENT_FRAME_WIDTH			(160)
#define EVENT_FRAME_HEIGHT			(281)
...
static NFOBJECT *g_caption_obj;
static NFOBJECT *g_title_obj;
static NFOBJECT *g_description_obj;

static GtkWidget *g_gtkwindow = 0;
static GtkWidget *g_gtkimage = 0;

static GENERIC_AOBJECT_INFO_T g_animate_info = {0, };
...
static int _proc_get_thumbnail(WRK_ID wrkid, CMM_MESSAGE_T *pmsg) 
{
    ...
    for (i = 0; i < MAX_ANIMATE_CNT; i++)
	{
		...
		retVal = nf_play_get_thumbnail_jpeg(param->ch, stime, etime, &jpeg_w, &jpeg_h, &jpeg_size, &jpeg_buffer, &ftime, NF_SECOND_SIZE);
...
static gint _draw_snap_frame(NFOBJECT *obj)
{
	...
	gdk_draw_rectangle(drawable, gc, FALSE, 522, 44, EVENT_FRAME_WIDTH, EVENT_FRAME_HEIGHT);
	...
	if (g_snap_pbuf)
	{
		gdk_draw_pixbuf(drawable, gc, g_snap_pbuf, 0, 0, 
			522+(EVENT_FRAME_WIDTH-gdk_pixbuf_get_width(g_snap_pbuf))/2, 44+(EVENT_FRAME_HEIGHT-gdk_pixbuf_get_height(g_snap_pbuf))/2, 
...
static gboolean post_playback_button_event_handler(NFOBJECT *obj, GdkEvent *evt, gpointer data)
{
	if (evt->type == GDK_BUTTON_RELEASE)
	{
		...
		VW_deepLearning_generic_animate_popup_hide();
	...
	switch (event->type) 
	{
		...
		case INFY_DEEPLEARNING_ANIMATE_THUMBNAIL:
		{
			...
			if (rcv_buffer) 
			{
				...
				g_animate_pbuf[idx] = gdk_pixbuf_scale_simple(npixbuf, 500, 281, GDK_INTERP_HYPER);

				if (idx == MAX_ANIMATE_CNT-1)
				{
					if (g_snap_pbuf) g_object_unref(g_snap_pbuf);
					g_snap_pbuf = 0;
					g_snap_pbuf = _get_target_pixbuf(gdk_pixbuf_get_width(npixbuf), gdk_pixbuf_get_height(npixbuf), npixbuf, g_animate_info.coords);
				}
...
void VW_deepLearning_generic_animate_popup_open(NFWINDOW *parent)
{
	...
	win = (NFOBJECT*)nfui_nfwindow_new(parent, 1920-192-204-7-700-7, (972-566)/2, 700, 566);
	...
	nfui_nfobject_set_size(fixed, 700, 566);
    ...
    obj = nfui_listbox_new(1, table_w, 32);
    ...
    nfui_nfobject_set_size(obj, 660, 98);
    nfui_nfobject_use_focus(obj, NFOBJECT_FOCUS_ON);
    nfui_nfobject_show(obj);
    nfui_nffixed_put((NFFIXED*)fixed, obj, 18, 333+tbl->height+2);  
	g_description_obj = obj;
...
void VW_deepLearning_generic_animate_popup_show(GENERIC_AOBJECT_INFO_T *animate_info)
{
	...
	memcpy(&g_animate_info, animate_info, sizeof(GENERIC_AOBJECT_INFO_T));

	g_message("%s, %d, ch:%d, time:%ld", __FUNCTION__, __LINE__, animate_info->ch, animate_info->ttime.tv_sec);

	if (VW_deepLearning_group_popup_is_shown()) {
		win_x = 1920-192-204-7-700-7;
		win_y = (972-566)/2;
	}
	else {
		win_x = 1920-192-700-7;
		win_y = (972-566)/2;
	}
	...
	memset(strBuf, 0x00, sizeof(strBuf));
	nfui_nflabel_set_text((NFLABEL*)g_caption_obj, animate_info->caption);

	memset(strBuf, 0x00, sizeof(strBuf));
	nfui_nflabel_set_text((NFLABEL*)g_title_obj, animate_info->title);

	nfui_listbox_delete_all(NF_LISTBOX(g_description_obj));
	token_ptr = strtok_r(animate_info->description, "\n", &pnext);
	pbuf = pnext;

	while (token_ptr != 0)
	{
		g_message("%s, %d, token:%s", __FUNCTION__, __LINE__, token_ptr);
		nfui_listbox_set_text(NF_LISTBOX(g_description_obj), &token_ptr);
