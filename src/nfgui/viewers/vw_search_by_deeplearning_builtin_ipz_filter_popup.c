...
static OPT_BUILTIN_IPZ_T *g_opt = NULL;

static NFOBJECT *g_builtin_ipz_car_obj;
static NFOBJECT *g_builtin_ipz_bike_obj;
static NFOBJECT *g_builtin_ipz_vehicle_obj;
...
static gboolean post_builtin_object_group_event_handler(NFOBJECT *obj, GdkEvent *evt, gpointer data)
{
    ...
    {   
        ...
        nfui_check_button_set_active_no_expose(NF_CHECKBUTTON(g_builtin_ipz_vehicle_obj), state);
        nfui_signal_emit(g_builtin_ipz_vehicle_obj, GDK_EXPOSE, TRUE);
    }
...
static gboolean post_btn_ok_event_handler(NFOBJECT *obj, GdkEvent *evt, gpointer data)
{
    ...
    if (evt->type == GDK_BUTTON_RELEASE)
    {
        ...
        g_opt->car = nfui_check_button_get_active(NF_CHECKBUTTON(g_builtin_ipz_car_obj));
        g_opt->bike = nfui_check_button_get_active(NF_CHECKBUTTON(g_builtin_ipz_bike_obj));
...
gint VW_Search_By_Builtin_Ipz_Filter_Popup(NFWINDOW *parent, OPT_BUILTIN_IPZ_T *opt)
{
    NFWINDOW *main_wnd;
    NFOBJECT *main_fixed;
    NFOBJECT *obj;
    ...
    g_builtin_ipz_vehicle_obj = obj;

    obj = (NFOBJECT*)nfui_nflabel_new_with_pango_font("ALL", nffont_get_pango_font(NFFONT_MEDIUM_NORMAL), COLOR_IDX(292));
