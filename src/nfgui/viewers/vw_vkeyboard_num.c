	...
	if(evt->type == GDK_BUTTON_PRESS)
	{
		...
		for(i = 0; i < 10; i++)
		{
			if(obj == num_btns[i])
			{
				...
				pre_value = nfui_nfvklabel_get_num((NFVKLABEL*)num_label);
				
                if (nfui_nfvklabel_get_cursor_state((NFVKLABEL*)num_label) == VK_SELECT)
                {   				
    				post_value = pre_value*10 + input_num;
				
				if (post_value > max_num) return FALSE;
                }

                if (pre_value == 0)
    				nfui_nfvklabel_set_num((NFVKLABEL*)num_label, input_num);
		        else			
	    			nfui_nfvklabel_input_num((NFVKLABEL*)num_label, input_num);
...
static gboolean post_ok_button_event_handler(NFOBJECT *obj, GdkEvent *evt,  gpointer data)
{
	...
	if(evt->type == GDK_BUTTON_RELEASE)
	{
		...
		numResult = nfui_nfvklabel_get_num((NFVKLABEL*)num_label);
...
gint NumberKey_Open(NFWINDOW *parent, gint init_num, guint x, guint y, gint max)
{
	NFOBJECT *main_wnd;
	NFOBJECT *main_fixed;
	NFOBJECT *fixed1;
...
// <---- NUM LABEL
    ...
    char_cnt = strlen(strBuf);

	obj = (NFOBJECT*)nfui_nfvklabel_new_num(init_num, char_cnt);
...
// <---- OK, CANCEL BUTTON
	...
	nfui_set_key_focus(num_label, TRUE);
