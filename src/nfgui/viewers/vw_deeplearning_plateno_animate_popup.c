...
#include "vw_deeplearning_plateno_animate_popup.h"
...
#define DBG_LEVEL		9
#define DBG_MODULE		"VW_DEEPLEARNING_PLATENO_ANIMATE_POPUP"
...
static NFOBJECT *g_plateno_obj;

static GtkWidget *g_gtkwindow = 0;
static GtkWidget *g_gtkimage = 0;

static PLATENO_AOBJECT_INFO_T g_animate_info = {0, };
...
static gboolean post_playback_button_event_handler(NFOBJECT *obj, GdkEvent *evt, gpointer data)
{
	if (evt->type == GDK_BUTTON_RELEASE)
	{
		...
		VW_deepLearning_plateno_animate_popup_hide();
...
void VW_deepLearning_plateno_animate_popup_open(NFWINDOW *parent)
{
	...
	guint table_w[] = {130, 660/2-130, 130, 660/2-130};
	...
	win = (NFOBJECT*)nfui_nfwindow_new(parent, 1920-192-204-7-700-7, (972-424)/2, 700, 424);
	...
	nfui_nfobject_set_size(fixed, 700, 424);
	...
	obj = (NFOBJECT*)nfui_nflabel_new_with_pango_font("LICENSE PLATE", nffont_get_pango_font(NFFONT_SMALL_SEMI), COLOR_IDX(387));
	nfui_nflabel_set_align((NFLABEL*)obj, NFALIGN_LEFT, 8);
	nfui_nfobject_modify_bg(obj, NFOBJECT_STATE_NORMAL, COLOR_IDX(200));
	nfui_nfobject_use_focus(obj, NFOBJECT_FOCUS_OFF);
	nfui_nfobject_show(obj);
...
void VW_deepLearning_plateno_animate_popup_show(PLATENO_AOBJECT_INFO_T *animate_info)
{
	...
	memcpy(&g_animate_info, animate_info, sizeof(PLATENO_AOBJECT_INFO_T));
	...
	memset(strBuf, 0x00, sizeof(strBuf));
	nfui_nflabel_set_text((NFLABEL*)g_rule_obj, "LICENSE PLATE RECOGNITION");

	memset(strBuf, 0x00, sizeof(strBuf));
	sprintf(strBuf, "%s(""%d""%)", animate_info->number, animate_info->confidence);
	nfui_nflabel_set_text((NFLABEL*)g_plateno_obj, strBuf);		
