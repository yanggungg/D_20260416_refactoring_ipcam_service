...
#define HELP_STR "Must be greater than 8 characters and less than 64 characters long."
...
static gchar *keyVal_default[2][KEY_CNT] = 
{
...
static gchar *actv_normal[2][KEY_CNT] = 
{
	{"~", "1", "2", "3", "4", "5", "6", "7", "8", "9", "0", "-", "=", "BS", 
	"q", "w", "e", "r", "t", "y", "u", "i", "o", "p", "{", "}", "\\", "DEL",
	"a", "s", "d", "f", "g", "h", "j", "k", "l", ".", "\"", "ENTER",
...
static void change_shift_key()
{
	...
	for(i=0; i<KEY_CNT; i++) {
		...
		printf("%s\n", active[shift][i]);
		if (strcmp(active[shift][i], "") == 0 || i==39) nfui_nfobject_disable(g_keyBtn[i]);
...
static gboolean ok_button_event_handler(NFOBJECT *obj, GdkEvent *event, gpointer data)
{
	...
	if(event->type == GDK_BUTTON_RELEASE)
	{
		...
		if(len < 8 || len > 64)
		{
		    nftool_mbox(g_curwnd, "NOTICE", HELP_STR, NFTOOL_MB_OK);
		    return FALSE;       
		}
...
gchar* VirtualKey3_Open(NFWINDOW *parent, const gchar *str, guint x, guint y, gint max_ch)
{
	NFOBJECT *close_btn;
	NFOBJECT *ntb;
	NFOBJECT *obj = NULL;
	...
	*/
	...
	for(i=0; i<KEY_CNT; i++) {
		...
		{
			g_keyBtn[i] = (NFOBJECT*)nfui_nfbutton_new_with_param(bgBtn_W140, keyVal[0][i]);
