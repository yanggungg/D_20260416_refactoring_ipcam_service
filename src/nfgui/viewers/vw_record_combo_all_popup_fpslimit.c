...
static gchar **fps_paramInfo = NULL;

static gboolean rca_ret = FALSE;

static GSList *capList;
static gchar **size_paramInfo = NULL;

static void _check_limit_data(gint ch, gchar *cap_data, gchar *limit_data)
{
    gint i, len;
    gint cnt = 0;
    ...
    if ((IS_PTZ(ch)) && (strcmp(size_info[CAP_RES_640x352], size_paramInfo[ch]) == 0))
    {
        for (i = 0; i < len; i++)
        {
            if ((strcmp(fps_info[CAP_FPS_30], translate_info_fps_capable(ch, cap_data[i])) != 0)
                && (strcmp(fps_info[CAP_FPS_15], translate_info_fps_capable(ch, cap_data[i])) != 0)
                && (strcmp(fps_info[CAP_FPS_25], translate_info_fps_capable(ch, cap_data[i])) != 0)
                && (strcmp(fps_info[CAP_FPS_12], translate_info_fps_capable(ch, cap_data[i])) != 0))
                limit_data[cnt++] = cap_data[i];
        }
    }
    else
    {
        for (i = 0; i < len; i++)
            limit_data[i] = cap_data[i];
    }
}

static void _sort_info()
{
    gchar *tmp_data;
    gchar ch1_data[32], ch2_data[32];   
    ...
    {
        tmp_data = get_record_fps_capable_data(ch1);
        
        memset(ch1_data, 0x00, sizeof(ch1_data));
        _check_limit_data(ch1, tmp_data, ch1_data);
        ...
        if (mask_checked & (1 << ch1))
        {
            ...
            {
                tmp_data = get_record_fps_capable_data(ch2);

                memset(ch2_data, 0x00, sizeof(ch2_data));               
                _check_limit_data(ch2, tmp_data, ch2_data);
...
static void _set_capable_fps_info(NFOBJECT *obj, gint ch)
{
    ...
    tmp_data = get_record_fps_capable_data(ch);
    
    _check_limit_data(ch, tmp_data, data);
...
static gboolean _post_ok_event_handler(NFOBJECT *obj, GdkEvent *evt, gpointer data)
{
    ...
    if(evt->type == GDK_BUTTON_RELEASE) {
        ...
        if(memcmp(info, fps_paramInfo, sizeof(gchar*) * GUI_CHANNEL_CNT))
            memcpy(info, fps_paramInfo, sizeof(gchar*) * GUI_CHANNEL_CNT);
...
gboolean VW_RecComboAll_Popup_fpsLimit_Open(NFWINDOW *parent, gint x, gint y, gchar **size_info, gchar **fps_info)
{
    NFOBJECT *fixed;
    ...
    size_paramInfo = size_info;
    fps_paramInfo = imalloc(sizeof(gchar*) * GUI_CHANNEL_CNT);  
    ...
    memcpy(fps_paramInfo, fps_info, sizeof(gchar*) * GUI_CHANNEL_CNT);
    ...
    for (i = 0; i < list_cnt; i++)
    {
        ...
        for (j = 0; j < GUI_CHANNEL_CNT; j++)
        {
            if (mask_cap & (1 << j))
            {
                ...
                nfui_nfimglabel_set_pango_font((NFIMGLABEL*)obj, nffont_get_pango_font(NFFONT_MEDIUM_SEMI), COLOR_IDX(163));
...
//        nfui_nffixed_put((NFFIXED*)fixed, obj, RCA_INFO_LABEL_X, src_y+(dst_y-src_y-RCA_LABEL_H)/2);
    ...
    nfui_nfobject_set_data(obj, "ori info", fps_info);
    nfui_nfobject_show(obj);
    nfui_nffixed_put((NFFIXED*)fixed, obj, pos_x, pos_y);
    ...
    nfui_page_open(PGID_REC_PARAM_COMBO_ALL_FPSLIMIT, rca_win, nfui_get_last_user());

    gtk_main();

    nfui_page_close(PGID_REC_PARAM_COMBO_ALL_FPSLIMIT, rca_win);

    return rca_ret;
}

void VW_RecComboAll_Popup_fpsLimit_Close()
{
    if (rca_win)
    {
        rca_ret = FALSE;
