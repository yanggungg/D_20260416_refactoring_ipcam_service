...
static gboolean post_okbutton_event_handler(NFOBJECT *obj, GdkEvent *evt, gpointer data)
{
	if(evt->type == GDK_BUTTON_RELEASE)
	{
		if(evt->button.button == MOUSE_RIGTH_BUTTON) 
			return FALSE;
		_dvabx_calibration_page(g_parent);
		_dvabx_calibration_hide(g_fixed);
 	}
...
gint _dvabx_calibration_result(NFOBJECT *parent)
{
	...
	nfui_nfobject_set_size(fixed, DVABX_ROCL_FIXED_W, DVABX_ROCL_FIXED_H - (DVABX_BTN_H + DVABX_BTN_GAP));
