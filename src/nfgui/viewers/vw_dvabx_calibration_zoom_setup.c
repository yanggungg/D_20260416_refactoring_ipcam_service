...
static gint _set_icon_height(DVAAID dvaaid, gint calbid, gint icon_height)
{
    ITX_DVACALB_CONF conf;    
...
static gint _update_calb_object_data(DVAAID dvaaid)
{
    ITX_DVACALB_CONF conf;    
    ITX_CALBID calb_idx;
    gint spin_index;

    if (_get_calibration_cnt(dvaaid) < 32) nfui_nfobject_enable(g_add_btn);
    else nfui_nfobject_disable(g_add_btn);

    if (_get_calibration_cnt(dvaaid) > 0) nfui_nfobject_enable(g_reset_btn);
    ...
    if (calb_idx != -1)
    {
        ...
        dvaa_itx_detector_get_calb_conf(dvaaid, calb_idx, &conf);
...
static gboolean post_fixed_event_cb(NFOBJECT *obj, GdkEvent *evt, gpointer data)
{
    ...
    else if (evt->type == INFY_DVAA_ITX_PRESS_CALB_ID)
    {
        DVAAID dvaaid;    
    
        dvaaid = dvaa_get_dvaaid(g_cur_ch);
        _update_calb_object_data(dvaaid);
...
static gboolean post_icon_height_spin_event_handler(NFOBJECT *obj, GdkEvent *evt, gpointer data)
{
    ...
    {       
        ...
        _set_icon_height(dvaaid, calb_idx, value);
    }
    else if(evt->type == GDK_2BUTTON_PRESS)
    {
        ...
        if(strTemp)
        {
            ...
            _set_icon_height(dvaaid, calb_idx, numTemp);  
            ifree(strTemp);
        }
    }    
	return FALSE;
...
static gboolean post_pausebutton_event_handler(NFOBJECT *obj, GdkEvent *evt, gpointer data)
{
	...
	{        
        ...
        {
            _dvabx_calibration_zoom_restart();
...
gint vw_dvabx_calibration_zoom_setup_open(NFOBJECT *parent, gint ch)
{
    NFOBJECT *win;
    NFOBJECT *obj;
    NFOBJECT *fixed;
