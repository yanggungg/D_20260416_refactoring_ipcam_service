...
static EA_SysDiskData	g_sdd[EVENT_TYPE_COUNT];
static EA_SysDiskData	g_osdd[EVENT_TYPE_COUNT];

static EA_SysDiskEvtData g_sded;
static EA_SysDiskEvtData g_osded;

static NFWINDOW *g_curwnd = 0;
static NFOBJECT *g_almObj[EVENT_TYPE_COUNT];
static NFOBJECT *g_buzzObj[EVENT_TYPE_COUNT];
static NFOBJECT *g_opopObj[EVENT_TYPE_COUNT];
...
static void init_sd_data()
{
	...
	memset(g_sdd, 0x00, sizeof(EA_SysDiskData) * EVENT_TYPE_COUNT);
	memset(g_osdd, 0x00, sizeof(EA_SysDiskData) * EVENT_TYPE_COUNT);
	for(i = 0; i < EVENT_TYPE_COUNT; i++)
		DAL_get_SysDisk_data(&g_sdd[i], i);

	g_memmove(g_osdd, g_sdd, sizeof(EA_SysDiskData) * EVENT_TYPE_COUNT);


	memset(&g_sded, 0x00, sizeof(EA_SysDiskEvtData));
	memset(&g_osded, 0x00, sizeof(EA_SysDiskEvtData));
	DAL_get_SysDiskEvt_data(&g_sded);

	g_memmove(&g_osded, &g_sded, sizeof(EA_SysDiskEvtData));
}

static void set_sd_data()
{
	g_memmove(g_osdd, g_sdd, sizeof(EA_SysDiskData)*EVENT_TYPE_COUNT);
	DAL_set_SysDisk_data_all(g_sdd);

	g_memmove(&g_osded, &g_sded, sizeof(EA_SysDiskEvtData));
	DAL_set_SysDiskEvt_data(g_sded);
...
static void set_data_to_obj(gint expose)
{
	...
	for(i=0; i<EVENT_TYPE_COUNT; i++) {
		...
		conv_alarmOut_data_to_string(g_sdd[i].almOut, str);
		...
		if (expose)
		{
			nfui_check_button_set_active((NFCHECKBUTTON*)g_buzzObj[i], g_sdd[i].buzzer);
			nfui_check_button_set_active((NFCHECKBUTTON*)g_opopObj[i], g_sdd[i].opop);
			nfui_check_button_set_active((NFCHECKBUTTON*)g_mailObj[i], g_sdd[i].email);
			nfui_check_button_set_active((NFCHECKBUTTON*)g_ftpObj[i], g_sdd[i].ftp);
#if defined(_SUPPORT_USER_PHONE)
			nfui_check_button_set_active((NFCHECKBUTTON*)g_mobileObj[i], g_sdd[i].mobile);
#endif
            if(support_mobilepush)
			    nfui_check_button_set_active((NFCHECKBUTTON*)g_mobilepushObj[i], g_sdd[i].mobilepush);
		}
		else
		{
			nfui_check_button_set_active_no_expose((NFCHECKBUTTON*)g_buzzObj[i], g_sdd[i].buzzer);
			nfui_check_button_set_active_no_expose((NFCHECKBUTTON*)g_opopObj[i], g_sdd[i].opop);
			nfui_check_button_set_active_no_expose((NFCHECKBUTTON*)g_mailObj[i], g_sdd[i].email);
			nfui_check_button_set_active_no_expose((NFCHECKBUTTON*)g_ftpObj[i], g_sdd[i].ftp);
#if defined(_SUPPORT_USER_PHONE)
			nfui_check_button_set_active_no_expose((NFCHECKBUTTON*)g_mobileObj[i], g_sdd[i].mobile);
#endif
        if(support_mobilepush)
		    nfui_check_button_set_active_no_expose((NFCHECKBUTTON*)g_mobilepushObj[i], g_sdd[i].mobilepush);
		}
	}
...
static void get_data_from_obj()
{
	...
	for(i=0; i<EVENT_TYPE_COUNT; i++) {
		g_sdd[i].buzzer = nfui_check_button_get_active((NFCHECKBUTTON*)g_buzzObj[i]);
		g_sdd[i].opop   = nfui_check_button_get_active((NFCHECKBUTTON*)g_opopObj[i]);
		g_sdd[i].email  = nfui_check_button_get_active((NFCHECKBUTTON*)g_mailObj[i]);
		g_sdd[i].ftp    = nfui_check_button_get_active((NFCHECKBUTTON*)g_ftpObj[i]);
#if defined(_SUPPORT_USER_PHONE)
		g_sdd[i].mobile = nfui_check_button_get_active((NFCHECKBUTTON*)g_mobileObj[i]);
#endif
        if(support_mobilepush)
    		g_sdd[i].mobilepush = nfui_check_button_get_active((NFCHECKBUTTON*)g_mobilepushObj[i]);
	}
}
...
gint conv_exhauVal_to_index(guint val)
{
	switch(val) {
		case 90: return 0;
		case 80: return 1;
...
guint conv_exhauIdx_to_val(gint index)
{
	switch(index) {
		case 0: return 90;
		case 1: return 80;
...
static gboolean post_ao_label_event_handler(NFOBJECT *obj, GdkEvent *evt, gpointer data)
{
	...
	if(evt->type == GDK_2BUTTON_PRESS || kpid == KEYPAD_ENTER) {
		...
		ch = GPOINTER_TO_UINT(nfui_nfobject_get_data(obj, "event type"));

		nfui_nfobject_get_offset(obj, &off_x, &off_y);

		mask = (ALARM_OUT_MASK_TYPE | g_sdd[ch].almOut);
		if(!VW_ChannelMask_Ctrl(g_curwnd, "ALARM OUT CHANNEL", off_x, off_y + obj->height, &mask))
			return FALSE;

		if(mask != g_sdd[ch].almOut) {
			g_sdd[ch].almOut = mask;

			memset(str, 0x00, ALARM_STRING_LENGTH);
			conv_alarmOut_data_to_string(g_sdd[ch].almOut, str);
...
static gboolean post_disk_exhau_btn_event_handler(NFOBJECT *obj, GdkEvent *evt, gpointer data)
{
	if(evt->type == GDK_BUTTON_PRESS) {
		gchar *strThreshold[] = {"90%", "80%", "70%", "60%", "50%"};
		guint off_x, off_y;
		gint init = 0;
		gint ret = -1;
		...
		init = conv_exhauVal_to_index(g_sded.disk_exhau);
		ret = VW_EvtTypeData_Ctrl(g_curwnd, strThreshold, 5, init, (gint)(off_x - 158), (gint)(off_y + obj->height + 4));

		if(ret >= 0)
			g_sded.disk_exhau = conv_exhauIdx_to_val(ret);
	}

	return FALSE;
}

static gboolean post_no_disk_btn_event_handler(NFOBJECT *obj, GdkEvent *evt, gpointer data)
{
	if(evt->type == GDK_BUTTON_PRESS) {
		gchar *strTry[] = {"Retry 5 times", "Retry 4 times", "Retry 3 times", "Retry 2 times", "Retry 1 more"};
		guint off_x, off_y;
		gint ret = -1;

		nfui_nfobject_get_offset(obj, &off_x, &off_y);
		ret = VW_EvtTypeData_Ctrl(g_curwnd, strTry, 5, (5 - g_sded.nodisk_cnt), (gint)(off_x - 218), (gint)(off_y + obj->height + 4));

		if(ret >= 0)
			g_sded.nodisk_cnt = (5 - ret);
	}
...
static gboolean post_cancelbutton_event_handler(NFOBJECT *obj, GdkEvent *evt, gpointer data)
{
	if(evt->type == GDK_BUTTON_RELEASE)
	{
		...
		g_memmove(g_sdd, g_osdd, sizeof(EA_SysDiskData)*EVENT_TYPE_COUNT);
		g_memmove(&g_sded, &g_osded, sizeof(EA_SysDiskEvtData));
...
static gboolean post_applybutton_event_handler(NFOBJECT *obj, GdkEvent *evt, gpointer data)
{
	if(evt->type == GDK_BUTTON_RELEASE)
	{
		...
		if(memcmp(g_osdd, g_sdd, sizeof(EA_SysDiskData) * EVENT_TYPE_COUNT)
				|| memcmp(&g_osded, &g_sded, sizeof(EA_SysDiskEvtData))) {
			set_sd_data();
...
void VW_Init_SysDisk_Evt_Page(NFOBJECT *parent)
{
	...
	gchar *strEvtType[] = {"EVENT TYPE", "OVERWRITE START EVENT", "DISK FULL EVENT", "DISK SPACE EXHAUSTED EVENT", "S.M.A.R.T EVENT", "NO DISK EVENT"};
...
*/
	...
	for(i=0; i<5; i++) {
		if(i == 2) {
			obj = (NFOBJECT*)nfui_nflabel_new_with_pango_font(strEvtType[i + 1], nffont_get_pango_font(NFFONT_MEDIUM_SEMI), COLOR_IDX(116));
			nfui_nflabel_set_align((NFLABEL*)obj, NFALIGN_LEFT, 4);
			...
			nfui_regi_post_event_callback(obj, post_disk_exhau_btn_event_handler);
			nfui_nfobject_show(obj);
			nfui_nffixed_put((NFFIXED*)content_fixed, obj, (pos_x + 350 - size_w), (pos_y + (i * 41)));
		}
		else if(i == 4)
		{
			obj = (NFOBJECT*)nfui_nflabel_new_with_pango_font(strEvtType[i + 1], nffont_get_pango_font(NFFONT_MEDIUM_SEMI), COLOR_IDX(116));
			...
			nfui_nfbutton_set_image(NF_BUTTON(obj), dropdown_img2);
			nfui_nfobject_set_size(obj, (guint)size_w, (guint)size_h);
			nfui_regi_post_event_callback(obj, post_no_disk_btn_event_handler);
			nfui_nfobject_show(obj);
			nfui_nffixed_put((NFFIXED*)content_fixed, obj, (pos_x + 350 - size_w), (pos_y + (i * 41)));
#endif
		}
	...
	for(i=0; i<EVENT_TYPE_COUNT; i++) {
		...
		conv_alarmOut_data_to_string(g_sdd[i].almOut, buf);
		...
		nfui_nffixed_put((NFFIXED*)content_fixed, g_almObj[i], (guint)pos_x, (guint)(pos_y + (i * 41)));
	...
	// buzzer (col 2) & video popup (col 3) & osd popup (col 4) & email (col 5)
	for(i = ACTION_BUZZER; i < ACTION_MAX; i++)
	{
		for(j = 0; j < EVENT_TYPE_COUNT; j++) {
			...
			if(i == ACTION_BUZZER) 		obj = (NFOBJECT*)nfui_checkbutton_new(g_sdd[j].buzzer);
			else if(i == ACTION_OPOPUP) obj = (NFOBJECT*)nfui_checkbutton_new(g_sdd[j].opop);
			else if(i == ACTION_EMAIL)  obj = (NFOBJECT*)nfui_checkbutton_new(g_sdd[j].email);
			else if(i == ACTION_FTP)    obj = (NFOBJECT*)nfui_checkbutton_new(g_sdd[j].ftp);
#if defined(_SUPPORT_USER_PHONE)
			else if(i == ACTION_MOBILE)	obj = (NFOBJECT*)nfui_checkbutton_new(g_sdd[j].mobile);
#endif
			else if(i == ACTION_MOBILEPUSH)	obj = (NFOBJECT*)nfui_checkbutton_new(g_sdd[j].mobilepush);
	...
	nfui_nfobject_set_data(g_opopObj[EVENT_TYPE_COUNT-1], "bottom focus", obj);
