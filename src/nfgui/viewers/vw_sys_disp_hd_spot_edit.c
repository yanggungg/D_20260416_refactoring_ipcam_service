...
static SpotElementData g_elem_data[16];
...
static gboolean post_modifybutton_event_handler(NFOBJECT *obj, GdkEvent *evt, gpointer data)
{
	if(evt->type == GDK_BUTTON_RELEASE)
	{
		...
		HD_SpotConf_Open(g_curwnd, g_elem_data, &g_num_items);

		g_memmove(g_spot_data->items, g_elem_data, sizeof(SpotElementData)*16);
...
guint HD_SpotEditDlg_Open(NFWINDOW *parent, SpotData *spot_data)
{
	...
	g_memmove(g_elem_data, spot_data->items, sizeof(SpotElementData)*16);
	...
	nfui_nfobject_set_size(lbSpotTitle, 164, 40);
	nfui_nffixed_put((NFFIXED*)fixed1, lbSpotTitle, pos_x - 10, pos_y);
