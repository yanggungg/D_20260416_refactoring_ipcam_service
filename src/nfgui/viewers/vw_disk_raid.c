...
void VW_Init_DiskRaid_Page(NFOBJECT *parent)
{
	...
	nftab = nfui_nftab_new(DISK_TAB, (gchar**)strImage_h, 300, 40, NFTAB_DIR_H, strTabTitle, colidx);
	...
	VW_Init_DiskInternal_Raid_Page(tab_page[0]);
	
	if(DISK_TAB != 1)
	VW_Init_DiskExternal_Raid_Page(tab_page[1]);
...
gboolean display_disk_raid(gboolean expose)
{
	display_internal_raid(expose);
	
	if(DISK_TAB != 1)
	display_external_raid(expose);
}
