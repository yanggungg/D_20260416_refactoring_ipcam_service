/*
 * vw_dvabx_component_rule_area.c
 *
...
static gboolean post_all_detect_object_event_handler(NFOBJECT *obj, GdkEvent *evt, gpointer data)
{
    if(evt->type == NFEVENT_CHECKBUTTON_CHANGED)
    {
        ...
        component_data->area.all_detect_obj = status;
        ...
        if (component_data->area.all_detect_obj) 
        {
            ...
            snprintf(component_data->area.interest_obj, sizeof(component_data->area.interest_obj)-1, "person,car,bike,%s", nfui_nflabel_get_text((NFLABEL*)manual_label));
        }
        else
        {
            ...
            snprintf(component_data->area.interest_obj, sizeof(component_data->area.interest_obj)-1, "%s", nfui_nflabel_get_text((NFLABEL*)manual_label));
        }
    ...
    else if (evt->type == NFEVENT_DVA_COMPONENT_DATA_SYNC)
    {
        ...
        top = nfui_nfobject_get_top(obj);
        component_data = (DVA_COMPONENT_DATA_T*)nfui_nfobject_get_data((NFOBJECT*)top, DVA_COMPONENT_DATA);
        nfui_check_button_set_active_no_expose((NFCHECKBUTTON*)obj, component_data->area.all_detect_obj);
    }
...
static gboolean post_object_filter_check_event_handler(NFOBJECT *obj, GdkEvent *evt, gpointer data)
{
    if(evt->type == NFEVENT_CHECKBUTTON_CHANGED)
    {
        ...
        else
        {
            ...
            component_data->area.all_detect_obj = 0;
            nfui_signal_emit(g_all_chk_obj, GDK_EXPOSE, TRUE);
        }
        ...
        memset(component_data->area.interest_obj, 0x00, sizeof(component_data->area.interest_obj));
        snprintf(component_data->area.interest_obj, sizeof(component_data->area.interest_obj)-1, "%s", filter_string);
    }
    else if (evt->type == NFEVENT_DVA_COMPONENT_DATA_SYNC)
    {
        NFOBJECT *top;
        ...
        g_message("%s, %d, interest_obj:%s", __FUNCTION__, __LINE__, component_data->area.interest_obj);
        ...
        snprintf(tmp_interest, sizeof(tmp_interest), "%s", component_data->area.interest_obj);
        ...
        //if (component_data->area.all_detect_obj) nfui_nfobject_disable(obj);
...
static gboolean post_object_filter_label_event_handler(NFOBJECT *obj, GdkEvent *evt, gpointer data)
{
    if (evt->type == NFEVENT_DVA_COMPONENT_DATA_SYNC)
    {
        ...
        snprintf(org_filter, sizeof(org_filter)-1, ",%s", component_data->area.interest_obj);
...
static gboolean post_object_filter_edit_button_event_handler(NFOBJECT *obj, GdkEvent *evt, gpointer data)
{
    if (evt->type == GDK_BUTTON_RELEASE)
    {
        ...
        vw_dvabx_object_filter_edit_popup_open((NFWINDOW*)top, 1300, 310, exception_string, filter_string, sizeof(filter_string)-sizeof("person,car,bike,"));
...
static gboolean post_audio_edit_button_event_handler(NFOBJECT *obj, GdkEvent *evt, gpointer data)
{
    ...
    else if (evt->type == NFEVENT_DVA_COMPONENT_DATA_SYNC)
    {
        ...
        //if (component_data->area.all_detect_obj) nfui_nfobject_disable(obj);
        if (component_data->area.intrusion) nfui_nfobject_enable(obj);
        else if (component_data->area.enter) nfui_nfobject_enable(obj);
        else if (component_data->area.exit) nfui_nfobject_enable(obj);
        else if (component_data->area.removed) nfui_nfobject_enable(obj);
        else if (component_data->area.loitering) nfui_nfobject_enable(obj);
        else if (component_data->area.stopped) nfui_nfobject_enable(obj);
...
static gboolean post_removed_spin_event_handler(NFOBJECT *obj, GdkEvent *evt, gpointer data)
{
    ...
    else if(evt->type == GDK_2BUTTON_PRESS)
    {
        ...
        numTemp = NumberKey_Open(top, nfui_spin_button_get_value((NFSPINBUTTON*)obj), x, y, 60);
...
static gboolean post_counter_add_button_event_handler(NFOBJECT *obj, GdkEvent *evt, gpointer data)
{
    ...
    if (evt->type == GDK_BUTTON_RELEASE)
    {
        ...
        top = nfui_nfobject_get_top(obj);
        component_action = (DVA_COMPONENT_ACTION_T*)(nfui_nfobject_get_data((NFOBJECT*)top, DVA_COMPONENT_ACTION));
        component_action->area_connected_counter_cb(obj);
    }
...
static gboolean post_event_spin_event_handler(NFOBJECT *obj, GdkEvent *evt, gpointer data)
{
     if(evt->type == NFEVENT_SPINBUTTON_CHANGED)
    {
        ...
        value = nfui_spin_button_get_value((NFSPINBUTTON*)obj);
        //component_data->area.stopped_val = value;
        component_data->area.c_threshold = value;
    }
    else if(evt->type == GDK_2BUTTON_PRESS)
    {
        gint numTemp;
        ...
        */  
        ...
        nfui_signal_emit(obj, GDK_EXPOSE, TRUE);
        //component_data->area.stopped_val = nfui_spin_button_get_value((NFSPINBUTTON*)obj);
        component_data->area.c_threshold = nfui_spin_button_get_value((NFSPINBUTTON*)obj);
    }
    else if (evt->type == NFEVENT_DVA_COMPONENT_DATA_SYNC)
    {
        NFOBJECT *top;
        ...
        if(component_data->area.c_threshold != value)
        {
            nfui_spin_button_set_index_no_expose((NFSPINBUTTON*)obj, (component_data->area.c_threshold)-1);
        }
    }
...
gint vw_dvabx_rule_area_component_open(NFOBJECT *parent, guint opt)
{
    ...
    nfui_nfobject_show(obj);
    nfui_nffixed_put((NFFIXED*)fixed, obj, pos_x, pos_y);
    nfui_regi_post_event_callback(obj, post_intrusion_check_event_handler);
    pos_x += 30;

    obj = (NFOBJECT*)nfui_nflabel_new_with_pango_font("INTRUSION", nffont_get_pango_font(NFFONT_SMALL_SEMI), COLOR_IDX(205));
    nfui_nflabel_set_align((NFLABEL*)obj, NFALIGN_LEFT, 5);
    nfui_nfobject_modify_bg(obj, NFOBJECT_STATE_NORMAL, COLOR_IDX(200));
    nfui_nfobject_use_focus(obj, NFOBJECT_FOCUS_OFF);
    nfui_nfobject_set_size(obj, 200, 30);
    ...
    //if ((ivsc.vendor_code == 28) || (ivsc.vendor_code == 128)) nfui_nfobject_hide(obj);
    ...
    obj = nfui_spinbutton_new_value_with_range(5, 5, 60, 1); 
    nfui_spinbutton_set_skin_type((NFSPINBUTTON*)obj, NFSPINBUTTON_TYPE_POPUP_SMALL_1);
    nfui_spin_button_set_pango_font((NFSPINBUTTON*)obj, nffont_get_pango_font(NFFONT_SMALL_SEMI));
    nfui_nfobject_set_size(obj, 100, 30);
    nfui_nfobject_show(obj);                
...
//CONFIDENCE THRESHOLD
    ...
    if (opt & (1 << OPT_AREA_CONNECTED_COUNTER))
    {
        pos_y += 70;
