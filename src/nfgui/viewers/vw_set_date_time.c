...
static void prvRefreshDateTime()
{
	...
	dtf_get_local_day(idt.tmp, &year, &mon, &day);
	dtf_get_local_hourmin(idt.tmp, &hour, &min, &sec);
...
static void _process_up(NFOBJECT *obj)
{
	...
	stTime = *LOCALTIME_R(&idt.tmp);
	stNFTime_temp1 = *NFLOCALTIME(&idt.tmp);
	...
	if(obj==time_up[SET_DT_YEAR] || obj==time_up[SET_DT_MON] || obj==time_up[SET_DT_DAY])
	{
		stNFTime_temp2 = *NFLOCALTIME(&idt.tmp);

		if(stNFTime_temp1.tm_isdst != stNFTime_temp2.tm_isdst)
		{
			if(stNFTime_temp1.tm_isdst)
				idt.tmp += dst_interval;
			else
				idt.tmp -= dst_interval;
		}
	}
...
// <---- TITLE
	...
	dtf_get_local_day(c_time, &year, &mon, &day);
	dtf_get_local_hourmin(c_time, &hour, &min, &sec);
