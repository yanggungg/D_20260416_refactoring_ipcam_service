...
static void _get_path_DataSave(char *path)
{
	...
	g_sprintf(path, "%s/%s.ndb", mnt_dir, file_name);	
...
static gboolean post_savebutton_event_handler(NFOBJECT *obj, GdkEvent *evt, gpointer data)
{
	...
	if(evt->type == GDK_BUTTON_RELEASE)
	{
		...
		ret = scm_export_db(full_file_name);
		if (ret == 0) {
			nftool_mbox(g_curwnd, "INFORMATION", "The system data has been saved successfully.", NFTOOL_MB_OK);
		}
		else {
			vw_mbox(g_curwnd, "INFORMATION", IMBX_FAIL_DB_SAVE, NFTOOL_MB_OK);
		}
	}
...
void VW_System_Data_Save_Open(NFWINDOW *parent, gchar *title, guint pos_x, guint pos_y)
{
	NFOBJECT *sds_win;
	NFOBJECT *main_fixed;
...
// <---- WINDOW
	sds_win = (NFOBJECT*)nfui_nfwindow_new(parent, pos_x, pos_y, SDS_WIN_SIZE_W, SDS_WIN_SIZE_H);
	g_curwnd = sds_win;
...
// <---- CANCEL BUTTON
	...
	nfui_page_open(PGID_SYS_SAVE_DATA_POPUP, sds_win, ssm_get_cur_id(NULL));

	gtk_main();

	nfui_page_close(PGID_SYS_SAVE_DATA_POPUP, sds_win);
}
