...
static void _out_handler_ase_main(gint page)
{
	mb_type ret;

    if (page == mcf.sys_alarm_sensor.menu_pos[SYS_ALARM_SENSOR_CAM])
    {
        VW_AlarmSensorEvt_Cam_tab_out_handler();
    }
    else if (page == mcf.sys_alarm_sensor.menu_pos[SYS_ALARM_SENSOR_LOCAL])
    {
#if defined(_IPX_MODEL_UX)
        VW_AlarmSensorEvt_NVR_tab_out_handler();
#endif        
    }
}
...
static gboolean pre_subtab_event_handler(NFOBJECT *obj, GdkEvent *evt, gpointer data)
{
	...
	if(evt->type == NFEVENT_TAB_BEFORE_CHANGE)
	{
		...
		_out_handler_ase_main(cur_page);
	}
...
void VW_Init_AlarmSensorEvt_Page(NFOBJECT *parent)
{
	...
	vw_menu_get_str_sys_menu_alarm_sensor_evt(strTabTitle);
	...
	nftab = nfui_nftab_new(mcf.sys_alarm_sensor.cnt, (gchar**)strImage_h, 250, 40, NFTAB_DIR_H, (gchar**)strTabTitle, colidx);
	...
	for (i = 0; i < mcf.sys_alarm_sensor.cnt; i++)
	{
    ...
    pos = mcf.sys_alarm_sensor.menu_pos[SYS_ALARM_SENSOR_CAM];	
	if (pos != -1) VW_Init_AlarmSensorEvt_Cam_Page(tab_page[pos]);

    pos = mcf.sys_alarm_sensor.menu_pos[SYS_ALARM_SENSOR_LOCAL];	
#if defined(_IPX_MODEL_UX)    
	if (pos != -1) VW_Init_AlarmSensorEvt_NVR_Page(tab_page[pos]);
#endif

	nfui_nfobject_show(tab_page[0]);
}

gboolean VW_AlarmSensorEvt_tab_out_handler()
{
    gint pos;
	gchar page_mask = 0;
	mb_type ret;
