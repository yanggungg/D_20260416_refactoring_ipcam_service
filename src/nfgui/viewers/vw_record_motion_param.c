...
#define MOTION_PARAM_TIME_LABEL_X               (4+292+2)
#define MOTION_PARAM_TIME_LABEL_Y               (0)
#define MOTION_PARAM_TIME_LABEL_W               (48)
#define MOTION_PARAM_TIME_LABEL_H               (40)

#define MOTION_PARAM_TIME_LABEL_COLS_SPACE      (2)

#define MOTION_PARAM_TILE_X                     (MOTION_PARAM_TIME_LABEL_X - 1)
#define MOTION_PARAM_TILE_Y                     (MOTION_PARAM_TIME_LABEL_Y+MOTION_PARAM_TIME_LABEL_H+2)
#define MOTION_PARAM_TILE_W                     ((MOTION_PARAM_TIME_LABEL_W+2) * 24)
#define MOTION_PARAM_TILE_H                     (MOTION_PARAM_TIME_LABEL_H)

#define MOTION_PARAM_DAY_MENU_W_SUM             (284)

#define MOTION_PARAM_DAY_LABEL_X                (MOTION_PARAM_TABLE_X)
#define MOTION_PARAM_DAY_LABEL_Y                (MOTION_PARAM_TILE_Y)
#define MOTION_PARAM_DAY_LABEL_W                (60)
#define MOTION_PARAM_DAY_LABEL_H                (40)

#define MOTION_PARAM_DAY_DROPDOWN_X             (MOTION_PARAM_TABLE_X + MOTION_PARAM_DAY_LABEL_W)
#define MOTION_PARAM_DAY_DROPDOWN_Y             (MOTION_PARAM_DAY_LABEL_Y)
#define MOTION_PARAM_DAY_DROPDOWN_W             (MOTION_PARAM_DAY_MENU_W_SUM - MOTION_PARAM_DAY_LABEL_W)
#define MOTION_PARAM_DAY_DROPDOWN_H             (MOTION_PARAM_DAY_LABEL_H)

#define MOTION_PARAM_TABLE_COL_CNT              (5)
#define MOTION_PARAM_TABLE_ROW_CNT              (GUI_CHANNEL_CNT)

#define MOTION_PARAM_TABLE_X                    (4)
#define MOTION_PARAM_TABLE_Y                    (MOTION_PARAM_TILE_Y+MOTION_PARAM_TILE_H+27)
#define MOTION_PARAM_TABLE_CELL_H               (39)

#define MOTION_PARAM_TABLE_COLS_SPACE           (2)
#define MOTION_PARAM_TABLE_ROWS_SPACE           (2)

#define MOTION_PARAM_HELP_BTN_X             (MOTION_PARAM_TABLE_X)
#define MOTION_PARAM_HELP_BTN_Y             (MENU_V_SUBTAB_INNER_H - 40 - 4)
...
static void _set_size_data_text_color(gint ch)
{
    ...
    val = get_motion_size_data_modify(ch);
    ...
    else
    {
        if (fg_color != 921)
        {
            nfui_nflabel_modify_fg(lbSize[ch], COLOR_IDX(921));
            nfui_signal_emit(lbSize[ch], GDK_EXPOSE, TRUE);
        }
    }
}
...
static void _set_fps_data_text_color(gint ch)
{
    ...
    val = get_motion_fps_data_modify(ch);
...
static void display_motion_rec_param_info(gboolean redraw) 
{
    gint i;
    ...
    for(i=0; i<GUI_CHANNEL_CNT; i++) {
        ...
        if(strcmp(size, motion_info[i][PARAM_SIZE_COL]) != 0) {
            nfui_nflabel_set_text((NFLABEL*)lbSize[i], motion_info[i][PARAM_SIZE_COL]);
        ...
        if(strcmp(fps, motion_info[i][PARAM_FPS_COL]) != 0) {
            nfui_nflabel_set_text((NFLABEL*)lbFPS[i], motion_info[i][PARAM_FPS_COL]);
        ...
        if(strcmp(qual, motion_info[i][PARAM_QUAL_COL]) != 0) {
            nfui_nflabel_set_text((NFLABEL*)lbQuality[i], motion_info[i][PARAM_QUAL_COL]);

            if(redraw) 
                nfui_signal_emit(lbQuality[i], GDK_EXPOSE, TRUE);
        }

        if(strcmp(audio, motion_info[i][PARAM_AUDIO_COL]) != 0) {
            nfui_nflabel_set_text((NFLABEL*)lbAudio[i], motion_info[i][PARAM_AUDIO_COL]);
...
static gboolean post_applybutton_event_handler(NFOBJECT *obj, GdkEvent *evt, gpointer data)
{
    if(evt->type == GDK_BUTTON_RELEASE)
    {
        ...
        if(is_changed_motion_data()) {      
            ...
            vw_mbox_auto(g_curwnd, 1, "NOTICE", IMBX_CONF_SAVED);
        }
    }
    ...
    if(evt->type == GDK_BUTTON_RELEASE)
    {
        ...
        if(get_record_schedMode()) {
            ...
            if(!get_record_motion_db(set_days, 0))
                g_error("%s : get record motion error", __FUNCTION__);

            display_motion_rec_param_info(TRUE);
        }else {
...
static gboolean tile_event_handler(NFOBJECT *obj, GdkEvent *evt, gpointer data)
{
    ...
    switch(evt->type) {
        ...
        {       
            ...
            if( sche_mode )
                get_record_motion_param_data(set_days, e_col);
            else 
                get_record_motion_param_data(REC_DAILY_PARAM, e_col);
        ...
        case NFEVENT_TILE_END_SELECT:
        {
            ...
            if(VW_RecParam_Submenu_Open(g_curwnd, motion_info, REC_PARAM_NORMAL_OPEN))
            {   
                sche_mode = get_record_schedMode();
                ...
                if( sche_mode )
                {
                    ...
                    set_record_motion_param_info(set_days, s_col, e_col);
                }
                else
                {
                    set_record_motion_param_info(REC_DAILY_PARAM, s_col, e_col);
                }
...
static gboolean days_combo_event_handler(NFOBJECT *obj, GdkEvent *evt, gpointer data)
{
    ...
    if(evt->type == NFEVENT_COMBOBOX_CHANGED){
        ...
        if(day) {
            get_record_motion_param_data(get_days(day), 0);
            display_motion_rec_param_info(TRUE);
        }
    }
    ...
    if(evt->type == GDK_EXPOSE) {
        if(get_record_schedMode()) {            //weekly
            if(days_combo->show == NFOBJECT_HIDE) {
                ...
                get_record_motion_param_data(get_days(day), 0);             
                display_motion_rec_param_info(FALSE);
            }
            ...
            if(days_combo->show == NFOBJECT_SHOW) {
                ...
                get_record_motion_param_data(REC_DAILY_PARAM, 0);
                display_motion_rec_param_info(FALSE);
            }
...
void VW_Init_RecMotionParam_Page(NFOBJECT *parent)
{
    ...
    gchar *strTitle[MOTION_PARAM_TABLE_COL_CNT] = {"", "RESOLUTION", "FPS", "QUALITY", "AUDIO"};
    gchar *days[8] = {"SUN", "MON", "TUE", "WED", "THU", "FRI", "SAT", "HOL"};
    gchar strBuf[STRING_SIZE_64];
    gchar image_name[64];
    
    guint width[MOTION_PARAM_TABLE_COL_CNT] = {0, };
    gint i;
    gint pos_x, pos_y;
...
// TIME LABEL
    for (i = 0; i < 24; i++)
    {
        ...
        nfui_nfobject_set_size(obj, MOTION_PARAM_TIME_LABEL_W, MOTION_PARAM_TIME_LABEL_H);
        nfui_nflabel_set_align((NFLABEL*)obj, NFALIGN_CENTER, 0);
        nfui_nfobject_modify_bg(obj, NFOBJECT_STATE_NORMAL, COLOR_IDX(115));
        nfui_nfobject_show(obj);
        nfui_nffixed_put((NFFIXED*)content_fixed, obj, MOTION_PARAM_TIME_LABEL_X + (MOTION_PARAM_TIME_LABEL_COLS_SPACE+MOTION_PARAM_TIME_LABEL_W)*i, MOTION_PARAM_TIME_LABEL_Y);
    }
...
// TIME TILE
    ...
    nfui_nfobject_set_size(nftile, MOTION_PARAM_TILE_W, MOTION_PARAM_TILE_H);
    nfui_regi_post_event_callback(nftile, tile_event_handler);
    nfui_nfobject_show(nftile);
    nfui_nffixed_put((NFFIXED*)content_fixed, nftile, MOTION_PARAM_TILE_X, MOTION_PARAM_TILE_Y);
...
// RECORD TABLE
    ...
    pos_x = MOTION_PARAM_TABLE_X;

    for(i=0; i<MOTION_PARAM_TABLE_COL_CNT; i++)
    {
        if (i != 0)
        {
            obj = (NFOBJECT*)nfui_nflabel_new_with_pango_font(strTitle[i], nffont_get_pango_font(NFFONT_MEDIUM_SEMI), COLOR_IDX(116));      
            nfui_nfobject_use_focus(obj, NFOBJECT_FOCUS_OFF);
            nfui_nfobject_set_size(obj, width[i], MOTION_PARAM_TABLE_CELL_H);
            nfui_nfobject_modify_bg(obj, NFOBJECT_STATE_NORMAL, COLOR_IDX(115));
            nfui_nfobject_show(obj);
            nfui_nffixed_put((NFFIXED*)content_fixed, obj, pos_x, MOTION_PARAM_TABLE_Y);
        }
    ...
    nfui_nffixed_put((NFFIXED*)content_fixed, scrolled_fixed, MOTION_PARAM_TABLE_X, MOTION_PARAM_TABLE_Y + 41);

    ntb1 = (NFOBJECT*)nfui_nftable_new(MOTION_PARAM_TABLE_COL_CNT, MOTION_PARAM_TABLE_ROW_CNT, MOTION_PARAM_TABLE_COLS_SPACE, MOTION_PARAM_TABLE_ROWS_SPACE, width, MOTION_PARAM_TABLE_CELL_H);
    ...
    for(i=0; i<GUI_CHANNEL_CNT; i++)
    {
        ...
        lbSize[i] = (NFOBJECT*)nfui_nflabel_new_text_box(motion_info[i][PARAM_SIZE_COL], nffont_get_pango_font(NFFONT_MEDIUM_SEMI));
        nfui_nflabel_set_skin_type((NFLABEL*)lbSize[i], NFTEXTBOX_TYPE_SUBTAB_OUTPUT);
        lbFPS[i] = (NFOBJECT*)nfui_nflabel_new_text_box(motion_info[i][PARAM_FPS_COL], nffont_get_pango_font(NFFONT_MEDIUM_SEMI));
        nfui_nflabel_set_skin_type((NFLABEL*)lbFPS[i], NFTEXTBOX_TYPE_SUBTAB_OUTPUT);
        lbQuality[i] = (NFOBJECT*)nfui_nflabel_new_text_box(motion_info[i][PARAM_QUAL_COL], nffont_get_pango_font(NFFONT_MEDIUM_SEMI));
        nfui_nflabel_set_skin_type((NFLABEL*)lbQuality[i], NFTEXTBOX_TYPE_SUBTAB_OUTPUT);
        lbAudio[i] = (NFOBJECT*)nfui_nflabel_new_text_box(motion_info[i][PARAM_AUDIO_COL], nffont_get_pango_font(NFFONT_MEDIUM_SEMI));
...
// DAY LABEL
    ...
    nfui_nfobject_set_size(days_label, MOTION_PARAM_DAY_LABEL_W, MOTION_PARAM_DAY_LABEL_H);
    ...
    nfui_nfobject_hide(days_label);
    nfui_nffixed_put((NFFIXED*)content_fixed, days_label, MOTION_PARAM_DAY_LABEL_X, MOTION_PARAM_DAY_LABEL_Y);


// DAY DROP DOWN
    days_combo = nfui_combobox_new(days, 8, 0);
    nfui_combobox_set_skin_type(NF_COMBOBOX(days_combo), NFCOMBOBOX_TYPE_SUBTAB_1);     
    nfui_nfobject_set_size(days_combo, MOTION_PARAM_DAY_DROPDOWN_W, MOTION_PARAM_DAY_DROPDOWN_H);
    nfui_nffixed_put((NFFIXED*)content_fixed, days_combo, MOTION_PARAM_DAY_DROPDOWN_X, MOTION_PARAM_DAY_DROPDOWN_Y);
    nfui_nfobject_hide(days_combo);
...
// HELP BUTTON
    ...
    nfui_nfobject_show(obj);
    nfui_nffixed_put((NFFIXED*)content_fixed, obj, MOTION_PARAM_HELP_BTN_X, MOTION_PARAM_HELP_BTN_Y);
...
// PARENT EVENT HANDLER
    nfui_regi_post_event_callback(parent, param_page_parent_event_handler);

}

void VW_RecMotionParam_data_reloaded(gboolean reload)
{
    if(isReloaded != reload)
        isReloaded = reload;
}

void VW_RecMotionParam_info_refresh()
{
    gint i;
    gchar *size, *fps, *qual, *audio;
