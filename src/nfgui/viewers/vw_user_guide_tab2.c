...
static gint _draw_guide_image()
{
    ...
    g_sprintf(image, "01_guide_%s_%02d.png", g_tmplang, g_pageIdx+1);
...
void init_UserGuide_tab2_page(NFOBJECT *parent)
{
	...
	ret = icf_get_value_by_int("guide2", "cnt");
    ...
    if (g_page_cnt > 1)
    {
	    ...
	    memset(image, 0x00, sizeof(image));
	    g_sprintf(image, "01_guide_%s_%02d.png", g_tmplang, 1);
...
gboolean UserGuide_tab2_in_handler()
{
    g_pageIdx = 0;   
	_change_page_label();
...
gboolean UserGuide_tab2_out_handler()
{
