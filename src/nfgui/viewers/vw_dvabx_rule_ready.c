...
static gboolean post_nextbutton_event_handler(NFOBJECT *obj, GdkEvent *evt, gpointer data)
{
	if(evt->type == GDK_BUTTON_RELEASE)
	{
		if(evt->button.button == MOUSE_RIGTH_BUTTON) 					
			return FALSE;
	    _dvabx_rule_area(g_parent);
	    _dvabx_rule_ready_hide(g_fixed);
...
gint _dvabx_rule_ready(NFOBJECT *parent)
{
	...
	//nfui_nfobject_modify_bg(fixed, NFOBJECT_STATE_NORMAL, COLOR_IDX(402));
	nfui_nfobject_set_size(fixed, g_parent->width, g_parent->height - DVABX_BTN1_H);
    ...
    opt = 0;
	vw_dvabx_rule_setup_component_open(g_fixed, opt);
	...
	obj = (NFOBJECT*)nftool_normal_button_create_subtab_type1("NEXT", DVABX_BTN_W);
	nfui_nfbutton_set_font_alignment(NF_BUTTON(obj), NFALIGN_CENTER, 0);	
