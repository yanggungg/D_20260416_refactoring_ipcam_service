...
static gboolean post_nextbutton_event_handler(NFOBJECT *obj, GdkEvent *evt, gpointer data)
{
	if(evt->type == GDK_BUTTON_RELEASE)
	{
		if(evt->button.button == MOUSE_RIGTH_BUTTON) 					
			return FALSE;
	    _vca_rev_rule_area(g_parent);
	    _vca_rev_rule_ready_hide(g_fixed);
...
gint _vca_rev_rule_ready(NFOBJECT *parent)
{
	...
	nfui_nfobject_set_size(fixed, g_parent->width, g_parent->height - VCA_REV_BTN1_H);
    ...
    opt = 0;
	vw_vca_rev_rule_setup_component_open(g_fixed, opt);
