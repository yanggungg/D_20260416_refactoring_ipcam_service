...
#include "vw_dvabx_modrule_popup.h"
...
gint vw_dvabx_modrule_popup_open(NFOBJECT *parent, gint ch, gchar *algorithm, gint rule_type, gint id)
{
    NFOBJECT *win;
    NFOBJECT *fixed;   
    NFOBJECT *obj;
...
// ADD MAIN FIXED   
	...
	component_data = (DVA_COMPONENT_DATA_T*)nfui_nfobject_get_data((NFOBJECT*)win, DVA_COMPONENT_DATA);
    
    if(rule_type == RTYPE_LINE)
    {
        nfui_nfobject_show(g_ruleline_fixed);
        _get_dvaa_line_component_data(id, component_data);
        vw_dvabx_rule_line_component_sync_data(g_ruleline_fixed);
    }
    else if(rule_type == RTYPE_AREA)
    {
        nfui_nfobject_show(g_rulearea_fixed);
        _get_dvaa_area_component_data(id, component_data);
        vw_dvabx_rule_area_component_sync_data(g_rulearea_fixed);
    }
    else if(rule_type == RTYPE_COUNTER)
    {
        nfui_nfobject_show(g_counter_fixed);
        _get_dvaa_counter_component_data(id, component_data);
        _set_dvaa_counter_list(component_data);
        vw_dvabx_counter_component_sync_data(g_counter_fixed);
    }
