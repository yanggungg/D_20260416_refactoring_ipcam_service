...
static gboolean post_okbutton_event_handler(NFOBJECT *obj, GdkEvent *evt, gpointer data)
{
	if(evt->type == GDK_BUTTON_RELEASE)
	{
		if(evt->button.button == MOUSE_RIGTH_BUTTON) 					
			return FALSE;
		_vca_rev_rule_page(g_parent);
		_vca_rev_rule_counter_hide(g_fixed);
	}
...
gint _vca_rev_rule_counter(NFOBJECT*parent)
{
	...
	nfui_nfobject_modify_bg(fixed, NFOBJECT_STATE_NORMAL, COLOR_IDX(408));
	...
	obj = (NFOBJECT*)nftool_normal_button_create_subtab_type1("ADD RULE", VCA_REV_BTN_W);
	nfui_nfbutton_set_font_alignment(NF_BUTTON(obj), NFALIGN_CENTER, 0);	
	nfui_regi_post_event_callback(obj, post_addbutton_event_handler);
	nfui_nfobject_show(obj);
	nfui_nffixed_put((NFFIXED*)g_parent, obj, VCA_REV_BTN1_X, VCA_REV_BTN1_Y);
	btn_add = obj;

	obj = (NFOBJECT*)nftool_normal_button_create_subtab_type1("OK", VCA_REV_BTN_W);
	nfui_nfbutton_set_font_alignment(NF_BUTTON(obj), NFALIGN_CENTER, 0);	
