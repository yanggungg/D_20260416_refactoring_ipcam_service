...
#define CONT_SCHE_TIME_LABEL_COLS_SPACE     (2)

#define CONT_SCHE_DAY_MENU_W_SUM            (284)

#define CONT_SCHE_DAY_LABEL_X               (4)
#define CONT_SCHE_DAY_LABEL_Y               (0)
#define CONT_SCHE_DAY_LABEL_W               (60)
#define CONT_SCHE_DAY_LABEL_H               (40)

#define CONT_SCHE_DAY_DROPDOWN_X            (CONT_SCHE_TABLE_X + CONT_SCHE_DAY_LABEL_W)
#define CONT_SCHE_DAY_DROPDOWN_Y            (CONT_SCHE_DAY_LABEL_Y)
#define CONT_SCHE_DAY_DROPDOWN_W            (CONT_SCHE_DAY_MENU_W_SUM - CONT_SCHE_DAY_LABEL_W)
#define CONT_SCHE_DAY_DROPDOWN_H            (CONT_SCHE_DAY_LABEL_H)

#define CONT_SCHE_CH_LABEL_W                (232)

#define CONT_SCHE_TABLE_COL_CNT             (1)
#define CONT_SCHE_TABLE_ROW_CNT             (GUI_CHANNEL_CNT)

#define CONT_SCHE_TIME_LABEL_X              (CONT_SCHE_DAY_LABEL_X+CONT_SCHE_CH_LABEL_W+2)
#define CONT_SCHE_TIME_LABEL_Y              (CONT_SCHE_DAY_LABEL_Y+CONT_SCHE_DAY_LABEL_H+27)
#define CONT_SCHE_TIME_LABEL_W              (48)
#define CONT_SCHE_TIME_LABEL_H              (40)

#define CONT_SCHE_TABLE_X                   (CONT_SCHE_DAY_LABEL_X)
#define CONT_SCHE_TABLE_Y                   (CONT_SCHE_DAY_LABEL_Y+CONT_SCHE_DAY_LABEL_H+68)
#define CONT_SCHE_TABLE_CELL_H              (39)

#define CONT_SCHE_TABLE_COLS_SPACE          (0)
#define CONT_SCHE_TABLE_ROWS_SPACE          (2)

#define CONT_SCHE_TILE2_ROW_CNT             (ROW_CNT_PER_PAGE)
#define CONT_SCHE_TILE2_COL_CNT             (24)

#define CONT_SCHE_TILE2_X                   (CONT_SCHE_CH_LABEL_W+2)
#define CONT_SCHE_TILE2_Y                   (CONT_SCHE_TABLE_Y-1)
#define CONT_SCHE_TILE2_W                   (((48+2) * 24))
#define CONT_SCHE_TILE2_H                   ((CONT_SCHE_TABLE_CELL_H+2)*CONT_SCHE_TILE2_ROW_CNT)

#define CONT_SCHE_COPY_BTN_X                (MENU_V_SUBTAB_INNER_W - 226 - 7)
#define CONT_SCHE_COPY_BTN_Y                (MENU_V_SUBTAB_INNER_H - 40 - 4)

#define CONT_SCHE_ON_LABEL_W                (80)
#define CONT_SCHE_ON_LABEL_H                (26)
#define CONT_SCHE_ON_LABEL_X                (MENU_V_SUBTAB_INNER_W - CONT_SCHE_ON_LABEL_W - 7)
#define CONT_SCHE_ON_LABEL_Y                (CONT_SCHE_DAY_LABEL_Y + CONT_SCHE_DAY_LABEL_H)

#define CONT_SCHE_ON_LEGEND_W               (22)
#define CONT_SCHE_ON_LEGEND_H               (22)
#define CONT_SCHE_ON_LEGEND_X               (CONT_SCHE_ON_LABEL_X - 6 - CONT_SCHE_ON_LEGEND_W)
#define CONT_SCHE_ON_LEGEND_Y               (CONT_SCHE_ON_LABEL_Y)

#define CONT_SCHE_OFF_LABEL_W               (80)
#define CONT_SCHE_OFF_LABEL_H               (26)
#define CONT_SCHE_OFF_LABEL_X               (CONT_SCHE_ON_LEGEND_X - CONT_SCHE_OFF_LABEL_W)
#define CONT_SCHE_OFF_LABEL_Y               (CONT_SCHE_ON_LABEL_Y)

#define CONT_SCHE_OFF_LEGEND_W              (22)
#define CONT_SCHE_OFF_LEGEND_H              (22)
#define CONT_SCHE_OFF_LEGEND_X              (CONT_SCHE_OFF_LABEL_X - CONT_SCHE_OFF_LEGEND_W - 7)
#define CONT_SCHE_OFF_LEGEND_Y              (CONT_SCHE_ON_LABEL_Y)
...
static void redraw_tile_modeArea(NFTILE *tile, guint day)
{
    ...
    s_ch = tile_idx * CONT_SCHE_TILE2_ROW_CNT;
    e_ch = s_ch + CONT_SCHE_TILE2_ROW_CNT;

    if(nfui_tile_drawable(NF_TILE(tile))) {
        for(ch = s_ch; ch < e_ch; ch++) {
            for(j = 0; j < time; j++) {
                get_record_continuous_sche_data(day, ch, j);

                if(!g_ascii_strcasecmp(continuous_info[ch][PARAM_MODE_COL], "0")) 
                    color_n = SELECT_STATE_COLOR_2;
                else if(!g_ascii_strcasecmp(continuous_info[ch][PARAM_MODE_COL], "1")) 
    ...
    switch(evt->type) {
        ...
        case NFEVENT_TILE_END_SELECT:
        {
            ...
            else
            {
                ...
                {
                    ch = i + (CONT_SCHE_TILE2_ROW_CNT * tile_idx);
                    
                    for(j = s_col; j <= e_col; j++) 
                    {
                        if(get_record_schedMode())  
                            get_record_continuous_sche_data(set_days, ch, j);
                        else                        
                            get_record_continuous_sche_data(REC_DAILY_PARAM, ch, j);

                        continuous_info[ch][PARAM_MODE_COL] = buf;

                        if(get_record_schedMode())  
                            set_record_continuous_sche_info(set_days, ch, j);
                        else                    
                            set_record_continuous_sche_info(REC_DAILY_PARAM, ch, j);
                    }
                }
...
static gboolean post_applybutton_event_handler(NFOBJECT *obj, GdkEvent *evt, gpointer data)
{
    if(evt->type == GDK_BUTTON_RELEASE)
    {
        ...
        if(is_changed_continuous_data()) {
            if(!set_record_continuous_db(REC_SCHED_DATA_CHANGED)) {
                g_warning("%s : set record continuous error", __FUNCTION__);
            }
...
static gboolean post_copy_button_event_handler(NFOBJECT *obj, GdkEvent *evt, gpointer data)
{
    if(evt->type == GDK_BUTTON_RELEASE)
    {
        ...
        for(ch = 0; ch < GUI_CHANNEL_CNT; ch++) 
        {
            ...
            {
                get_record_continuous_sche_data(set_days, ch, time);

                for (day = REC_SUN_PARAM; day <= REC_SAT_PARAM; day++) 
                {
                    if (day == set_days) continue;
                    if (day_mask & (1 << day))  
                        set_record_continuous_sche_info(day, ch, time);
                }
                
                if (day_mask & (1 << (REC_USR_HOL_PARAM) )) {
                    set_record_continuous_sche_info(REC_USR_HOL_PARAM, ch, time);
        		}
            }
        }
    }
    ...
    if(evt->type == GDK_BUTTON_RELEASE)
    {
        ...
        if(get_record_schedMode()) {
            if(!get_record_continuous_db(set_days, 0)) 
                g_warning("%s : get record continuous error", __FUNCTION__);
...
static gboolean sched_page_parent_event_handler(NFOBJECT *obj, GdkEvent *evt, gpointer data)
{
    ...
    if(evt->type == GDK_EXPOSE) {
        ...
        gdk_draw_rectangle(drawable, gc, TRUE, x+CONT_SCHE_OFF_LEGEND_X, y+CONT_SCHE_OFF_LEGEND_Y, CONT_SCHE_OFF_LEGEND_W, CONT_SCHE_OFF_LEGEND_H);

        gdk_gc_set_rgb_fg_color(gc, &on_color);                     // ON
        gdk_draw_rectangle(drawable, gc, TRUE, x+CONT_SCHE_ON_LEGEND_X, y+CONT_SCHE_ON_LEGEND_Y, CONT_SCHE_ON_LEGEND_W, CONT_SCHE_ON_LEGEND_H);
...
void VW_Init_RecContSched_Page(NFOBJECT *parent)
{
    ...
    guint width[] = {CONT_SCHE_CH_LABEL_W};
    guint x, y;
    guint i,space;
	gint size_w, size_h;
...
// TIME LABEL
    for (i = 0; i < 24; i++)
    {
        ...
        nfui_nfobject_set_size(obj, CONT_SCHE_TIME_LABEL_W, CONT_SCHE_TIME_LABEL_H);
        nfui_nflabel_set_align((NFLABEL*)obj, NFALIGN_CENTER, 0);
        nfui_nfobject_modify_bg(obj, NFOBJECT_STATE_NORMAL, COLOR_IDX(115));
        nfui_nfobject_show(obj);
        nfui_nffixed_put((NFFIXED*)content_fixed, obj, CONT_SCHE_TIME_LABEL_X + (CONT_SCHE_TIME_LABEL_COLS_SPACE+CONT_SCHE_TIME_LABEL_W)*i, CONT_SCHE_TIME_LABEL_Y);
    }
...
// DAY LABEL
    ...
    nfui_nfobject_set_size(days_label, CONT_SCHE_DAY_LABEL_W, CONT_SCHE_DAY_LABEL_H);
    ...
    nfui_nfobject_hide(days_label);
    nfui_nffixed_put((NFFIXED*)content_fixed, days_label, CONT_SCHE_DAY_LABEL_X, CONT_SCHE_DAY_LABEL_Y);


// DAY DROP DOWN
    days_combo = nfui_combobox_new(days, 8, 0);
    nfui_combobox_set_skin_type(NF_COMBOBOX(days_combo), NFCOMBOBOX_TYPE_SUBTAB_1);     
    nfui_nfobject_set_size(days_combo, CONT_SCHE_DAY_DROPDOWN_W, CONT_SCHE_DAY_DROPDOWN_H);
    nfui_nfobject_hide(days_combo);
    nfui_nffixed_put((NFFIXED*)content_fixed, days_combo, CONT_SCHE_DAY_DROPDOWN_X, CONT_SCHE_DAY_DROPDOWN_Y);
...
// LEGEND LABEL - ON, OFF
    ...
    nfui_nfobject_use_focus(obj, NFOBJECT_FOCUS_OFF);
    nfui_nfobject_set_size(obj, CONT_SCHE_OFF_LABEL_W, CONT_SCHE_OFF_LABEL_H);
    nfui_nflabel_set_align((NFLABEL*)obj, NFALIGN_LEFT, 0);
    nfui_nfobject_modify_bg(obj, NFOBJECT_STATE_NORMAL, COLOR_IDX(186));
    nfui_nfobject_show(obj);
    nfui_nffixed_put((NFFIXED*)content_fixed, obj, CONT_SCHE_OFF_LABEL_X, CONT_SCHE_OFF_LABEL_Y);

    obj = (NFOBJECT*)nfui_nflabel_new_with_pango_font("ON", nffont_get_pango_font(NFFONT_SMALL_SEMI), COLOR_IDX(968));
    nfui_nfobject_use_focus(obj, NFOBJECT_FOCUS_OFF);
    nfui_nfobject_set_size(obj, CONT_SCHE_ON_LABEL_W, CONT_SCHE_ON_LABEL_H);
    nfui_nflabel_set_align((NFLABEL*)obj, NFALIGN_LEFT, 0);
    nfui_nfobject_modify_bg(obj, NFOBJECT_STATE_NORMAL, COLOR_IDX(186));
    nfui_nfobject_show(obj);
    nfui_nffixed_put((NFFIXED*)content_fixed, obj, CONT_SCHE_ON_LABEL_X, CONT_SCHE_ON_LABEL_Y);


// TABLE - CH   
    size_w = 0;
    for (i = 0; i < CONT_SCHE_TABLE_COL_CNT; i++) {
        size_w += width[i];
    }
    size_w += CONT_SCHE_TILE2_W;
    ...
    nfui_nffixed_put((NFFIXED*)content_fixed, main_page_fixed, CONT_SCHE_TABLE_X, CONT_SCHE_TABLE_Y);
    ...
    for (i = 0; i < PAGE_FIXED_CNT; i++)
    {
        ...
        page_ntb[i] = (NFOBJECT*)nfui_nftable_new(CONT_SCHE_TABLE_COL_CNT, ROW_CNT_PER_PAGE, CONT_SCHE_TABLE_COLS_SPACE, 1, width, 40);
...
// TIME TILE
    for (i = 0; i < PAGE_FIXED_CNT; i++)
    {
        nftile = nfui_tile_new(CONT_SCHE_TILE2_ROW_CNT, CONT_SCHE_TILE2_COL_CNT);
        nfui_nfobject_set_size(nftile, CONT_SCHE_TILE2_W, CONT_SCHE_TILE2_H);
    	...
    	nfui_nffixed_put((NFFIXED*)g_page_fixed[i], nftile, CONT_SCHE_TILE2_X, 0);
...
// COPY SCHEDULE TO BUTTON
    ...
    nfui_nffixed_put((NFFIXED*)content_fixed, obj, CONT_SCHE_COPY_BTN_X, CONT_SCHE_COPY_BTN_Y);
...
guint VW_RecContParam_get_sched_day()
{
    if(get_record_schedMode())
        return set_days;
    else
