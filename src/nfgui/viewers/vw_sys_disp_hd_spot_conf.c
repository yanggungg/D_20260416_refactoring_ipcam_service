...
#include "vw_sys_disp_hd_spot_conf.h"
#include "vw_sys_disp_hd_spot_setup.h"
#include "vw_sys_disp_hd_spot_conf_submenu.h"
...
static void _process_popup_ret_val(gint val)
{
	...
	if (val == 0)			// modify
	{
		...
		ret = HD_SpotSetupDlg_Open(g_curwnd, &g_elem_data[cur_focus], FALSE);
...
static gboolean pre_conf_fixed_event_handler(NFOBJECT *obj, GdkEvent *evt,  gpointer data)
{
	...
	else if(evt->type == GDK_BUTTON_RELEASE)
	{
		...
		ret_val = VW_Create_HD_Spot_SubMenu(g_curwnd, x, y);
...
static gboolean post_add_btn_event_handler(NFOBJECT *obj, GdkEvent *evt, gpointer data)
{
	if(evt->type == GDK_BUTTON_RELEASE)
	{
		...
		ret = HD_SpotSetupDlg_Open(g_curwnd, &g_elem_data[*g_num_items], TRUE);
...
void HD_SpotConf_Open(NFWINDOW *parent, SpotElementData *elem_data, guint *num_items)
{
	NFOBJECT *main_win, *main_fixed;
	NFOBJECT *add_btn, *close_btn;
	guint i;
	...
	nfui_nffixed_put((NFFIXED*)main_fixed, add_btn, 210, CONF_BTN_POS_Y);
	nfui_nffixed_put((NFFIXED*)main_fixed, close_btn, 210+198, CONF_BTN_POS_Y);
	...
	nfui_page_open(PGID_HD_SPOT_CONF, main_win, nfui_get_last_user());

	gtk_main();

	nfui_page_close(PGID_HD_SPOT_CONF, main_win);
}
