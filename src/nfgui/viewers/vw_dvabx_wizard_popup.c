...
#include "vw_dvabx_wizard_popup.h"
...
static ITX_DVARULE_PROP g_pre_rule_prop;
...
static ITX_DVACALB_CONF g_pre_calb_conf[32];
static ITX_DVACALB_SHAPE g_pre_calb_shape[32];
static ITX_DVACALB_RESULT g_pre_calb_result;
...
static gint _set_dvaa_prop(DVA_COMPONENT_DATA_T *component_data)
{
    DVAAID dvaaid;
...
static gint _delete_dvaa_line(DVA_COMPONENT_DATA_T *component_data)
{
    DVAAID dvaaid;
    ITX_DVAZONE_CONF conf;
    ...
    dvaa_itx_detector_get_zone_conf(dvaaid, component_data->zone_id, &conf);   
    conf.use_zone = 0;
    dvaa_itx_detector_set_zone_conf(dvaaid, component_data->zone_id, &conf);

    component_data->zone_id = -1;
    return 0;
}

static gint _delete_dvaa_area(DVA_COMPONENT_DATA_T *component_data)
{
    DVAAID dvaaid;
    ITX_DVAZONE_CONF conf;
...
static gint _delete_dvaa_counter(DVA_COMPONENT_DATA_T *component_data)
{
    DVAAID dvaaid;
    ITX_DVACNTR_CONF conf;
    ...
    dvaa_itx_detector_get_cntr_conf(dvaaid, component_data->cntr_id, &conf);   
    conf.use_cntr = 0;
    dvaa_itx_detector_set_cntr_conf(dvaaid, component_data->cntr_id, &conf);
...
static gint _get_calb_shape_from_zoomdata(DVAAID dvaaid, ivca_calib_t *vca_calb)
{
    gint i;
    ITX_DVACALB_SHAPE shape;    
    ...
    for (i = 0; i < 32; i++)
    {
        ...
        dvaa_itx_detector_set_calb_shape(dvaaid, i, &shape);
    }

    return 0;
}

static gint _get_calb_conf_from_zoomdata(DVAAID dvaaid, ivca_calib_t *vca_calb)
{
    gint i;
    ITX_DVACALB_CONF conf;    
    ...
    for (i = 0; i < 32; i++)
    {
        ...
        dvaa_itx_detector_set_calb_conf(dvaaid, i, &conf);
    }
...
static gint _set_default_calibration_component_data(DVA_COMPONENT_DATA_T *component_data)
{
...
static gint _set_default_counter_component_data(DVA_COMPONENT_DATA_T *component_data)
{
    strcpy(component_data->counter.name, "");
...
static gint _counter_component_use_cb(gpointer user_data)
{
    ...
    {
        _delete_dvaa_counter(component_data);
        vw_dvabx_counter_component_sync_data(g_counter_fixed);
    }
...
static gboolean post_closebutton_event_handler(NFOBJECT *obj, GdkEvent *evt, gpointer data)
{
	if (evt->type == GDK_BUTTON_RELEASE)
	{
        ...
        dvaa_itx_detector_set_rule_prop(dvaaid, &g_pre_rule_prop);
        ...
        for (i = 0; i < 32; i++)
        {
            dvaa_itx_detector_set_calb_conf(dvaaid, i, &g_pre_calb_conf[i]);
            dvaa_itx_detector_set_calb_shape(dvaaid, i, &g_pre_calb_shape[i]);
        }    

        dvaa_itx_detector_set_calb_result(dvaaid, &g_pre_calb_result);
...
static gboolean post_ruleline_prevbutton_event_handler(NFOBJECT *obj, GdkEvent *evt, gpointer data)
{
    if (evt->type == GDK_BUTTON_RELEASE)
    {
        ...
        _delete_dvaa_line(component_data);
...
static gboolean post_rulearea_prevbutton_event_handler(NFOBJECT *obj, GdkEvent *evt, gpointer data)
{
    if (evt->type == GDK_BUTTON_RELEASE)
    {
        ...
        _delete_dvaa_area(component_data);
...
gint vw_dvabx_wizard_popup_open(NFOBJECT *parent, gint ch)
{
    NFOBJECT *win;
    NFOBJECT *fixed;   
    NFOBJECT *obj;
    ...
    memset(&g_pre_rule_prop, 0x00, sizeof(ITX_DVARULE_PROP));
    ...
    memset(g_pre_calb_conf, 0x00, sizeof(ITX_DVACALB_CONF) * 32);
    memset(g_pre_calb_shape, 0x00, sizeof(ITX_DVACALB_SHAPE) * 32);
    memset(&g_pre_calb_result, 0x00, sizeof(ITX_DVACALB_RESULT));

    dvaa_itx_detector_get_rule_prop(dvaaid, &g_pre_rule_prop);
    ...
    for (i = 0; i < 32; i++)
    {
        dvaa_itx_detector_get_calb_conf(dvaaid, i, &g_pre_calb_conf[i]);
        dvaa_itx_detector_get_calb_shape(dvaaid, i, &g_pre_calb_shape[i]);
    }    

    dvaa_itx_detector_get_calb_result(dvaaid, &g_pre_calb_result);
...
// UNIT SETUP FIXED
    ...
    vw_dvabx_unit_setup_component_open(fixed, opt);
...
// CALIBRATION SETUP FIXED
    ...
    vw_dvabx_calibration_setup_component_open(fixed, opt);
...
// ADD MAIN FIXED	
    ...
    component_data = (DVA_COMPONENT_DATA_T*)nfui_nfobject_get_data(win, DVA_COMPONENT_DATA);
    ...
    vw_dvabx_video_component_sync_preview(g_video_fixed);    
    vw_dvabx_video_component_sync_data(g_video_fixed);
    vw_dvabx_unit_setup_component_sync_data(g_unitsetup_fixed);
    vw_dvabx_calibration_setup_component_sync_data(g_calisetup_fixed);
    vw_dvabx_calibration_adrz_component_sync_data(g_caliadrz_fixed);
    vw_dvabx_calibration_result_component_sync_data(g_caliresult_fixed);
    vw_dvabx_rule_setup_component_sync_data(g_rulesetup_fixed);
    vw_dvabx_rule_line_component_sync_data(g_ruleline_fixed);
