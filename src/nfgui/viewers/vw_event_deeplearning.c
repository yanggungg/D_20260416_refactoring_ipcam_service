...
#include "vw_event_deeplearning.h"
...
static EA_DvaData g_dvad[GUI_CHANNEL_CNT];
static EA_DvaData g_org_dvad[GUI_CHANNEL_CNT];
...
static void init_deeplearning_data()
{
	guint i;

	memset(g_dvad, 0x00, sizeof(EA_DvaData) * GUI_CHANNEL_CNT);
	memset(g_org_dvad, 0x00, sizeof(EA_DvaData) * GUI_CHANNEL_CNT);
	for(i=0; i<GUI_CHANNEL_CNT; i++)
		DAL_get_dva_event_data(&g_dvad[i], i);

	g_memmove(g_org_dvad, g_dvad, sizeof(EA_DvaData)*GUI_CHANNEL_CNT);
}
...
static void set_deeplearning_data()
{
	DAL_set_dva_event_data_all(g_dvad, GUI_CHANNEL_CNT);
...
static gboolean post_cancelbutton_event_handler(NFOBJECT *obj, GdkEvent *evt, gpointer data)
{
	if(evt->type == GDK_BUTTON_RELEASE)
	{
		...
		g_memmove(g_dvad, g_org_dvad, sizeof(EA_DvaData)*GUI_CHANNEL_CNT);
...
static gboolean post_applybutton_event_handler(NFOBJECT *obj, GdkEvent *evt, gpointer data)
{
	if(evt->type == GDK_BUTTON_RELEASE)
	{
		...
		if(memcmp(g_org_dvad, g_dvad, sizeof(EA_DvaData) * GUI_CHANNEL_CNT)) {
			set_deeplearning_data();
