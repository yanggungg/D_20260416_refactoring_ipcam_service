...
#define	CHANNEL_COMBO_WIDTH		((CHANNEL_VIEW_WIDTH-6)/4-6)
	...
	switch(g_curType)
	{
...
#endif
		{
			...
			for(i=0; i<max; i++)
			{
				...
				if(!temp)
				{
#if defined(_SUPPORT_HD_SPOT_ONLY_MULTI)
#ifdef GUI_8CH_SUPPORT
					obj1 = div_btn[2];
...
post_div_btn1_event_handler(NFOBJECT *obj, GdkEvent *evt, gpointer data)
{
	if(evt->type == NFEVENT_RADIO_GET_FOCUS)
	{
		...
		nfui_spin_button_set_index_no_expose((NFSPINBUTTON*)sel_ch[0], 0);
...
static gboolean post_type_fixed_event_handler(NFOBJECT *obj, GdkEvent *evt, gpointer data)
{
	...
	if(evt->type == GDK_EXPOSE) 
	{
		...
		nfutil_draw_image(drawable, gc, MK_IMG_GROUP_BG_212_352, x, y, -1, -1, NFALIGN_LEFT, 0);
		nfui_nfobject_gc_unref(gc);
	}
...
static gboolean post_config_fixed_event_handler(NFOBJECT *obj, GdkEvent *evt, gpointer data)
{
	...
	if(evt->type == GDK_EXPOSE) 
	{
		...
		gc = nfui_nfobject_get_gc(obj);
		nfui_nfobject_get_offset(obj, &x, &y);			
		nfutil_draw_image(drawable, gc, MK_IMG_GROUP_BG_308_352, x, y, -1, -1, NFALIGN_LEFT, 0);
...
guint HD_SpotSetupDlg_Open(NFWINDOW *parent, SpotElementData *elem_data, gboolean add)
{
	...
	const gchar *strButton[] = {"CONFIRM", "CANCEL"};
  	...
  	gchar *strChannel[GUI_CHANNEL_CNT];
...
#if defined(_SUPPORT_HD_SPOT_ONLY_MULTI)
        if (i == SCR_DIV_TYPE1) nfui_nfobject_hide(div_btn[i]);
        if (i == SCR_DIV_TYPE4) nfui_nfobject_hide(div_btn[i]);
#ifndef GUI_4CH_SUPPORT       
        if (i == SCR_DIV_TYPE8) nfui_nfobject_hide(div_btn[i]);
        if (i == SCR_DIV_TYPE6) nfui_nfobject_hide(div_btn[i]);        
#endif        
#endif
	}
	...
	for(i=0; i<GUI_CHANNEL_CNT; i++)
	{
		sel_ch[i] = nfui_spinbutton_new(strChannel, GUI_CHANNEL_CNT, i);
...
// < > BUTTONS
	...
	nfui_nfobject_set_data(div_btn[1], "down", ss_btns[0]);
    	...
    	nfui_nfobject_set_data(div_btn[2], "down", ss_btns[0]);    
    #elif defined(_NOT_SUPPORT_SPC_DIV)
    	nfui_nfobject_set_data(div_btn[0], "right", div_btn[1]);
	...
	nfui_page_open(PGID_HD_SPOT_SETUP, main_wnd, nfui_get_last_user());

	gtk_main();

	nfui_page_close(PGID_HD_SPOT_SETUP, main_wnd);
