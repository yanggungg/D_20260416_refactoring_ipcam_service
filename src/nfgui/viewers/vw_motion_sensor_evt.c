...
#define ROW_CNT_PER_PAGE        (CAM_ALARM_IN / PAGE_FIXED_CNT)
...
static EA_MotSenData g_omsd[GUI_CHANNEL_CNT];

static NFWINDOW *g_curwnd = 0;
static NFOBJECT *g_intObj[GUI_CHANNEL_CNT];
...
static void init_ms_data();
static void set_ms_data();
static void set_data_to_obj(gint expose);
static void get_data_from_obj();

static guint conv_interval_to_index(gint interval);
static gint conv_index_to_interval(gint index);
static void conv_alarmOut_data_to_string(guint64 data, gchar str[]);
static void conv_index_to_alarmOut_data(gint index, gchar data[]);
...
static void init_ms_data()
{
	...
	memset(g_msd, 0x00, sizeof(EA_MotSenData) * GUI_CHANNEL_CNT);
	memset(g_omsd, 0x00, sizeof(EA_MotSenData) * GUI_CHANNEL_CNT);
	for(i=0; i<GUI_CHANNEL_CNT; i++)
		DAL_get_MotSen_data(&g_msd[i], i);

	g_memmove(g_omsd, g_msd, sizeof(EA_MotSenData)*GUI_CHANNEL_CNT);
}
...
static void set_data_to_obj(gint expose)
{
	...
	for(i=0; i<GUI_CHANNEL_CNT; i++) {
		if (expose) nfui_spin_button_set_index((NFSPINBUTTON*)g_intObj[i], conv_interval_to_index(g_msd[i].interval));
		else nfui_spin_button_set_index_no_expose((NFSPINBUTTON*)g_intObj[i], conv_interval_to_index(g_msd[i].interval));

		memset(str, 0x00, ALARM_STRING_LENGTH);
		conv_alarmOut_data_to_string(g_msd[i].almOut, str);
		...
		{
#if defined(_SUPPORT_EVENT_PRESET)
			nfui_check_button_set_active((NFCHECKBUTTON*)g_presetObj[i], g_msd[i].preset);

            if (g_msd[i].preset)    nfui_nfobject_enable(g_presetEditObj[i]);
            else                    nfui_nfobject_disable(g_presetEditObj[i]);

            nfui_signal_emit(g_presetEditObj[i], GDK_EXPOSE, FALSE);
#endif
			nfui_check_button_set_active((NFCHECKBUTTON*)g_buzzObj[i], g_msd[i].buzzer);
			nfui_check_button_set_active((NFCHECKBUTTON*)g_vpopObj[i], g_msd[i].vpop);
			nfui_check_button_set_active((NFCHECKBUTTON*)g_mailObj[i], g_msd[i].email);
			nfui_check_button_set_active((NFCHECKBUTTON*)g_ftpObj[i], g_msd[i].ftp);
#if defined(_SUPPORT_USER_PHONE)
			nfui_check_button_set_active((NFCHECKBUTTON*)g_mobileObj[i], g_msd[i].mobile);
#endif
            if(support_mobilepush)
			    nfui_check_button_set_active((NFCHECKBUTTON*)g_mobilepushObj[i], g_msd[i].mobilepush);
		}
		else
		{
#if defined(_SUPPORT_EVENT_PRESET)
			nfui_check_button_set_active_no_expose((NFCHECKBUTTON*)g_presetObj[i], g_msd[i].preset);

            if (g_msd[i].preset)    nfui_nfobject_enable(g_presetEditObj[i]);
            else                    nfui_nfobject_disable(g_presetEditObj[i]);
#endif
			nfui_check_button_set_active_no_expose((NFCHECKBUTTON*)g_buzzObj[i], g_msd[i].buzzer);
			nfui_check_button_set_active_no_expose((NFCHECKBUTTON*)g_vpopObj[i], g_msd[i].vpop);
			nfui_check_button_set_active_no_expose((NFCHECKBUTTON*)g_mailObj[i], g_msd[i].email);
			nfui_check_button_set_active_no_expose((NFCHECKBUTTON*)g_ftpObj[i], g_msd[i].ftp);
#if defined(_SUPPORT_USER_PHONE)
			nfui_check_button_set_active_no_expose((NFCHECKBUTTON*)g_mobileObj[i], g_msd[i].mobile);
#endif
            if(support_mobilepush)
    			nfui_check_button_set_active_no_expose((NFCHECKBUTTON*)g_mobilepushObj[i], g_msd[i].mobilepush);
		}
	}
...
static void get_data_from_obj()
{
	...
	for(i=0; i<GUI_CHANNEL_CNT; i++) {
		index = (gint)nfui_spin_button_get_index((NFSPINBUTTON*)g_intObj[i]);
		g_msd[i].interval = conv_index_to_interval(index);

		g_msd[i].buzzer   = nfui_check_button_get_active((NFCHECKBUTTON*)g_buzzObj[i]);
		g_msd[i].vpop     = nfui_check_button_get_active((NFCHECKBUTTON*)g_vpopObj[i]);
		g_msd[i].email    = nfui_check_button_get_active((NFCHECKBUTTON*)g_mailObj[i]);
		g_msd[i].ftp      = nfui_check_button_get_active((NFCHECKBUTTON*)g_ftpObj[i]);
#if defined(_SUPPORT_USER_PHONE)
		g_msd[i].mobile = nfui_check_button_get_active((NFCHECKBUTTON*)g_mobileObj[i]);
#endif
        if(support_mobilepush)
    		g_msd[i].mobilepush = nfui_check_button_get_active((NFCHECKBUTTON*)g_mobilepushObj[i]);
	}
}
...
static gboolean post_ignore_all_event_handler(NFOBJECT *obj, GdkEvent *evt, gpointer data)
{
	if(evt->type == NFEVENT_COMBOBOX_CHANGED) {
		...
		for(i=0; i<GUI_CHANNEL_CNT; i++)
			nfui_spin_button_set_index((NFSPINBUTTON*)g_intObj[i], idx);

		for(i=0; i<GUI_CHANNEL_CNT; i++)
			nfui_signal_emit(g_intObj[i], GDK_EXPOSE, TRUE);
	}
...
static gboolean post_ao_all_btn_event_handler(NFOBJECT *obj, GdkEvent *evt, gpointer data)
{
	if(evt->type == GDK_BUTTON_PRESS) {
		...
		if(!VW_ChannelMask_Ctrl(g_curwnd, "ALARM OUT CHANNEL", (off_x - 153), off_y + obj->height, &mask))
			return FALSE;

		for(i=0; i<GUI_CHANNEL_CNT; i++) {
			if(mask != g_msd[i].almOut) {
				g_msd[i].almOut = mask;
...
static gboolean post_preset_edit_btn_event_handler(NFOBJECT *obj, GdkEvent *evt, gpointer data)
{
	...
	if(evt->type == GDK_BUTTON_RELEASE)
	{
        ...
        for (j = 0; j < GUI_CHANNEL_CNT; j++)
            info->number[j] = g_msd[i].preset_num[j];

        vw_evt_ptz_preset_popup_open(g_curwnd, info, 0xffffffff);

        for (j = 0; j < GUI_CHANNEL_CNT; j++)
            g_msd[i].preset_num[j] = info->number[j];
...
static gboolean post_ao_label_event_handler(NFOBJECT *obj, GdkEvent *evt, gpointer data)
{
	...
	if(evt->type == GDK_2BUTTON_PRESS || kpid == KEYPAD_ENTER) {
		...
		mask = (ALARM_OUT_MASK_TYPE | g_msd[ch].almOut);
		if(!VW_ChannelMask_Ctrl(g_curwnd, "ALARM OUT CHANNEL", off_x, off_y + obj->height, &mask))
			return FALSE;

		if(mask != g_msd[ch].almOut) {
			g_msd[ch].almOut = mask;

			memset(str, 0x00, ALARM_STRING_LENGTH);
			conv_alarmOut_data_to_string(g_msd[ch].almOut, str);
...
static gboolean post_cancelbutton_event_handler(NFOBJECT *obj, GdkEvent *evt, gpointer data)
{
	if(evt->type == GDK_BUTTON_RELEASE)
	{
		...
		g_memmove(g_msd, g_omsd, sizeof(EA_MotSenData)*GUI_CHANNEL_CNT);
...
static gboolean post_applybutton_event_handler(NFOBJECT *obj, GdkEvent *evt, gpointer data)
{
	if(evt->type == GDK_BUTTON_RELEASE)
	{
		...
		if(memcmp(g_omsd, g_msd, sizeof(EA_MotSenData) * GUI_CHANNEL_CNT)) {
			set_ms_data();
...
void VW_Init_MotionSensorEvt_Page(NFOBJECT *parent)
{
	...
	gchar *strParam[] = {"CAM", "IGNORING INTERVAL"};
	gchar *strInv[] = {"5 SEC", "10 SEC", "15 SEC", "20 SEC", "25 SEC", "30 SEC"};
...
*/
	...
	// cam_n
	obj = (NFOBJECT*)nfui_nflabel_new_with_pango_font(strParam[0], nffont_get_pango_font(NFFONT_MEDIUM_SEMI), COLOR_IDX(116));
	nfui_nflabel_set_align((NFLABEL*)obj, NFALIGN_CENTER, 0);
	...
	obj = nfui_combobox_new(strInv, 6, 0);
	nfui_combobox_set_display_string(NF_COMBOBOX(obj), strParam[1]);
	...
	for(i=0; i<GUI_CHANNEL_CNT; i++) {
		obj = nfui_spinbutton_new(strInv, 6, conv_interval_to_index(g_msd[i].interval));
	...
	for(i=0; i<GUI_CHANNEL_CNT; i++) {
		memset(buf, 0x00, sizeof(buf));
		conv_alarmOut_data_to_string(g_msd[i].almOut, buf);
    ...
    for (j = 0; j < GUI_CHANNEL_CNT; j++)
    {
    	...
    	obj = (NFOBJECT*)nfui_checkbutton_new(g_msd[j].preset);
    	...
    	if (!g_msd[j].preset)   nfui_nfobject_disable(obj);
	...
	for(i = ACTION_BUZZER; i < ACTION_MAX; i++)
	{
		...
		for(j = 0; j < GUI_CHANNEL_CNT; j++)
		{
			...
			if(i == ACTION_BUZZER) 		    obj = (NFOBJECT*)nfui_checkbutton_new(g_msd[j].buzzer);
			else if(i == ACTION_VPOPUP)     obj = (NFOBJECT*)nfui_checkbutton_new(g_msd[j].vpop);
			else if(i == ACTION_EMAIL)      obj = (NFOBJECT*)nfui_checkbutton_new(g_msd[j].email);
			else if(i == ACTION_FTP)        obj = (NFOBJECT*)nfui_checkbutton_new(g_msd[j].ftp);
#if defined(_SUPPORT_USER_PHONE)
			else if(i == ACTION_MOBILE)	    obj = (NFOBJECT*)nfui_checkbutton_new(g_msd[j].mobile);
#endif
			else if(i == ACTION_MOBILEPUSH)	 	obj = (NFOBJECT*)nfui_checkbutton_new(g_msd[j].mobilepush);
...
gboolean VW_MotionSensorEvt_tab_out_handler(NFOBJECT *obj)
{
	mb_type ret;
