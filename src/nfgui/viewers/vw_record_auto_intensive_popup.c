...
static NFOBJECT *normal_size_obj[GUI_CHANNEL_CNT];
static NFOBJECT *normal_fps_obj[GUI_CHANNEL_CNT];
static NFOBJECT *normal_quality_obj[GUI_CHANNEL_CNT];
static NFOBJECT *event_size_obj[GUI_CHANNEL_CNT];
static NFOBJECT *event_fps_obj[GUI_CHANNEL_CNT];
static NFOBJECT *event_quality_obj[GUI_CHANNEL_CNT];
...
static gchar* (*paramInfo)[PARAM_AUTO_ITS_MAX] = NULL;
...
static void _limit_capable_normal_fps_info(gint ch)
{
    gint i;
    gchar *data;
    gint data_len;

    nfui_combobox_remove_all(NF_COMBOBOX(normal_fps_obj[ch]));
    ...
    for (i = 0; i < data_len; i++)
    {
        if ((strcmp(fps_info[CAP_FPS_30], translate_info_fps_capable(ch, data[i])) != 0)
            ...
            nfui_combobox_append_data(normal_fps_obj[ch], translate_info_fps_capable(ch, data[i]));
    }

    nfui_combobox_set_data(NF_COMBOBOX(normal_fps_obj[ch]), _get_param_info(ch, PARAM_AUTO_ITS_NORMAL_FPS));    
}

static void _recover_capable_normal_fps_info(gint ch)
{
    gint i;
    gchar *data;
    gint data_len;
...
static void _limit_capable_event_fps_info(gint ch)
{
    gint i;
    gchar *data;
    gint data_len;

    nfui_combobox_remove_all(NF_COMBOBOX(event_fps_obj[ch]));
    ...
    for (i = 0; i < data_len; i++)
    {
        if ((strcmp(fps_info[CAP_FPS_30], translate_info_fps_capable(ch, data[i])) != 0)
            ...
            && (strcmp(fps_info[CAP_FPS_25], translate_info_fps_capable(ch, data[i])) != 0)         
            && (strcmp(fps_info[CAP_FPS_12], translate_info_fps_capable(ch, data[i])) != 0))
            nfui_combobox_append_data(event_fps_obj[ch], translate_info_fps_capable(ch, data[i]));
    }

    nfui_combobox_set_data(NF_COMBOBOX(event_fps_obj[ch]), _get_param_info(ch, PARAM_AUTO_ITS_EVENT_FPS));  
}

static void _recover_capable_event_fps_info(gint ch)
{
    gint i;
    gchar *data;
    gint data_len;
...
static gboolean _normal_size_all_event_handler(NFOBJECT *obj, GdkEvent *evt, gpointer data)
{
    if(evt->type == GDK_BUTTON_PRESS) {
        ...
        {   
            for (ch = 0; ch < GUI_CHANNEL_CNT; ch++)
            {
                info = nfui_combobox_get_value(NF_COMBOBOX(normal_size_obj[ch]));
                ...
                if (strcmp(info, size_data[ch]) != 0)
                {
                    ...
                    _set_param_info(ch, PARAM_AUTO_ITS_NORMAL_SIZE, size_info[i]);
                    nfui_combobox_set_data(NF_COMBOBOX(normal_size_obj[ch]), size_info[i]);                 
                    ...
                    if ((IS_PTZ(ch)) && (strcmp(size_data[ch], size_info[CAP_RES_640x352]) == 0))
                    {
                        ...
                        cur_fps = nfui_combobox_get_value(NF_COMBOBOX(normal_fps_obj[ch]));
                        
                        if (strcmp(cur_fps, fps_info[CAP_FPS_30]) == 0)
                            _set_param_info(ch, PARAM_AUTO_ITS_NORMAL_FPS, fps_info[CAP_FPS_07]);
                        if (strcmp(cur_fps, fps_info[CAP_FPS_15]) == 0)
                            _set_param_info(ch, PARAM_AUTO_ITS_NORMAL_FPS, fps_info[CAP_FPS_07]);                       
                        else if(strcmp(cur_fps, fps_info[CAP_FPS_25]) == 0)
                            _set_param_info(ch, PARAM_AUTO_ITS_NORMAL_FPS, fps_info[CAP_FPS_06]);
                        else if(strcmp(cur_fps, fps_info[CAP_FPS_12]) == 0)
                            _set_param_info(ch, PARAM_AUTO_ITS_NORMAL_FPS, fps_info[CAP_FPS_06]);
                            
                        _limit_capable_normal_fps_info(ch);
                    }
                    else
                    {
                        _recover_capable_normal_fps_info(ch);
                    }
#endif                      
                }
            }               
...
static gboolean _normal_size_event_handler(NFOBJECT *obj, GdkEvent *evt, gpointer data)
{
    guint ch, i;
    gchar *info;
...
static gboolean _normal_fps_all_event_handler(NFOBJECT *obj, GdkEvent *evt, gpointer data)
{
    if(evt->type == GDK_BUTTON_PRESS) {
        if(evt->button.button == MOUSE_RIGTH_BUTTON) 
            return FALSE;
        ...
        for (ch = 0; ch < GUI_CHANNEL_CNT; ch++)
            size_data[ch] = _get_param_info(ch, PARAM_AUTO_ITS_NORMAL_SIZE);
        ...
        {   
            for (ch = 0; ch < GUI_CHANNEL_CNT; ch++)
            {
                info = nfui_combobox_get_value(NF_COMBOBOX(normal_fps_obj[ch]));
                ...
                if (strcmp(info, fps_data[ch]) != 0)
                {
                    ...
                    _set_param_info(ch, PARAM_AUTO_ITS_NORMAL_FPS, fps_info[i]);
                    nfui_combobox_set_data(NF_COMBOBOX(normal_fps_obj[ch]), fps_info[i]);                   
                }
            }               
        }
...
static gboolean _normal_fps_event_handler(NFOBJECT *obj, GdkEvent *evt, gpointer data)
{
    guint ch, i;
    gchar *info;
...
static gboolean _normal_quality_all_event_handler(NFOBJECT *obj, GdkEvent *evt, gpointer data)
{
    guint i;
    gint index = -1;
    ...
    if(evt->type == NFEVENT_COMBOBOX_CHANGED) {
        ...
        {
            _set_param_info(i, PARAM_AUTO_ITS_NORMAL_QUAL, quality_info[index]);
            nfui_combobox_set_data(normal_quality_obj[i], quality_info[index]);         
        }
    }
...
static gboolean _normal_quality_event_handler(NFOBJECT *obj, GdkEvent *evt, gpointer data)
{
    guint i;
    gint index = -1;
...
static gboolean _event_size_all_event_handler(NFOBJECT *obj, GdkEvent *evt, gpointer data)
{
    if(evt->type == GDK_BUTTON_PRESS) {
        if(evt->button.button == MOUSE_RIGTH_BUTTON) 
            return FALSE;
        ...
        {   
            for (ch = 0; ch < GUI_CHANNEL_CNT; ch++)
            {
                info = nfui_combobox_get_value(NF_COMBOBOX(event_size_obj[ch]));
                ...
                if (strcmp(info, size_data[ch]) != 0)
                {
                    ...
                    _set_param_info(ch, PARAM_AUTO_ITS_EVENT_SIZE, size_info[i]);
                    nfui_combobox_set_data(NF_COMBOBOX(event_size_obj[ch]), size_info[i]);                  
                    ...
                    if ((IS_PTZ(ch)) && (strcmp(size_data[ch], size_info[CAP_RES_640x352]) == 0))
                    {
                        ...
                        cur_fps = nfui_combobox_get_value(NF_COMBOBOX(event_fps_obj[ch]));
                        
                        if (strcmp(cur_fps, fps_info[CAP_FPS_30]) == 0)
                            _set_param_info(ch, PARAM_AUTO_ITS_EVENT_FPS, fps_info[CAP_FPS_07]);
                        else if(strcmp(cur_fps, fps_info[CAP_FPS_15]) == 0)
                            _set_param_info(ch, PARAM_AUTO_ITS_EVENT_FPS, fps_info[CAP_FPS_07]);
                        else if(strcmp(cur_fps, fps_info[CAP_FPS_25]) == 0)
                            _set_param_info(ch, PARAM_AUTO_ITS_EVENT_FPS, fps_info[CAP_FPS_06]);                            
                        else if(strcmp(cur_fps, fps_info[CAP_FPS_12]) == 0)
                            _set_param_info(ch, PARAM_AUTO_ITS_EVENT_FPS, fps_info[CAP_FPS_06]);
                            
                        _limit_capable_event_fps_info(ch);
                    }
                    else
                    {
                        _recover_capable_event_fps_info(ch);
                    }
#endif                                                  
                }
            }               
...
static gboolean _event_size_event_handler(NFOBJECT *obj, GdkEvent *evt, gpointer data)
{
    guint ch, i;
    gchar *info;
...
static gboolean _event_fps_all_event_handler(NFOBJECT *obj, GdkEvent *evt, gpointer data)
{
    if(evt->type == GDK_BUTTON_PRESS) {
        if(evt->button.button == MOUSE_RIGTH_BUTTON) 
            return FALSE;
        ...
        for (ch = 0; ch < GUI_CHANNEL_CNT; ch++)
            size_data[ch] = _get_param_info(ch, PARAM_AUTO_ITS_EVENT_SIZE);
        ...
        {           
            for (ch = 0; ch < GUI_CHANNEL_CNT; ch++)
            {
                info = nfui_combobox_get_value(NF_COMBOBOX(event_fps_obj[ch]));
                ...
                if (strcmp(info, fps_data[ch]) != 0)
                {
                    ...
                    _set_param_info(ch, PARAM_AUTO_ITS_EVENT_FPS, fps_info[i]);
                    nfui_combobox_set_data(NF_COMBOBOX(event_fps_obj[ch]), fps_info[i]);                    
                }
            }               
        }
...
static gboolean _event_fps_event_handler(NFOBJECT *obj, GdkEvent *evt, gpointer data)
{
    guint ch, i;
    gchar *info;
...
static gboolean _event_quality_all_event_handler(NFOBJECT *obj, GdkEvent *evt, gpointer data)
{
    guint i;
    gint index = -1;
    ...
    if(evt->type == NFEVENT_COMBOBOX_CHANGED) {
        ...
        for (i = 0; i < GUI_CHANNEL_CNT; i++)
        {
            _set_param_info(i, PARAM_AUTO_ITS_EVENT_QUAL, quality_info[index]);
            nfui_combobox_set_data(event_quality_obj[i], quality_info[index]);          
        }
    }
...
static gboolean _event_quality_event_handler(NFOBJECT *obj, GdkEvent *evt, gpointer data)
{
    guint i;
    gint index = -1;
...
static gboolean _audio_all_event_handler(NFOBJECT *obj, GdkEvent *evt, gpointer data)
{
    ...
    if(evt->type == NFEVENT_COMBOBOX_CHANGED) {
        ...
        for (i = 0; i < GUI_CHANNEL_CNT; i++)
        {
            _set_param_info(i, PARAM_AUTO_ITS_AUDIO, audio_info[index]);
...
{
    NFOBJECT *top;
    gchar* (*info)[PARAM_AUTO_ITS_MAX] = NULL;
    gchar *pTime;
    gchar buf[20];  
    ...
    if(evt->type == GDK_BUTTON_RELEASE)
    {
        ...
        if(memcmp(info, paramInfo, sizeof(gchar*) * (GUI_CHANNEL_CNT*PARAM_AUTO_ITS_MAX))) {
            memcpy(info, paramInfo, sizeof(gchar*) * (GUI_CHANNEL_CNT*PARAM_AUTO_ITS_MAX));
        }    
...
static gboolean _auto_its_popup_fixed_event_handler(NFOBJECT *obj, GdkEvent *evt, gpointer data)
{
...
static gboolean _auto_its_popup_win_event_handler(NFOBJECT *obj, GdkEvent *evt, gpointer data)
{
...
static void _set_capable_normal_fps_info(NFOBJECT *obj, gint ch)
{
    gint i, data_len;
    gchar *data;
    ...
    for (i = 0; i < data_len; i++)
    {
        if ((IS_PTZ(ch)) && (strcmp(size_info[CAP_RES_640x352], _get_param_info(ch, PARAM_AUTO_ITS_NORMAL_SIZE)) == 0))
        {
            if ((strcmp(fps_info[CAP_FPS_30], translate_info_fps_capable(ch, data[i])) != 0)
                && (strcmp(fps_info[CAP_FPS_15], translate_info_fps_capable(ch, data[i])) != 0)
                && (strcmp(fps_info[CAP_FPS_25], translate_info_fps_capable(ch, data[i])) != 0)
...
static void _set_capable_event_fps_info(NFOBJECT *obj, gint ch)
{
    gint i, data_len;
    gchar *data;
    ...
    for (i = 0; i < data_len; i++)
    {
        if ((IS_PTZ(ch)) && (strcmp(size_info[CAP_RES_640x352], _get_param_info(ch, PARAM_AUTO_ITS_EVENT_SIZE)) == 0))
        {
            if ((strcmp(fps_info[CAP_FPS_30], translate_info_fps_capable(ch, data[i])) != 0)
                && (strcmp(fps_info[CAP_FPS_15], translate_info_fps_capable(ch, data[i])) != 0)
                && (strcmp(fps_info[CAP_FPS_25], translate_info_fps_capable(ch, data[i])) != 0)
...
gboolean VW_RecAutoItsPopup_Open(NFWINDOW *parent, gchar* (*info)[], OPEN_AUTO_ITS mode)
{
    ...
    gchar *strTitle[] = {"INTENSIVE MOTION RECORD", 
                        "INTENSIVE ALARM RECORD", 
...
// SUBMENU DATA INIT
    paramInfo = imalloc(sizeof(gchar*) * (GUI_CHANNEL_CNT*PARAM_AUTO_ITS_MAX)); 
    ...
    memcpy(paramInfo, info, sizeof(gchar*) * (GUI_CHANNEL_CNT*PARAM_AUTO_ITS_MAX));
...
// LOAD
    ...
    win_width += (param_tbl_width+table_gap)*2;
...
// WIN
    ...
    nfui_regi_pre_event_callback(rop_win, _auto_its_popup_win_event_handler);   


// FIXED
    fixed = (NFOBJECT*)nfui_nffixed_new();
    nfui_nfobject_set_size(fixed, win_width, win_height);
    nfui_regi_pre_event_callback(fixed, _auto_its_popup_fixed_event_handler);   
...
// GENERAL TABLE
    tbl_pos_x = (ROP_CAM_TITLE_POS_X + ROP_LABEL_TITLE_COL_W + 2);
    tbl_pos_y = (ROP_CAM_TITLE_POS_Y - ROP_LABEL_H - 1);    

    nfui_get_pixbuf_size(dropdown1_img[0], &size_w, &size_h);

    dp_obj[0] = (NFOBJECT*)nfui_nflabel_new_with_pango_font("GENERAL", nffont_get_pango_font(NFFONT_MEDIUM_SEMI), COLOR_IDX(116));
    nfui_nflabel_set_align((NFLABEL*)dp_obj[0], NFALIGN_CENTER, 20);
    nfui_nfobject_modify_bg(dp_obj[0], NFOBJECT_STATE_NORMAL, COLOR_IDX(115));
    nfui_nfobject_set_size(dp_obj[0], param_tbl_w[0]+2+param_tbl_w[1]+2+param_tbl_w[2], 40);
    nfui_nfobject_use_focus(dp_obj[0], NFOBJECT_FOCUS_OFF);
    nfui_nfobject_show(dp_obj[0]);
    nfui_nffixed_put((NFFIXED*)fixed, dp_obj[0], tbl_pos_x, tbl_pos_y-41);

    normal_tbl = (NFOBJECT*)nfui_nftable_new(3, 1, 2, 1, param_tbl_w, 40);  
    nfui_nfobject_show(normal_tbl);
    nfui_nffixed_put((NFFIXED*)fixed, normal_tbl, tbl_pos_x, tbl_pos_y);
    ...
    nfui_nftable_attach((NFTABLE*)normal_tbl, tbl_tmp, 0, 0);
    ...
    nfui_nfobject_set_size(obj, (guint)size_w, (guint)size_h);
    nfui_nfobject_show(obj);
    nfui_regi_post_event_callback(obj, _normal_size_all_event_handler); 
    nfui_nftable_attach((NFTABLE*)tbl_tmp, obj, 1, 0);

    pos_x = ROP_LABEL_TITLE_COL_W + 2;
    
    page_num = row_num = 0;

    for (i = 0; i < GUI_CHANNEL_CNT; i++)
    {
        normal_size_obj[i] = nfui_combobox_new(NULL, 0, 0);
        nfui_combobox_set_skin_type(NF_COMBOBOX(normal_size_obj[i]), NFCOMBOBOX_TYPE_POPUP_1);      
        nfui_combobox_set_align(NF_COMBOBOX(normal_size_obj[i]), NFALIGN_CENTER, 0);
        nfui_nfobject_set_size(normal_size_obj[i], ROP_TABLE_SIZE_COL_W, ROP_LABEL_H);
        nfui_regi_post_event_callback(normal_size_obj[i], _normal_size_event_handler);  
        nfui_nfobject_show(normal_size_obj[i]);
    	nfui_nffixed_put((NFFIXED*)g_page_fixed[page_num], normal_size_obj[i], pos_x, (row_num * 41));

        _set_capable_size_info(normal_size_obj[i], i);
        if(ivsc.vendor_code == 101) {
            //max resolution
            nfui_combobox_set_data_no_expose(NF_COMBOBOX(normal_size_obj[i]), _get_param_info(i, PARAM_AUTO_ITS_EVENT_SIZE));
        }
        else
            nfui_combobox_set_data_no_expose(NF_COMBOBOX(normal_size_obj[i]), _get_param_info(i, PARAM_AUTO_ITS_NORMAL_SIZE));
    ...
    tbl_tmp = (NFOBJECT*)nfui_nftable_new(2, 1, 0, 0, tmp_table_w, ROP_LABEL_H);    
    nfui_nfobject_show(tbl_tmp);
    nfui_nftable_attach((NFTABLE*)normal_tbl, tbl_tmp, 1, 0);
    ...
    nfui_nfobject_set_size(obj, (guint)size_w, (guint)size_h);
    nfui_nfobject_show(obj);
    nfui_regi_post_event_callback(obj, _normal_fps_all_event_handler);  
    nfui_nftable_attach((NFTABLE*)tbl_tmp, obj, 1, 0);

    pos_y = 0;
    pos_x += ROP_TABLE_SIZE_COL_W + 2;

    page_num = row_num = 0;

    for (i = 0; i < GUI_CHANNEL_CNT; i++)
    {
        normal_fps_obj[i] = nfui_combobox_new(NULL, 0, 0);
        nfui_combobox_set_skin_type(NF_COMBOBOX(normal_fps_obj[i]), NFCOMBOBOX_TYPE_POPUP_1);       
        nfui_combobox_set_align(NF_COMBOBOX(normal_fps_obj[i]), NFALIGN_CENTER, 0);             
        nfui_nfobject_set_size(normal_fps_obj[i], ROP_TABLE_FPS_COL_W, ROP_LABEL_H);
        nfui_regi_post_event_callback(normal_fps_obj[i], _normal_fps_event_handler);    
        nfui_nfobject_show(normal_fps_obj[i]);
    	nfui_nffixed_put((NFFIXED*)g_page_fixed[page_num], normal_fps_obj[i], pos_x, (row_num * 41));

#if defined(_PTZ_2STREAM_FPS_LIMIT)
        _set_capable_normal_fps_info(normal_fps_obj[i], i);
#else           
        _set_capable_fps_info(normal_fps_obj[i], i);
#endif      
        nfui_combobox_set_data_no_expose(NF_COMBOBOX(normal_fps_obj[i]), _get_param_info(i, PARAM_AUTO_ITS_NORMAL_FPS));
    ...
    nfui_regi_post_event_callback(obj, _normal_quality_all_event_handler);
    nfui_nfobject_show(obj);
    nfui_nftable_attach((NFTABLE*)normal_tbl, obj, 2, 0);

    pos_y = 0;
    pos_x += ROP_TABLE_FPS_COL_W + 2;

    page_num = row_num = 0;

    for (i = 0; i < GUI_CHANNEL_CNT; i++)
    {
        normal_quality_obj[i] = nfui_combobox_new(quality_info, 5, 0);
        nfui_combobox_set_skin_type(NF_COMBOBOX(normal_quality_obj[i]), NFCOMBOBOX_TYPE_POPUP_1);
        nfui_combobox_set_align(NF_COMBOBOX(normal_quality_obj[i]), NFALIGN_CENTER, 0);             
        nfui_combobox_set_data_no_expose(NF_COMBOBOX(normal_quality_obj[i]), _get_param_info(i, PARAM_AUTO_ITS_NORMAL_QUAL));
        nfui_nfobject_set_size(normal_quality_obj[i], ROP_TABLE_QUALITY_COL_W, ROP_LABEL_H);
        nfui_regi_post_event_callback(normal_quality_obj[i], _normal_quality_event_handler);    
        nfui_nfobject_show(normal_quality_obj[i]);
    	nfui_nffixed_put((NFFIXED*)g_page_fixed[page_num], normal_quality_obj[i], pos_x, (row_num * 41));
    ...
    tbl_pos_x += (param_tbl_width + table_gap);
    
    
// EVENT TABLE
    if (mode == OPEN_AUTO_ITS_MOT) 
        dp_obj[1] = (NFOBJECT*)nfui_nflabel_new_with_pango_font("WHEN MOTION", nffont_get_pango_font(NFFONT_MEDIUM_SEMI), COLOR_IDX(116));
    else if (mode == OPEN_AUTO_ITS_ALARM)
        dp_obj[1] = (NFOBJECT*)nfui_nflabel_new_with_pango_font("WHEN ALARM", nffont_get_pango_font(NFFONT_MEDIUM_SEMI), COLOR_IDX(116));
    else
        dp_obj[1] = (NFOBJECT*)nfui_nflabel_new_with_pango_font("WHEN MOTION OR ALARM", nffont_get_pango_font(NFFONT_MEDIUM_SEMI), COLOR_IDX(116));     
    nfui_nflabel_set_align((NFLABEL*)dp_obj[1], NFALIGN_CENTER, 20);
    nfui_nfobject_modify_bg(dp_obj[1], NFOBJECT_STATE_NORMAL, COLOR_IDX(115));
    nfui_nfobject_set_size(dp_obj[1], param_tbl_w[0]+2+param_tbl_w[1]+2+param_tbl_w[2], 40);
    nfui_nfobject_use_focus(dp_obj[1], NFOBJECT_FOCUS_OFF);
    nfui_nfobject_show(dp_obj[1]);
    nfui_nffixed_put((NFFIXED*)fixed, dp_obj[1], tbl_pos_x, tbl_pos_y-41);

    event_tbl = (NFOBJECT*)nfui_nftable_new(3, 1, 2, 1, param_tbl_w, 40);   
    nfui_nfobject_show(event_tbl);
    nfui_nffixed_put((NFFIXED*)fixed, event_tbl, tbl_pos_x, tbl_pos_y);

    tmp_table_w[0] = ROP_TABLE_SIZE_COL_W - size_w;
    tmp_table_w[1] = size_w;    
    tbl_tmp = (NFOBJECT*)nfui_nftable_new(2, 1, 0, 0, tmp_table_w, ROP_LABEL_H);    
    nfui_nfobject_show(tbl_tmp);
    nfui_nftable_attach((NFTABLE*)event_tbl, tbl_tmp, 0, 0);
    ...
    nfui_nfobject_set_size(obj, (guint)size_w, (guint)size_h);
    nfui_nfobject_show(obj);
    nfui_regi_post_event_callback(obj, _event_size_all_event_handler);  
    nfui_nftable_attach((NFTABLE*)tbl_tmp, obj, 1, 0);
    
    pos_x += ROP_TABLE_QUALITY_COL_W + (table_gap);

    page_num = row_num = 0;

    for (i = 0; i < GUI_CHANNEL_CNT; i++)
    {
        event_size_obj[i] = nfui_combobox_new(NULL, 0, 0);
        nfui_combobox_set_skin_type(NF_COMBOBOX(event_size_obj[i]), NFCOMBOBOX_TYPE_POPUP_1);
        nfui_combobox_set_align(NF_COMBOBOX(event_size_obj[i]), NFALIGN_CENTER, 0);
        nfui_nfobject_set_size(event_size_obj[i], ROP_TABLE_SIZE_COL_W, ROP_LABEL_H);
        nfui_regi_post_event_callback(event_size_obj[i], _event_size_event_handler);
        nfui_nfobject_show(event_size_obj[i]);
    	nfui_nffixed_put((NFFIXED*)g_page_fixed[page_num], event_size_obj[i], pos_x, (row_num * 41));

        _set_capable_size_info(event_size_obj[i], i);
        nfui_combobox_set_data_no_expose(NF_COMBOBOX(event_size_obj[i]), _get_param_info(i, PARAM_AUTO_ITS_EVENT_SIZE));
    ...
    tbl_tmp = (NFOBJECT*)nfui_nftable_new(2, 1, 0, 0, tmp_table_w, ROP_LABEL_H);    
    nfui_nfobject_show(tbl_tmp);
    nfui_nftable_attach((NFTABLE*)event_tbl, tbl_tmp, 1, 0);
    ...
    nfui_nfobject_set_size(obj, (guint)size_w, (guint)size_h);
    nfui_nfobject_show(obj);
    nfui_regi_post_event_callback(obj, _event_fps_all_event_handler);   
    nfui_nftable_attach((NFTABLE*)tbl_tmp, obj, 1, 0);
    ...
    for (i = 0; i < GUI_CHANNEL_CNT; i++)
    {
        event_fps_obj[i] = nfui_combobox_new(NULL, 0, 0);
        nfui_combobox_set_skin_type(NF_COMBOBOX(event_fps_obj[i]), NFCOMBOBOX_TYPE_POPUP_1);
        nfui_combobox_set_align(NF_COMBOBOX(event_fps_obj[i]), NFALIGN_CENTER, 0);              
        nfui_nfobject_set_size(event_fps_obj[i], ROP_TABLE_FPS_COL_W, ROP_LABEL_H);
        nfui_regi_post_event_callback(event_fps_obj[i], _event_fps_event_handler);
        nfui_nfobject_show(event_fps_obj[i]);
    	nfui_nffixed_put((NFFIXED*)g_page_fixed[page_num], event_fps_obj[i], pos_x, (row_num * 41));

#if defined(_PTZ_2STREAM_FPS_LIMIT)
        _set_capable_event_fps_info(event_fps_obj[i], i);
#else       
        _set_capable_fps_info(event_fps_obj[i], i);
#endif      
        nfui_combobox_set_data_no_expose(NF_COMBOBOX(event_fps_obj[i]), _get_param_info(i, PARAM_AUTO_ITS_EVENT_FPS));
    ...
    nfui_combobox_set_skin_type(NF_COMBOBOX(obj), NFCOMBOBOX_TYPE_POPUP_2);
    nfui_combobox_set_align(NF_COMBOBOX(obj), NFALIGN_CENTER, 0);
    nfui_regi_post_event_callback(obj, _event_quality_all_event_handler);
    nfui_nfobject_show(obj);
    nfui_nftable_attach((NFTABLE*)event_tbl, obj, 2, 0);
    ...
    for (i = 0; i < GUI_CHANNEL_CNT; i++)
    {
        event_quality_obj[i] = nfui_combobox_new(quality_info, 5, 0);
        nfui_combobox_set_skin_type(NF_COMBOBOX(event_quality_obj[i]), NFCOMBOBOX_TYPE_POPUP_1);
        nfui_combobox_set_align(NF_COMBOBOX(event_quality_obj[i]), NFALIGN_CENTER, 0);              
        nfui_combobox_set_data_no_expose(NF_COMBOBOX(event_quality_obj[i]), _get_param_info(i, PARAM_AUTO_ITS_EVENT_QUAL));
        nfui_nfobject_set_size(event_quality_obj[i], ROP_TABLE_QUALITY_COL_W, ROP_LABEL_H);
        nfui_regi_post_event_callback(event_quality_obj[i], _event_quality_event_handler);
        nfui_nfobject_show(event_quality_obj[i]);
    	nfui_nffixed_put((NFFIXED*)g_page_fixed[page_num], event_quality_obj[i], pos_x, (row_num * 41));
...
// AUDIO
    ...
    nfui_nfobject_show(obj);
    nfui_nffixed_put((NFFIXED*)fixed, obj, tbl_pos_x, tbl_pos_y);
    ...
    for (i = 0; i < GUI_CHANNEL_CNT; i++)
    {
        ...
        nfui_combobox_set_data_no_expose(NF_COMBOBOX(audio_obj[i]), _get_param_info(i, PARAM_AUTO_ITS_AUDIO));
...
// BUTTON
    ...
    nfui_page_open(PGID_REC_AUTO_INTENSIVE_CONFIG, rop_win, nfui_get_last_user());

    gtk_main();

    nfui_page_close(PGID_REC_AUTO_INTENSIVE_CONFIG, rop_win);
