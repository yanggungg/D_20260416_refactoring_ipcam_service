...
#define DBG_LEVEL		9
#define DBG_MODULE		"VW_DEEPLEARNING_ANIMATE_POPUP"
...
static AOBJECT_INFO_T g_animate_info = {0, };
...
void VW_deepLearning_animate_popup_open(NFWINDOW *parent)
{
	...
	obj = (NFOBJECT*)nfui_nflabel_new_with_pango_font("OBJECT", nffont_get_pango_font(NFFONT_SMALL_SEMI), COLOR_IDX(387));
	nfui_nflabel_set_align((NFLABEL*)obj, NFALIGN_LEFT, 8);
	nfui_nfobject_modify_bg(obj, NFOBJECT_STATE_NORMAL, COLOR_IDX(200));
	nfui_nfobject_use_focus(obj, NFOBJECT_FOCUS_OFF);
	nfui_nfobject_show(obj);
...
void VW_deepLearning_animate_popup_show(AOBJECT_INFO_T *animate_info)
{
	...
	memcpy(&g_animate_info, animate_info, sizeof(AOBJECT_INFO_T));
	...
	memset(strBuf, 0x00, sizeof(strBuf));
	nfui_nflabel_set_text((NFLABEL*)g_rule_obj, animate_info->rule_name);

	memset(strBuf, 0x00, sizeof(strBuf));
	sprintf(strBuf, "%s(""%d""%)", animate_info->class_name, animate_info->confidence);
