...
#include "vw_sys_camera_ipcam_install_manual_add_popup.h"
...
static gboolean post_specific_ip_btn_handler(NFOBJECT *obj, GdkEvent *evt, gpointer data)
{
	switch(evt->type) 
	{
    	case GDK_BUTTON_RELEASE:
    		if(evt->button.button == MOUSE_RIGTH_BUTTON) return FALSE;
                ...
                VW_Manual_Add_Camera_Popup_hide();
    			VW_specific_camera_set_Popup(g_curwnd, win_x, win_y);
    		break;
...
static gboolean post_virtual_cam_btn_handler(NFOBJECT *obj, GdkEvent *evt, gpointer data)
{
	switch(evt->type) 
	{
    	case GDK_BUTTON_RELEASE:
    		if(evt->button.button == MOUSE_RIGTH_BUTTON) return FALSE;
    		    ...
    		    VW_Manual_Add_Camera_Popup_hide();
    			VW_virtual_camera_set_Popup(g_curwnd, win_x, win_y);
...
gboolean VW_Create_Manual_Add_Camera_Popup_Open(NFWINDOW *parent, gint x, gint y)
{
	NFOBJECT *wnd;
	NFOBJECT *main_fixed;
	NFOBJECT *obj;
	
	gchar *strBtn[2] = {"SPECIFIC IP / HOST NAME", "VIRTUAL CAMERA"};
	guint pos_y = 0;
	gint i;
	...
	g_curwnd = wnd;
	nfui_nfwindow_set_title(wnd, "MANUAL ADD CAMERA");
	...
	for(i=0; i<2; i++) {
		...
		if (i == 0)
			nfui_regi_post_event_callback(obj, post_specific_ip_btn_handler);

		if (i == 1)
			nfui_regi_post_event_callback(obj, post_virtual_cam_btn_handler);
...
int VW_Destroy_Manual_Add_Camera_Popup()
{
...
int VW_Manual_Add_Camera_Popup_show(gint x, gint y)
{
	if (!g_curwnd) return 0;
...
int VW_Manual_Add_Camera_Popup_hide()
{
	if (!g_curwnd) return 0;
