...
static EA_AnalysisData g_dvad[GUI_CHANNEL_CNT];
static EA_AnalysisData g_org_dvad[GUI_CHANNEL_CNT];
static guint g_org_double_knock;
static guint g_double_knock;
...
static NFOBJECT *g_doubleknockObj[GUI_CHANNEL_CNT];
...
static NFOBJECT *g_double_knock_obj;
static NFOBJECT *g_page_fixed[PAGE_FIXED_CNT];
static NFOBJECT *g_lb_page_num;



static void init_analysis_data()
{
	guint i;

	memset(g_dvad, 0x00, sizeof(EA_AnalysisData) * GUI_CHANNEL_CNT);
	memset(g_org_dvad, 0x00, sizeof(EA_AnalysisData) * GUI_CHANNEL_CNT);
	for(i=0; i<GUI_CHANNEL_CNT; i++)
		DAL_get_analysis_event_data(&g_dvad[i], i);

	g_memmove(g_org_dvad, g_dvad, sizeof(EA_AnalysisData)*GUI_CHANNEL_CNT);

	g_double_knock = DAL_get_double_knock_data();
	g_org_double_knock = g_double_knock;
}

static void set_analysis_data()
{
	DAL_set_analysis_event_data_all(g_dvad, GUI_CHANNEL_CNT);
	DAL_set_double_knock_data(g_double_knock);
...
static void set_data_to_obj(gint expose)
{
	...
	for(i=0; i<GUI_CHANNEL_CNT; i++) {
		...
		conv_alarmOut_data_to_string(g_dvad[i].almOut, str);
		...
		{
#if defined(_SUPPORT_EVENT_PRESET)
			nfui_check_button_set_active((NFCHECKBUTTON*)g_presetObj[i], g_dvad[i].preset);

            if (g_dvad[i].preset)    nfui_nfobject_enable(g_presetEditObj[i]);
            else                    nfui_nfobject_disable(g_presetEditObj[i]);

            nfui_signal_emit(g_presetEditObj[i], GDK_EXPOSE, FALSE);
#endif
			nfui_check_button_set_active((NFCHECKBUTTON*)g_buzzObj[i], g_dvad[i].buzzer);
			nfui_check_button_set_active((NFCHECKBUTTON*)g_vpopObj[i], g_dvad[i].vpop);	
			nfui_check_button_set_active((NFCHECKBUTTON*)g_mailObj[i], g_dvad[i].email);
			nfui_check_button_set_active((NFCHECKBUTTON*)g_ftpObj[i], g_dvad[i].ftp);
#if defined(_SUPPORT_USER_PHONE)
			nfui_check_button_set_active((NFCHECKBUTTON*)g_mobileObj[i], g_dvad[i].mobile);
#endif
            if(support_mobilepush)
    			nfui_check_button_set_active((NFCHECKBUTTON*)g_mobilepushObj[i], g_dvad[i].mobilepush);

			nfui_spin_button_set_index((NFSPINBUTTON*)g_doubleknockObj[i], g_dvad[i].doubleknock);
		}
		else
		{
#if defined(_SUPPORT_EVENT_PRESET)
			nfui_check_button_set_active_no_expose((NFCHECKBUTTON*)g_presetObj[i], g_dvad[i].preset);

            if (g_dvad[i].preset)    nfui_nfobject_enable(g_presetEditObj[i]);
            else                    nfui_nfobject_disable(g_presetEditObj[i]);
#endif
			nfui_check_button_set_active_no_expose((NFCHECKBUTTON*)g_buzzObj[i], g_dvad[i].buzzer);
			nfui_check_button_set_active_no_expose((NFCHECKBUTTON*)g_vpopObj[i], g_dvad[i].vpop);
			nfui_check_button_set_active_no_expose((NFCHECKBUTTON*)g_mailObj[i], g_dvad[i].email);
			nfui_check_button_set_active_no_expose((NFCHECKBUTTON*)g_ftpObj[i], g_dvad[i].ftp);
#if defined(_SUPPORT_USER_PHONE)
			nfui_check_button_set_active_no_expose((NFCHECKBUTTON*)g_mobileObj[i], g_dvad[i].mobile);
#endif
            if(support_mobilepush)
			nfui_check_button_set_active_no_expose((NFCHECKBUTTON*)g_mobilepushObj[i], g_dvad[i].mobilepush);

			nfui_spin_button_set_index_no_expose((NFSPINBUTTON*)g_doubleknockObj[i], g_dvad[i].doubleknock);
		}
	}
	_set_all_chk_btn_all_event(expose);
#if 0
	if (var_get_supported_double_knock()) 
	{
	    nfui_check_button_set_active_no_expose((NFCHECKBUTTON*)g_double_knock_obj, g_double_knock);
	    if (expose) nfui_signal_emit(g_double_knock_obj, GDK_EXPOSE, TRUE);
	}
#endif
}
...
static void get_data_from_obj()
{
	...
	for(i=0; i<GUI_CHANNEL_CNT; i++) {
		g_dvad[i].buzzer   = nfui_check_button_get_active((NFCHECKBUTTON*)g_buzzObj[i]);
		g_dvad[i].vpop = nfui_check_button_get_active((NFCHECKBUTTON*)g_vpopObj[i]);
		g_dvad[i].email    = nfui_check_button_get_active((NFCHECKBUTTON*)g_mailObj[i]);
		g_dvad[i].ftp    = nfui_check_button_get_active((NFCHECKBUTTON*)g_ftpObj[i]);
#if defined(_SUPPORT_USER_PHONE)
		g_dvad[i].mobile = nfui_check_button_get_active((NFCHECKBUTTON*)g_mobileObj[i]);
#endif
        if(support_mobilepush)
    		g_dvad[i].mobilepush = nfui_check_button_get_active((NFCHECKBUTTON*)g_mobilepushObj[i]);
		
		g_dvad[i].doubleknock = (gint)nfui_spin_button_get_index((NFSPINBUTTON*)g_doubleknockObj[i]);
	}
#if 0
	if (var_get_supported_double_knock()) {
	    g_double_knock = nfui_check_button_get_active((NFCHECKBUTTON*)g_double_knock_obj);
	}
#endif
}
...
static gboolean post_ao_all_btn_event_handler(NFOBJECT *obj, GdkEvent *evt, gpointer data)
{
	if(evt->type == GDK_BUTTON_PRESS) {
		...
		for(i=0; i<GUI_CHANNEL_CNT; i++) {
			if(mask != g_dvad[i].almOut) {
				g_dvad[i].almOut = mask;
...
static gboolean post_preset_edit_btn_event_handler(NFOBJECT *obj, GdkEvent *evt, gpointer data)
{
	...
	if(evt->type == GDK_BUTTON_RELEASE)
	{
        ...
        for (j = 0; j < GUI_CHANNEL_CNT; j++)
            info->number[j] = g_dvad[i].preset_num[j];

        vw_evt_ptz_preset_popup_open(g_curwnd, info, 0xffffffff);

        for (j = 0; j < GUI_CHANNEL_CNT; j++)
            g_dvad[i].preset_num[j] = info->number[j];
...
static gboolean post_ignore_all_event_handler(NFOBJECT *obj, GdkEvent *evt, gpointer data)
{
	if(evt->type == NFEVENT_COMBOBOX_CHANGED) {
		...
		for(i=0; i<GUI_CHANNEL_CNT; i++)
			nfui_spin_button_set_index((NFSPINBUTTON*)g_doubleknockObj[i], idx);

		for(i=0; i<GUI_CHANNEL_CNT; i++)
			nfui_signal_emit(g_doubleknockObj[i], GDK_EXPOSE, TRUE);
	}
...
static gboolean post_ao_label_event_handler(NFOBJECT *obj, GdkEvent *evt, gpointer data)
{
	...
	if(evt->type == GDK_2BUTTON_PRESS || kpid == KEYPAD_ENTER) {
		...
		mask = (ALARM_OUT_MASK_TYPE | g_dvad[ch].almOut);
		if(!VW_ChannelMask_Ctrl(g_curwnd, "ALARM OUT CHANNEL", off_x, off_y + obj->height, &mask))
			return FALSE;

		if(mask != g_dvad[ch].almOut) {
			g_dvad[ch].almOut = mask;

			memset(str, 0x00, ALARM_STRING_LENGTH);
			conv_alarmOut_data_to_string(g_dvad[ch].almOut, str);
...
static gboolean post_cancelbutton_event_handler(NFOBJECT *obj, GdkEvent *evt, gpointer data)
{
	if(evt->type == GDK_BUTTON_RELEASE)
	{
		...
		g_memmove(g_dvad, g_org_dvad, sizeof(EA_AnalysisData)*GUI_CHANNEL_CNT);
		g_double_knock = g_org_double_knock;
...
static gboolean post_applybutton_event_handler(NFOBJECT *obj, GdkEvent *evt, gpointer data)
{
	...
	if(evt->type == GDK_BUTTON_RELEASE)
	{
		...
		if(memcmp(g_org_dvad, g_dvad, sizeof(EA_AnalysisData) * GUI_CHANNEL_CNT)) {
        	DAL_set_analysis_event_data_all(g_dvad, GUI_CHANNEL_CNT);
			g_memmove(g_org_dvad, g_dvad, sizeof(EA_AnalysisData)*GUI_CHANNEL_CNT);
		...
		if (g_double_knock != g_org_double_knock) {
		    g_org_double_knock = g_double_knock;
        	DAL_set_double_knock_data(g_double_knock);
...
static gboolean post_closebutton_event_handler(NFOBJECT *obj, GdkEvent *evt, gpointer data)
{
	if(evt->type == GDK_BUTTON_RELEASE)
	{
		...
		if((!memcmp(g_org_dvad, g_dvad, sizeof(EA_AnalysisData) * GUI_CHANNEL_CNT)) && (g_double_knock == g_org_double_knock)) {
...
*/
	...
	nfui_combobox_set_display_string(NF_COMBOBOX(obj), "DOUBLE KNOCK");
	nfui_combobox_set_skin_type(NF_COMBOBOX(obj), NFCOMBOBOX_TYPE_2);
	nfui_combobox_set_align(NF_COMBOBOX(obj), NFALIGN_CENTER, 0);
	nfui_nfobject_set_size(obj, 244, 40);
	nfui_nfobject_show(obj);
	nfui_regi_post_event_callback(obj, post_ignore_all_event_handler);
	...
	for(i=0; i<GUI_CHANNEL_CNT; i++) {
		...
		obj = nfui_spinbutton_new(strOnOff, 2, g_dvad[i].doubleknock);
		...
		nfui_nfobject_set_size(obj, 244, 40);
		nfui_nfobject_show(obj);
		nfui_nffixed_put((NFFIXED*)g_page_fixed[page_num], obj, action_pos_x + 166, (row_num * 41));
		g_doubleknockObj[i] = obj;
	...
	{
		memset(buf, 0x00, sizeof(buf));
		conv_alarmOut_data_to_string(g_dvad[i].almOut, buf);
	...
	for(j=0; j<GUI_CHANNEL_CNT; j++)
    {
    	...
    	obj = (NFOBJECT*)nfui_checkbutton_new(g_dvad[j].preset);
    	...
    	if (!g_dvad[j].preset)   nfui_nfobject_disable(obj);
	...
	for(i = ACTION_BUZZER; i < ACTION_MAX; i++)
	{
		...
		for(j=0; j<GUI_CHANNEL_CNT; j++)
		{
			...
			if(i == ACTION_BUZZER)	    obj = (NFOBJECT*)nfui_checkbutton_new(g_dvad[j].buzzer);
			else if(i == ACTION_VPOPUP) obj = (NFOBJECT*)nfui_checkbutton_new(g_dvad[j].vpop);
			else if(i == ACTION_EMAIL)  obj = (NFOBJECT*)nfui_checkbutton_new(g_dvad[j].email);
			else if(i == ACTION_FTP)    obj = (NFOBJECT*)nfui_checkbutton_new(g_dvad[j].ftp);
#if defined(_SUPPORT_USER_PHONE)
			else if(i == ACTION_MOBILE)	obj = (NFOBJECT*)nfui_checkbutton_new(g_dvad[j].mobile);
#endif
			else if(i == ACTION_MOBILEPUSH)	obj = (NFOBJECT*)nfui_checkbutton_new(g_dvad[j].mobilepush);
...
// DOUBLE CHECK
    ...
    {
        pos_x = 27;
        pos_y += (41 * GUI_CHANNEL_CNT) + 20;

        obj = (NFOBJECT*)nfui_checkbutton_new(g_double_knock);
        nfui_check_button_set_skin_type(NF_CHECKBUTTON(obj), NFCHECK_TYPE_NORMAL);
        nfui_check_get_size(NF_CHECKBUTTON(obj), &size_w, &size_h);
        nfui_nfobject_show(obj);
        nfui_nffixed_put((NFFIXED*)content_fixed, obj, pos_x, pos_y + (40 - size_h)/2);
        g_double_knock_obj = obj;

        pos_x += 35;

        obj = (NFOBJECT*)nfui_nflabel_new_with_pango_font("ENABLE DOUBLE KNOCK", nffont_get_pango_font(NFFONT_MEDIUM_SEMI), COLOR_IDX(120));
