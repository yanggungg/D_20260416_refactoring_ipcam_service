...
static gint _get_cntr_name_string(VAAID vaaid, gint cntr_id, gchar *str)
{
    ITX_VACNTR_SHAPE shape;
   
    vaa_itx_get_cntr_shape(vaaid, cntr_id, &shape);
    strcpy(str, shape.name);
    
    return 0;
}

static gint _get_cntr_color_string(VAAID vaaid, gint cntr_id, gchar *str)
{
    ITX_VACNTR_SHAPE shape;
...
{
    if(evt->type == INFY_VCA_EVENT_NOTIFY)
    {
    	NFOBJECT *top;
		...
		for(i = 0; i < p[1]; i++, pevt++) 
		{
        	...
        	if (pevt->type & IVCA_ET_COUNTER) 
        	{
                ...
                _get_cntr_color_string(vaaid, pevt->rule_id, strBuf);
                log_list[LOGLABEL_RULE_COLOR] = g_strdup(strBuf);

                memset(strBuf, 0x00, sizeof(strBuf));
                _get_cntr_name_string(vaaid, pevt->rule_id, strBuf);
                log_list[LOGLABEL_RULE_NAME] = g_strdup(strBuf);
        	}
        	else
        	{
        	    memset(strBuf, 0x00, sizeof(strBuf));
                _get_rule_color_string(vaaid, pevt->rule_id, strBuf);
                log_list[LOGLABEL_RULE_COLOR] = g_strdup(strBuf);

                memset(strBuf, 0x00, sizeof(strBuf));
                _get_rule_name_string(vaaid, pevt->rule_id, strBuf);
                log_list[LOGLABEL_RULE_NAME] = g_strdup(strBuf);
        	}
...
gint _vca_rev_live_log_page(NFOBJECT *parent)
{
    ...
    uxm_reg_imsg_event(obj, INFY_VCA_EVENT_NOTIFY);
    uxm_monitor_on_imsg_event(obj, INFY_VCA_EVENT_NOTIFY);
