...
static gboolean post_upgradebutton_event_handler(NFOBJECT *obj, GdkEvent *evt, gpointer data)
{
	if(evt->type == GDK_BUTTON_RELEASE)
	{
		...
		if(ret == NFTOOL_MB_OK)
		{
            ...
            evt_send_to_local(INFY_SCM_PREPARE_FWUP_OPEN, UPGRADE_TYPE_LOCAL, 0, 0);

            _check_valid_fwfile(mount_path, file_name);
#else
...
void VW_System_FWUpgrade_Open(NFWINDOW *parent)
{
	NFOBJECT *main_fixed;
	NFOBJECT *obj;
...
// <---- UPGRADE BUTTON
	upgrade_obj = nftool_normal_button_create_type1("UPGRADE", FW_UP_LOAD_BTN_W);
	...
	nfui_regi_post_event_callback(upgrade_obj, post_upgradebutton_event_handler);
