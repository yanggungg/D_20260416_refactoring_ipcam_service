...
static NFOBJECT *g_option_plateno_obj = NULL;
...
static gint _load_plateno_option_data(DVA_COMPONENT_DATA_T *component_data, gint expose)
{
    ITX_LPRRULE_PROP prop;
    
    memset(&prop, 0x00, sizeof(ITX_LPRRULE_PROP));
    dvaa_itx_plateno_get_rule_prop(g_dvaaid, &prop);

    nfui_check_button_set_active_no_expose((NFCHECKBUTTON*)g_option_bbx_obj, prop.sw_obj_bb);
    //nfui_check_button_set_active_no_expose((NFCHECKBUTTON*)g_option_plateno_obj, prop.sw_obj_number);
    ...
    if (expose) nfui_signal_emit(g_option_plateno_obj, GDK_EXPOSE, TRUE);
...
static gboolean post_bbx_check_event_handler(NFOBJECT *obj, GdkEvent *evt, gpointer data)
{
    if (evt->type == NFEVENT_CHECKBUTTON_CHANGED)
    {
        ...
        dvaa_itx_plateno_set_rule_prop(g_dvaaid, &prop);
    }

    return FALSE;
}

static gboolean post_plateno_check_event_handler(NFOBJECT *obj, GdkEvent *evt, gpointer data)
{
    if (evt->type == NFEVENT_CHECKBUTTON_CHANGED)
    {
        ITX_LPRRULE_PROP prop;
        ...
        if (nfui_check_button_get_active((NFCHECKBUTTON*)obj)) 
        {
            //prop.sw_obj_number = 1;
        }
        else 
        {
            //prop.sw_obj_number = 0;
        }
...
gint _dvabx_plateno_option_page(NFOBJECT *parent)
{
    NFOBJECT *fixed;
    NFOBJECT *obj;
...
void _dvabx_plateno_option_show(NFOBJECT *parent)
{
    ...
    g_dvaaid = dvaa_get_dvaaid(component_data->preview.ch);
    _load_plateno_option_data(component_data, 0);

    nfui_nfobject_show(parent);
}

void _dvabx_plateno_option_hide(NFOBJECT *parent)
{
	NFOBJECT *topwin;
