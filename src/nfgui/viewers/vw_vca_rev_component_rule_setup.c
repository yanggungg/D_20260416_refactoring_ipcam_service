/*
 * vw_vca_rev_component_rule_setup.c
 *
...
{
    NFOBJECT *icon_line[2];
    NFOBJECT *check_line[2];
}LINE_DATA_T;

typedef struct _AREA_DATA_T
{
    NFOBJECT *icon_area[5];
    NFOBJECT *check_area[5];
}AREA_DATA_T;
...
static void _change_area_status(AREA_DATA_T *area_data, gint status, gint expose)
{
    ...
    if(status)
    {
        nfui_nfimage_change_image((NFIMAGE*)area_data->icon_area[0], IMG_LIVE_ENTER_N);
        nfui_nfimage_change_image((NFIMAGE*)area_data->icon_area[1], IMG_LIVE_EXIT_N);
        nfui_nfimage_change_image((NFIMAGE*)area_data->icon_area[2], IMG_LIVE_REMOVE_N);
    ...
    {
        nfui_nfimage_change_image((NFIMAGE*)area_data->icon_area[0], IMG_LIVE_ENTER_D);
        nfui_nfimage_change_image((NFIMAGE*)area_data->icon_area[1], IMG_LIVE_EXIT_D);
        nfui_nfimage_change_image((NFIMAGE*)area_data->icon_area[2], IMG_LIVE_REMOVE_D);
...
gint vw_vca_rev_rule_setup_component_open(NFOBJECT *parent, guint opt)
{
    ...
    gchar *str_area[] = {"VCA-ENTER", "VCA-EXIT", "VCA-REMOVED", "VCA-LOITERING", "VCA-STOPPED"};
    ...
    if (opt & (1 << OPT_RULE_SETUP_AREA))
    {
        ...
        area_data->icon_area[0] = (NFOBJECT*)nfui_nfimage_new((IMG_LIVE_ENTER_D));
        area_data->icon_area[1] = (NFOBJECT*)nfui_nfimage_new((IMG_LIVE_EXIT_D));
        area_data->icon_area[2] = (NFOBJECT*)nfui_nfimage_new((IMG_LIVE_REMOVE_D));
        ...
        nfui_regi_post_event_callback(area_data->check_area[0], post_enter_check_event_handler);
        nfui_regi_post_event_callback(area_data->check_area[1], post_exit_check_event_handler);
        nfui_regi_post_event_callback(area_data->check_area[2], post_removed_check_event_handler);
