...
void VW_Init_DiskExternal_Info(NFOBJECT *parent)
{
	...
	GdkPixbuf *pbIcon[EXT_DISP_DISK_COUNT+1];
	...
	/* disk n table */
	diskTbl = (NFOBJECT*)nfui_nftable_new(7, EXT_DISP_DISK_COUNT, 2, 1, table_w, 40);	
	...
	for(i=0; i<EXT_DISP_DISK_COUNT+1; i++) {
...
gboolean display_external_disk_information(gboolean expose)
{
	...
	/* get disk data */
	disk_i = get_disk_info(EXTERNAL_DISK_GRP);
	smart_i = get_smart_disk_info(EXTERNAL_DISK_GRP);
	disk_t = get_disk_rec_time(EXTERNAL_DISK_GRP);
