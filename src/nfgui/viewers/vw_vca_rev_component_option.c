...
static gboolean post_shadow_removal_check_event_handler(NFOBJECT *obj, GdkEvent *evt, gpointer data)
{
	if(evt->type == NFEVENT_CHECKBUTTON_CHANGED)
	{
        ...
        component_data->option.shadow_removal = status;
        component_action->option_cb(obj);
	}
	else if (evt->type == NFEVENT_VCAREV_COMPONENT_DATA_SYNC)
	{
        ...
        nfui_check_button_set_active_no_expose((NFCHECKBUTTON*)obj, component_data->option.shadow_removal);
        nfui_signal_emit(obj, GDK_EXPOSE, TRUE);
	}
...
static gboolean post_dis_obj_id_check_event_handler(NFOBJECT *obj, GdkEvent *evt, gpointer data)
{
	...
	else if (evt->type == NFEVENT_VCAREV_COMPONENT_DATA_SYNC)
	{
        ...
        vaa_itx_get_calb_result(vaaid, &calb_res);
...
// VA OPTIONS FIXED
    ...
    obj = (NFOBJECT*)nfui_nflabel_new_with_pango_font("VA OPTION", nffont_get_pango_font(NFFONT_MEDIUM_SEMI), COLOR_IDX(205));
    ...
    nfui_regi_post_event_callback(obj, post_shadow_removal_check_event_handler);
    
    obj = (NFOBJECT*)nfui_nflabel_new_with_pango_font("SHADOW REMOVAL (RECOMMENDED)", nffont_get_pango_font(NFFONT_SMALL_SEMI), COLOR_IDX(205));
    nfui_nfobject_modify_bg((NFOBJECT*)obj, NFOBJECT_STATE_NORMAL, COLOR_IDX(200));
...
// OSD OPTION FIXED
    ...
    nfui_nfobject_show(fixed);
    nfui_nffixed_put((NFFIXED*)parent, fixed, 5, 340-36);
...
// UNITS SETUP OPTION FIXED
    ...
    {
        fixed = (NFOBJECT*)nfui_nffixed_new();
        nfui_nfobject_set_size(fixed, DEFAULT_COMPONENT_WIDTH-20, 92);
        nfui_nfobject_show(fixed);
        nfui_nffixed_put((NFFIXED*)parent, fixed, 5, 314 + 287 + 20);
        nfui_regi_post_event_callback(fixed, post_osd_option_fixed_event_handler);
...
//        nfui_nfobject_show(obj);
    ...
    nfui_nffixed_put((NFFIXED*)parent, obj, DEFAULT_COMPONENT_WIDTH-10-230, pos_y+20);
