...
#include "vw_provide_devinfo_notice2.h"


static NFWINDOW *g_curwnd = 0;
static NFOBJECT *g_list_obj;

static gint g_agr_policy;
...
static gboolean post_close_event_handler(NFOBJECT *obj, GdkEvent *evt, gpointer data)
{
	if(evt->type == GDK_BUTTON_RELEASE)
	{
        ...
        DAL_set_agr_policy(g_agr_policy);
...
static gboolean post_fixed_event_handler(NFOBJECT *obj, GdkEvent *evt, gpointer data)
{
	...
	{
		uxm_unreg_imsg_event(obj, INFY_SYSDB_CHANGE_NOTIFY);
	...
	else if (evt->type == INFY_SYSDB_CHANGE_NOTIFY)
	{
		g_agr_policy = DAL_get_agr_policy();
		if (g_agr_policy)
		{
			top = nfui_nfobject_get_top(obj);
			if(top) nfui_nfobject_destroy(top);
...
gint vw_provide_devinfo_notice2_open(NFOBJECT *parent)
{
	NFOBJECT *win;
	NFOBJECT *fixed;
	NFOBJECT *obj;
	...
	// <----- title
	obj = (NFOBJECT*)nfui_nflabel_new_with_pango_font("NOTICE", nffont_get_pango_font(NFFONT_LARGE_SEMI), COLOR_IDX(205));
    ...
    nfutil_get_line_feed_string(lookup_string("You must agree to provide device information in order to utilize DDNS,\nremote access, and other network-related features."), 
	...
	nfui_nfobject_set_size(list_obj, win->width-40, win->height-pos_y-140);
	...
	nfui_nffixed_put((NFFIXED*)fixed, list_obj, pos_x, pos_y);
	g_list_obj = list_obj;
    ...
    nfui_nfobject_use_focus(obj, NFOBJECT_FOCUS_OFF);
    nfui_nfobject_show(obj);
    nfui_nfobject_set_size(obj, win->width-40-80, 40); 
	...
	uxm_reg_imsg_event(fixed, INFY_SYSDB_CHANGE_NOTIFY);
	uxm_monitor_on_imsg_event(fixed, INFY_SYSDB_CHANGE_NOTIFY);
