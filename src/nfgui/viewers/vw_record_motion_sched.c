...
#include "vw_record_motion_sched.h"
...
#define MOTION_SCHE_TIME_LABEL_COLS_SPACE       (2)

#define MOTION_SCHE_DAY_MENU_W_SUM              (284)

#define MOTION_SCHE_DAY_LABEL_X                 (4)
#define MOTION_SCHE_DAY_LABEL_Y                 (0)
#define MOTION_SCHE_DAY_LABEL_W                 (60)
#define MOTION_SCHE_DAY_LABEL_H                 (40)

#define MOTION_SCHE_DAY_DROPDOWN_X              (MOTION_SCHE_TABLE_X + MOTION_SCHE_DAY_LABEL_W)
#define MOTION_SCHE_DAY_DROPDOWN_Y              (MOTION_SCHE_DAY_LABEL_Y)
#define MOTION_SCHE_DAY_DROPDOWN_W              (MOTION_SCHE_DAY_MENU_W_SUM - MOTION_SCHE_DAY_LABEL_W)
#define MOTION_SCHE_DAY_DROPDOWN_H              (MOTION_SCHE_DAY_LABEL_H)

#define MOTION_SCHE_CH_LABEL_W                  (232)

#define MOTION_SCHE_TABLE_COL_CNT               (1)
#define MOTION_SCHE_TABLE_ROW_CNT               (GUI_CHANNEL_CNT)

#define MOTION_SCHE_TIME_LABEL_X                (MOTION_SCHE_DAY_LABEL_X+MOTION_SCHE_CH_LABEL_W+2)
#define MOTION_SCHE_TIME_LABEL_Y                (MOTION_SCHE_DAY_LABEL_Y+MOTION_SCHE_DAY_LABEL_H+27)
#define MOTION_SCHE_TIME_LABEL_W                (48)
#define MOTION_SCHE_TIME_LABEL_H                (40)

#define MOTION_SCHE_TABLE_X                     (MOTION_SCHE_DAY_LABEL_X)
#define MOTION_SCHE_TABLE_Y                     (MOTION_SCHE_DAY_LABEL_Y+MOTION_SCHE_DAY_LABEL_H+68)
#define MOTION_SCHE_TABLE_CELL_H                (39)

#define MOTION_SCHE_TABLE_COLS_SPACE            (0)
#define MOTION_SCHE_TABLE_ROWS_SPACE            (2)

#define MOTION_SCHE_TILE2_ROW_CNT               (ROW_CNT_PER_PAGE)
#define MOTION_SCHE_TILE2_COL_CNT               (24)

#define MOTION_SCHE_TILE2_X                     (MOTION_SCHE_CH_LABEL_W+2)
#define MOTION_SCHE_TILE2_Y                     (MOTION_SCHE_TABLE_Y-1)
#define MOTION_SCHE_TILE2_W                     (((48+2) * 24))
#define MOTION_SCHE_TILE2_H                     ((MOTION_SCHE_TABLE_CELL_H+2)*MOTION_SCHE_TILE2_ROW_CNT)


#define MOTION_SCHE_COPY_BTN_X                  (MENU_V_SUBTAB_INNER_W - 226 - 7)
#define MOTION_SCHE_COPY_BTN_Y                  (MENU_V_SUBTAB_INNER_H - 40 - 4)

#define MOTION_SCHE_ON_LABEL_W                  (80)
#define MOTION_SCHE_ON_LABEL_H                  (26)
#define MOTION_SCHE_ON_LABEL_X                  (MENU_V_SUBTAB_INNER_W - MOTION_SCHE_ON_LABEL_W - 7)
#define MOTION_SCHE_ON_LABEL_Y                  (MOTION_SCHE_DAY_LABEL_Y + MOTION_SCHE_DAY_LABEL_H)

#define MOTION_SCHE_ON_LEGEND_W                 (22)
#define MOTION_SCHE_ON_LEGEND_H                 (22)
#define MOTION_SCHE_ON_LEGEND_X                 (MOTION_SCHE_ON_LABEL_X - 6 - MOTION_SCHE_ON_LEGEND_W)
#define MOTION_SCHE_ON_LEGEND_Y                 (MOTION_SCHE_ON_LABEL_Y)

#define MOTION_SCHE_OFF_LABEL_W                 (80)
#define MOTION_SCHE_OFF_LABEL_H                 (26)
#define MOTION_SCHE_OFF_LABEL_X                 (MOTION_SCHE_ON_LEGEND_X - MOTION_SCHE_OFF_LABEL_W)
#define MOTION_SCHE_OFF_LABEL_Y                 (MOTION_SCHE_ON_LABEL_Y)

#define MOTION_SCHE_OFF_LEGEND_W                (22)
#define MOTION_SCHE_OFF_LEGEND_H                (22)
#define MOTION_SCHE_OFF_LEGEND_X                (MOTION_SCHE_OFF_LABEL_X - MOTION_SCHE_OFF_LEGEND_W - 7)
#define MOTION_SCHE_OFF_LEGEND_Y                (MOTION_SCHE_ON_LABEL_Y)
...
static void redraw_tile_modeArea(NFTILE *tile, guint day)
{
    ...
    s_ch = tile_idx * MOTION_SCHE_TILE2_ROW_CNT;
    e_ch = s_ch + MOTION_SCHE_TILE2_ROW_CNT;

    if(nfui_tile_drawable(NF_TILE(tile))) {
        for(ch = s_ch; ch < e_ch; ch++) {
            for(j=0; j<time; j++) {
                get_record_motion_sche_data(day, ch, j);

                if(!g_ascii_strcasecmp(motion_info[ch][PARAM_MODE_COL], "0")) 
                    color_n = SELECT_STATE_COLOR_2;
                else if(!g_ascii_strcasecmp(motion_info[ch][PARAM_MODE_COL], "1")) 
    ...
    switch(evt->type) {
        ...
        case NFEVENT_TILE_END_SELECT:
        {
            ...
            else
            {
                ...
                {
                    ch = i + (MOTION_SCHE_TILE2_ROW_CNT * tile_idx);
                    
                    for(j = s_col; j <= e_col; j++) 
                    {
                        if(get_record_schedMode())  
                            get_record_motion_sche_data(set_days, ch, j);
                        else                        
                            get_record_motion_sche_data(REC_DAILY_PARAM, ch, j);

                        motion_info[ch][PARAM_MODE_COL] = buf;

                        if(get_record_schedMode())  
                            set_record_motion_sche_info(set_days, ch, j);
                        else                    
                            set_record_motion_sche_info(REC_DAILY_PARAM, ch, j);
                    }
                }
...
static gboolean post_applybutton_event_handler(NFOBJECT *obj, GdkEvent *evt, gpointer data)
{
    if(evt->type == GDK_BUTTON_RELEASE)
    {
        ...
        if(is_changed_motion_data()) {
            if(!set_record_motion_db(REC_SCHED_DATA_CHANGED)) {
                g_warning("%s : set record motion error", __FUNCTION__);
            }
...
static gboolean post_copy_button_event_handler(NFOBJECT *obj, GdkEvent *evt, gpointer data)
{
	...
	if(set_days == 8){
        ...
        for(ch = 0; ch < GUI_CHANNEL_CNT; ch++) 
        {
            ...
            {
                get_record_motion_sche_data(set_days, ch, time);

                for (day = REC_SUN_PARAM; day <= REC_SAT_PARAM; day++) 
                {
                    if (day == set_days) continue;
                    if (day_mask & (1 << day))  
                        set_record_motion_sche_info(day, ch, time);
                }
                
                if (day_mask & (1 << (REC_USR_HOL_PARAM) )) {
                        set_record_motion_sche_info(REC_USR_HOL_PARAM, ch, time);
		}
            }
        }
    }
...
static gboolean post_cancelbutton_event_handler(NFOBJECT *obj, GdkEvent *evt, gpointer data)
{
    ...
    if(evt->type == GDK_BUTTON_RELEASE)
    {
        ...
        if(get_record_schedMode()) {
            if(!get_record_motion_db(set_days, 0)) 
                g_warning("%s : get record motion error", __FUNCTION__);

            day_param = set_days;
        }else {
            if(!get_record_motion_db(REC_DAILY_PARAM, 0)) {
                g_warning("%s : get record motion error", __FUNCTION__);
            }
...
static gboolean sched_page_parent_event_handler(NFOBJECT *obj, GdkEvent *evt, gpointer data)
{
    ...
    if(evt->type == GDK_EXPOSE) {
        ...
        gdk_draw_rectangle(drawable, gc, TRUE, x+MOTION_SCHE_OFF_LEGEND_X, y+MOTION_SCHE_OFF_LEGEND_Y, MOTION_SCHE_OFF_LEGEND_W, MOTION_SCHE_OFF_LEGEND_H);

        gdk_gc_set_rgb_fg_color(gc, &on_color);                     // ON
        gdk_draw_rectangle(drawable, gc, TRUE, x+MOTION_SCHE_ON_LEGEND_X, y+MOTION_SCHE_ON_LEGEND_Y, MOTION_SCHE_ON_LEGEND_W, MOTION_SCHE_ON_LEGEND_H);
...
void VW_Init_RecMotionSched_Page(NFOBJECT *parent)
{
    ...
    guint width[] = {MOTION_SCHE_CH_LABEL_W};
    guint x, y;
    guint i,space;
	gint size_w, size_h;
...
// TIME LABEL
    for (i = 0; i < 24; i++)
    {
        ...
        nfui_nfobject_set_size(obj, MOTION_SCHE_TIME_LABEL_W, MOTION_SCHE_TIME_LABEL_H);
        nfui_nflabel_set_align((NFLABEL*)obj, NFALIGN_CENTER, 0);
        nfui_nfobject_modify_bg(obj, NFOBJECT_STATE_NORMAL, COLOR_IDX(115));
        nfui_nfobject_show(obj);
        nfui_nffixed_put((NFFIXED*)content_fixed, obj, MOTION_SCHE_TIME_LABEL_X + (MOTION_SCHE_TIME_LABEL_COLS_SPACE+MOTION_SCHE_TIME_LABEL_W)*i, MOTION_SCHE_TIME_LABEL_Y);
    }
...
// DAY LABEL
    ...
    nfui_nfobject_set_size(days_label, MOTION_SCHE_DAY_LABEL_W, MOTION_SCHE_DAY_LABEL_H);
    ...
    nfui_nfobject_hide(days_label);
    nfui_nffixed_put((NFFIXED*)content_fixed, days_label, MOTION_SCHE_DAY_LABEL_X, MOTION_SCHE_DAY_LABEL_Y);


// DAY DROP DOWN
    days_combo = nfui_combobox_new(days, 8, 0);
    nfui_combobox_set_skin_type(NF_COMBOBOX(days_combo), NFCOMBOBOX_TYPE_SUBTAB_1);
    nfui_nfobject_set_size(days_combo, MOTION_SCHE_DAY_DROPDOWN_W, MOTION_SCHE_DAY_DROPDOWN_H);
    nfui_nfobject_hide(days_combo);
    nfui_nffixed_put((NFFIXED*)content_fixed, days_combo, MOTION_SCHE_DAY_DROPDOWN_X, MOTION_SCHE_DAY_DROPDOWN_Y);
...
// LEGEND LABEL - ON, OFF
    ...
    nfui_nfobject_use_focus(obj, NFOBJECT_FOCUS_OFF);
    nfui_nfobject_set_size(obj, MOTION_SCHE_OFF_LABEL_W, MOTION_SCHE_OFF_LABEL_H);
    nfui_nflabel_set_align((NFLABEL*)obj, NFALIGN_LEFT, 0);
    nfui_nfobject_modify_bg(obj, NFOBJECT_STATE_NORMAL, COLOR_IDX(186));
    nfui_nfobject_show(obj);
    nfui_nffixed_put((NFFIXED*)content_fixed, obj, MOTION_SCHE_OFF_LABEL_X, MOTION_SCHE_OFF_LABEL_Y);

    obj = (NFOBJECT*)nfui_nflabel_new_with_pango_font("ON", nffont_get_pango_font(NFFONT_SMALL_SEMI), COLOR_IDX(968));
    nfui_nfobject_use_focus(obj, NFOBJECT_FOCUS_OFF);
    nfui_nfobject_set_size(obj, MOTION_SCHE_ON_LABEL_W, MOTION_SCHE_ON_LABEL_H);
    nfui_nflabel_set_align((NFLABEL*)obj, NFALIGN_LEFT, 0);
    nfui_nfobject_modify_bg(obj, NFOBJECT_STATE_NORMAL, COLOR_IDX(186));
    nfui_nfobject_show(obj);
    nfui_nffixed_put((NFFIXED*)content_fixed, obj, MOTION_SCHE_ON_LABEL_X, MOTION_SCHE_ON_LABEL_Y);


// TABLE - CH
    size_w = 0;
    for (i = 0; i < MOTION_SCHE_TABLE_COL_CNT; i++) {
        size_w += width[i];
    }
    size_w += MOTION_SCHE_TILE2_W;
    ...
    nfui_nffixed_put((NFFIXED*)content_fixed, main_page_fixed, MOTION_SCHE_TABLE_X, MOTION_SCHE_TABLE_Y);
    ...
    for (i = 0; i < PAGE_FIXED_CNT; i++)
    {
        ...
        page_ntb[i] = (NFOBJECT*)nfui_nftable_new(MOTION_SCHE_TABLE_COL_CNT, ROW_CNT_PER_PAGE, MOTION_SCHE_TABLE_COLS_SPACE, 1, width, 40);
...
// TIME TILE
    for (i = 0; i < PAGE_FIXED_CNT; i++)
    {
        nftile = nfui_tile_new(MOTION_SCHE_TILE2_ROW_CNT, MOTION_SCHE_TILE2_COL_CNT);
        nfui_nfobject_set_size(nftile, MOTION_SCHE_TILE2_W, MOTION_SCHE_TILE2_H);
    	...
    	nfui_nffixed_put((NFFIXED*)g_page_fixed[i], nftile, MOTION_SCHE_TILE2_X, 0);
...
// COPY SCHEDULE TO BUTTON
    ...
    nfui_nffixed_put((NFFIXED*)content_fixed, obj, MOTION_SCHE_COPY_BTN_X, MOTION_SCHE_COPY_BTN_Y);
...
guint VW_RecMotionParam_get_sched_day()
{
    if(get_record_schedMode())
        return set_days;
    else
