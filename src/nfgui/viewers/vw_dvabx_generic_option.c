...
static gint _load_generic_option_data(DVA_COMPONENT_DATA_T *component_data, gint expose)
{
    ITX_DVARULE_PROP prop;
    
    memset(&prop, 0x00, sizeof(ITX_DVARULE_PROP));
    dvaa_itx_detector_get_rule_prop(g_dvaaid, &prop);
...
static gboolean post_bbx_check_event_handler(NFOBJECT *obj, GdkEvent *evt, gpointer data)
{
    if (evt->type == NFEVENT_CHECKBUTTON_CHANGED)
    {
        ...
        dvaa_itx_detector_set_rule_prop(g_dvaaid, &prop);
    }
...
void _dvabx_generic_option_show(NFOBJECT *parent)
{
    ...
    _load_generic_option_data(component_data, 0);
}
