...
static NFOBJECT *num_btns[11];

static gchar *strResult = NULL;
static gint g_rele_num;
static gdouble g_max_num;
static gint g_cnt;
	...
	if (evt->type == GDK_BUTTON_PRESS)
	{
		...
		for (i = 0; i < 11; i++)
		{
			if (obj == num_btns[i])
			{
				...
				memset(pre_strBuf, 0x00, sizeof(pre_strBuf));
				snprintf(pre_strBuf, sizeof(pre_strBuf), "%s", nfui_nfvklabel_get_all_str((NFVKLABEL*)num_label));

    			nfui_nfvklabel_input_character((NFVKLABEL*)num_label, keyVal[0]);

				memset(post_strBuf, 0x00, sizeof(post_strBuf));
				snprintf(post_strBuf, sizeof(post_strBuf), "%s", nfui_nfvklabel_get_all_str((NFVKLABEL*)num_label));

                for(j = 0; j < strlen(post_strBuf); j++ )
                {
                    if(post_strBuf[j] == '.')
                        break;
                }

                if((strlen(post_strBuf)-j > g_cnt) || (atof(post_strBuf) > g_max_num))
				{
					nfui_nfvklabel_set_string((NFVKLABEL*)num_label, pre_strBuf);
				}                    
...
static gboolean post_backspace_key_event_handler(NFOBJECT *obj, GdkEvent *evt,  gpointer data)
{
	...
	if(evt->type == GDK_BUTTON_PRESS)
	{
        ...
        if(!point)
        {
            nfui_nfobject_enable(num_btns[10]);	
			nfui_signal_emit(num_btns[10], GDK_EXPOSE, TRUE);
        }
	}
...
gchar* Real_NumberKey_Str_Open(NFWINDOW *parent, gchar *str, guint x, guint y, gchar *max_str)
{
	NFOBJECT *main_wnd;
	NFOBJECT *main_fixed;
	NFOBJECT *fixed1;
	...
	strResult = NULL;
	g_max_num = atof(max_str);

    for(i = 0; i < strlen(max_str); i++)
    {
        if(max_str[i] == '.')
        {
            g_cnt = strlen(max_str) - i;
            break;
        }
    }
...
// <---- NUM LABEL
	obj = (NFOBJECT*)nfui_nfvklabel_new_str((gchar*)str, strlen(max_str));
