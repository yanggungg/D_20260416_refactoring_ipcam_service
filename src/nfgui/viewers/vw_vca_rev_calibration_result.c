/*
 * vw_vca_rev_calibration_result.c
 *
...
static gboolean post_okbutton_event_handler(NFOBJECT *obj, GdkEvent *evt, gpointer data)
{
	if(evt->type == GDK_BUTTON_RELEASE)
	{
		if(evt->button.button == MOUSE_RIGTH_BUTTON) 
			return FALSE;
    _vca_rev_calibration_page(g_parent);
...
gint _vca_rev_calibration_result(NFOBJECT *parent)
{
	...
	nfui_nfobject_set_size(fixed, VCA_REV_ROCL_FIXED_W, VCA_REV_ROCL_FIXED_H - (VCA_REV_BTN_H + VCA_REV_BTN_GAP));
