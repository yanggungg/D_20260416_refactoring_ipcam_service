...
static gboolean post_cc_sub_win_event_cb(NFOBJECT *obj, GdkEvent *evt, gpointer data)
{
    ...
    else if (evt->type == INFY_SHORTCUT_PIP_CAPTURE_IMAGE)
    {
        ...
        if (image->ch != g_vimage_channel)
        {
            ...
            ret = scm_req_playback_capture_without_time(INFY_SHORTCUT_PIP_CAPTURE_IMAGE, g_vimage_channel);
            if (ret == -1) evt_send_to_local(INFY_RETRY_SHORTCUT_PIP_CAPTURE_IMAGE, 0, 0, 0);
        ...
        pos_y = obj->y + g_sel_ch[g_vimage_channel]->y;
    ...
    else if (evt->type == INFY_RETRY_SHORTCUT_PIP_CAPTURE_IMAGE)
    {
        NF_NOTIFY_INFO buf;
        guint covert_mask = 0;
        gint ret;
    ...
    else if (evt->type == GDK_DELETE)
    {
        ...
        uxm_unreg_imsg_event(obj, INFY_RETRY_SHORTCUT_PIP_CAPTURE_IMAGE);
        g_curwnd = 0;
    }
...
static gboolean post_cc_sub_btn_event_cb(NFOBJECT *obj, GdkEvent *evt, gpointer data)
{
	...
	else if (evt->type == GDK_ENTER_NOTIFY)
    {
        ...
        if (ivsc.model_code == IPX_M4_MODEL) return FALSE;
        if (ivsc.model_code == IPX_P4E_MODEL) return FALSE;
...
void VW_create_camchange_submenu_pb(NFWINDOW *parent, NFOBJECT *parent_item)
{
    ...
    submenu_img[0] = nf_ui_create_image_button_no_alpha(MK_IMG_LIVE_SHORTCUT_N_PB_BUTTON, 212, IMG_BTN5_N_L, IMG_BTN5_N_M, IMG_BTN5_N_R);
	submenu_img[1] = nf_ui_create_image_button_no_alpha(MK_IMG_LIVE_SHORTCUT_O_PB_BUTTON, 212, IMG_BTN5_O_L, IMG_BTN5_O_M, IMG_BTN5_O_R);
	submenu_img[2] = nf_ui_create_image_button_no_alpha(MK_IMG_LIVE_SHORTCUT_P_PB_BUTTON, 212, IMG_BTN5_P_L, IMG_BTN5_P_M, IMG_BTN5_P_R);
	submenu_img[3] = nf_ui_create_image_button_no_alpha(MK_IMG_LIVE_SHORTCUT_D_PB_BUTTON, 212, IMG_BTN5_D_L, IMG_BTN5_D_M, IMG_BTN5_D_R);
    ...
    nfui_nfwindow_set_title((NFWINDOW*)g_ccWin, "PLAYBACK SHORTCUT SUB CC");
    nfui_regi_post_event_callback(g_ccWin, post_cc_sub_win_event_cb);    
	uxm_reg_imsg_event(g_ccWin, INFY_SHORTCUT_PIP_CAPTURE_IMAGE);
    uxm_reg_imsg_event(g_ccWin, INFY_RETRY_SHORTCUT_PIP_CAPTURE_IMAGE);
    g_curwnd = g_ccWin;
    ...
    for (i = 0; i < GUI_CHANNEL_CNT; i++)
    {
        ...
        obj = (NFOBJECT*)nfui_nfbutton_new_with_param(submenu_img, buf);
...
void VW_show_cc_submenu_pb()
{
    ...
    if(!nfui_nfobject_is_shown(g_ccWin))
    {
        ...
        {            
			...
			if( i == cur_ch )
			{
				nfui_nfobject_disable(g_sel_ch[i]);
			}
			else
			{
				if( nfui_nfobject_is_disabled(g_sel_ch[i]) )
					nfui_nfobject_enable(g_sel_ch[i]);
			}
