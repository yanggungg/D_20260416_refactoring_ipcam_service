...
static DvaBxSchedData g_vsd[GUI_CHANNEL_CNT];
static DvaBxSchedData g_org_vsd[GUI_CHANNEL_CNT];
...
{
	DAL_get_dvabx_schd_data_all(g_vsd, GUI_CHANNEL_CNT);
	memcpy(g_org_vsd, g_vsd, sizeof(g_vsd));
}

static void _save_schedule(void)
{
    DAL_set_dvabx_schd_data_all(g_vsd, GUI_CHANNEL_CNT);
    memcpy(g_org_vsd, g_vsd, sizeof(g_vsd));
}
...
//combobox
	...
	for (i = 0; i < GUI_CHANNEL_CNT; i++) 
	{
		...
		nfui_nfobject_modify_bg(obj, NFOBJECT_STATE_NORMAL, COLOR_IDX(115));
		nfui_nfobject_use_focus(obj, NFOBJECT_FOCUS_OFF);
		nfui_nffixed_put((NFFIXED*)content_fixed, obj, 62, 87 + (i * 42));
		nfui_nfobject_show(obj);
		g_cam_obj[i] = obj;
	}
	...
	nfui_nfobject_set_size(g_tileObj, (50 * 24), (42 * GUI_CHANNEL_CNT));
