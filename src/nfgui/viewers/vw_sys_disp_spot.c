...
#define SPLIT_CH                (SPOTSD_TAB1_MAX_DIV)
#define MAX_SPOT_DATA           (SPOTSD_TAB1_PORT_CNT)
#define OUTPUT_CH               (SPOTSD_TAB1_OUTPUT_CH)
...
static NFOBJECT *edit[MAX_SPOT_DATA];
...
static gboolean post_name_label_event_handler(NFOBJECT *obj, GdkEvent *evt, gpointer data)
{
    ...
    if(evt->type == GDK_2BUTTON_PRESS || kpid == KEYPAD_ENTER)
    {
        ...
        ret = SpotEditDlg_Open(g_curwnd, &spotdata[i], SPLIT_CH, OUTPUT_CH);
...
static gboolean post_applybutton_event_handler(NFOBJECT *obj, GdkEvent *evt, gpointer data)
{
    if(evt->type == GDK_BUTTON_RELEASE)
    {
        ...
        if(memcmp(org_spotdata, spotdata, sizeof(SpotData)*MAX_SPOT_DATA))
        {
            ...
            DAL_set_spot_data_all(spotdata, MAX_SPOT_DATA);

            if(is_changed)
            {           
                NFOBJECT *save_pop;
                save_pop = nftool_mbox_auto(g_curwnd, 1, "NOTICE", "Configuration has been saved.");

                DAL_save_setup_db(NFSETUP_WINDOW_DISPLAY);
                g_timeout_add(3000, _proc_escape, save_pop);
            }
            else
            {
...
void init_DispSpot_page(NFOBJECT *parent)
{
...
// FIXED
    ...
    for(i = 0; i < MAX_SPOT_DATA; i++)
    {
        ...
        DAL_get_spot_data(&spotdata[i], i);
