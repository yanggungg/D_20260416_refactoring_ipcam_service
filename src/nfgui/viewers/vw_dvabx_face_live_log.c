...
static int _free_pixbuf_data(gpointer data, gpointer user_data)
{
    ...
    if (testlog_data->regist_pbuf) g_object_unref(testlog_data->regist_pbuf);
	ifree(data);
	return 0;
}
...
static gint _insert_fr_pixbuf(DVA_COMPONENT_DATA_T *component_data, gint ch, gint cnt, ai_fr_event_t *pevt)
{
    GInputStream *stream = 0;
	GdkPixbuf *event_npixbuf = 0;
    GdkPixbuf *tmp_pixbuf = 0;
	...
	if (retVal) {
		...
		event_npixbuf = gdk_pixbuf_new_from_stream(stream, NULL, NULL);
    ...
    if (analysis_data.dvabox_active) recv_jpeg_data = nf_api_fr_get_image(component_data->aibox_addr, pevt[0].info[0].face_id, 0);
    else recv_jpeg_data = nf_api_aicam_fr_get_image(component_data->preview.ch, pevt[0].info[0].face_id, 0);
    
    if (recv_jpeg_data.size)
    {
        stream = g_memory_input_stream_new_from_data(recv_jpeg_data.memory, recv_jpeg_data.size, NULL);
        tmp_pixbuf = gdk_pixbuf_new_from_stream(stream, NULL, NULL);
        regist_npixbuf = gdk_pixbuf_scale_simple(tmp_pixbuf, 71, 71, GDK_INTERP_BILINEAR);
	...
	for (i = 0; i < cnt; i++)
	{
        ...
        if (event_npixbuf) 
        {
            img_w = gdk_pixbuf_get_width(event_npixbuf);
            img_h = gdk_pixbuf_get_height(event_npixbuf);
            ...
            testlog_data->event_pbuf = _get_target_pixbuf(img_w, img_h, event_npixbuf, coords);
        }

        if (regist_npixbuf) 
        {
            img_w = gdk_pixbuf_get_width(regist_npixbuf);
            img_h = gdk_pixbuf_get_height(regist_npixbuf);

            g_message("%s, %d, w:%d, h:%d", __FUNCTION__, __LINE__, img_w, img_h);
            testlog_data->regist_pbuf = regist_npixbuf;
        }
...
static gboolean post_livelog_list_event_cb(NFOBJECT *obj, GdkEvent *evt, gpointer data)
{
    if ((evt->type == GDK_EXPOSE) || (evt->type == GDK_BUTTON_PRESS) || (evt->type == NFEVENT_LISTBOX_CHANGED))
    {
        ...
        for (i = 0; i < ((NFLISTBOX*)obj)->box_row_cnt; i++)
        {
            ...
            gdk_draw_rectangle(drawable, gc, TRUE, gap_x+2, gap_y+2, 71, 71);	
            gdk_draw_rectangle(drawable, gc, TRUE, gap_x+75+2, gap_y+2, 71, 71);	
            ...
            if (testlog_data->regist_pbuf) {
                img_w = gdk_pixbuf_get_width(testlog_data->regist_pbuf);
                img_h = gdk_pixbuf_get_height(testlog_data->regist_pbuf);
                gdk_draw_pixbuf(drawable, gc, testlog_data->regist_pbuf, 0, 0, gap_x+75+2+(71-img_w)/2, gap_y+2+(71-img_h)/2, -1, -1, GDK_RGB_DITHER_NONE, 0, 0);
            }
    ...
    else if (evt->type == INFY_DELAY_AI_FACE_EVENT)
    {
    	NFOBJECT *top;
        ...
        _insert_fr_pixbuf(component_data, p[0], p[1], pevt);
		...
		for(i = 0; i < p[1]; i++, pevt++) 
		{
            ...
            snprintf(strBuf, sizeof(strBuf), "%s", pevt->info[i].name);
            log_list[2] = g_strdup(strBuf);

            memset(strBuf, 0x00, sizeof(strBuf));        
            _get_timestamp_string(pevt->timestamp, strBuf);
            log_list[3] = g_strdup(strBuf);
    ...
    else if (evt->type == GDK_DELETE)
    {
        ...
        uxm_unreg_imsg_event(obj, INFY_DELAY_AI_FACE_EVENT);
    }
...
gint _dvabx_face_live_log_page(NFOBJECT *parent)
{
    NFOBJECT *fixed;
    NFOBJECT *obj;
    ...
    obj = nfui_listbox_new(4, tbl_livelog_w, 75);
    ...
    nfui_listbox_set_column_align(NF_LISTBOX(obj), 2, NFALIGN_LEFT);
    nfui_listbox_set_column_align(NF_LISTBOX(obj), 3, NFALIGN_CENTER);
    ...
    uxm_reg_imsg_event(obj, INFY_DELAY_AI_FACE_EVENT);
    uxm_monitor_on_imsg_event(obj, INFY_DELAY_AI_FACE_EVENT);

    return 0;
}

void _dvabx_face_live_log_show(NFOBJECT *parent)
{
    nfui_nfobject_show(parent);
}

void _dvabx_face_live_log_hide(NFOBJECT *parent)
{
	nfui_nfobject_hide(parent);
}

gint _dvabx_face_live_log_clear()
{
