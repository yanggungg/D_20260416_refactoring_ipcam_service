...
#include "vw_sys_camera_ipcam_install_search_order_menu.h"



#define MAC_SIZE_W							(360)
#define MAC_SIZE_H							(152)
...
static gboolean post_ascending_event_handler(NFOBJECT *obj, GdkEvent *evt, gpointer data)
{
	switch(evt->type) 
	{
...
static gboolean post_descending_event_handler(NFOBJECT *obj, GdkEvent *evt, gpointer data)
{
	switch(evt->type) 
	{
    	case GDK_BUTTON_RELEASE:
...
static gboolean post_resetsort_event_handler(NFOBJECT *obj, GdkEvent *evt, gpointer data)
{
	switch(evt->type) 
	{
    	case GDK_BUTTON_RELEASE:
...
gboolean VW_Create_Camera_Order_Menu_Open(NFWINDOW *parent, gint x, gint y)
{
	NFOBJECT *wnd;
	NFOBJECT *main_fixed;
	NFOBJECT *obj;
	
	gchar *strBtn[3] = {"ASCENDING (HOSTNAME)", "DESCENDING (HOSTNAME)", "SORT INITIALIZATION"};
	guint pos_y = 0;
	gint i;
	...
	g_curwnd = wnd;
	nfui_nfwindow_set_title(wnd, "CAMERA LIST ORDER");
	...
	for(i=0; i<3; i++) {
		...
		if (i == 0)
			nfui_regi_post_event_callback(obj, post_ascending_event_handler);

		if (i == 1)
			nfui_regi_post_event_callback(obj, post_descending_event_handler);

		if (i == 2)
			nfui_regi_post_event_callback(obj, post_resetsort_event_handler);
...
int VW_Destroy_Camera_Order_Popup()
{
...
int VW_Camera_Order_Popup_show()
{
	if (!g_curwnd) return 0;
	g_ordered = 0;
...
int VW_Camera_Order_Popup_hide()
{
	if (!g_curwnd) return 0;
