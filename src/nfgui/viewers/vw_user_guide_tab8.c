...
static gint _draw_guide_image()
{
    ...
    g_sprintf(image, "07_guide_%s_%02d.png", g_tmplang, g_pageIdx+1);
...
void init_UserGuide_tab8_page(NFOBJECT *parent)
{
	...
	ret = icf_get_value_by_int("guide8", "cnt");
    ...
    if (g_page_cnt > 1)
    {
	    ...
	    memset(image, 0x00, sizeof(image));
	    g_sprintf(image, "07_guide_%s_%02d.png", g_tmplang, 1);
...
gboolean UserGuide_tab8_in_handler()
{
    g_pageIdx = 0;   
	_change_page_label();
...
gboolean UserGuide_tab8_out_handler()
{
