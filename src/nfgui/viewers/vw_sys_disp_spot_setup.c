...
static gint g_divType = -1;
static guint g_output_ch;
static gint g_div9_ch_cnt;
static gint g_menu;
static gint g_max_ch_cnt;
...
static NFOBJECT *div_btn[6];
static NFOBJECT *sel_ch[GUI_CHANNEL_CNT];
static NFOBJECT *dir_btns[2];
static NFOBJECT *ss_btns[SSB_BUTTONS];

static SpotElementData *g_elem_data;
static guint ret=0;


static gint _get_spot_port_max_ch_cnt(guint output_ch)
{
    gint i, cnt = 0;
...
static gint _get_div9_ch_cnt(gint max_ch_cnt)
{
    if (max_ch_cnt >= 9) return 9;
    else return 8;   
}

static gint _get_sel_ch(guint output_ch, gint idx)
{
    gint i, j;

    for (i = 0; i < GUI_CHANNEL_CNT; i++)
    {
        if (output_ch & (1 << i)) break;      //find start ch
    }

    if (i == GUI_CHANNEL_CNT) return 0;
...
static void prvCalcRect(nfrect *arRect, guint div_type)
{
	...
	for(i=0; i<g_max_ch_cnt; i++)
	{
		if(i<show_num)	nfui_nfobject_show(sel_ch[i]);
		else			nfui_nfobject_hide(sel_ch[i]);

		x = 18 + g_rtRect[i].x + (g_rtRect[i].width - CHANNEL_COMBO_WIDTH)/2;
		y = 17 + g_rtRect[i].y + g_rtRect[i].height/2 - 20;
...
static void prvMakeChNavTable()
{
	...
	switch(g_curType)
	{
		case SCR_DIV_TYPE1:
			...
			nfui_nfobject_set_data(sel_ch[0], "down", dir_btns[0]);
			break;
...
#ifndef GUI_4CH_SUPPORT
			else if(g_curType == SCR_DIV_TYPE9)		{	max=g_div9_ch_cnt;	div=3;	}
#ifndef GUI_8CH_SUPPORT
		...
		case SCR_DIV_TYPE8:
			for(i=0; i<8; i++)
			{
				...
				else if(i>=4 && i<7)
				{
					...
					nfui_nfobject_set_data(sel_ch[i], "down", dir_btns[0]);
				}
				else if(i==7)
				{
...
static void prvMakeChNavTable2()
{
	nfui_nfobject_set_data(sel_ch[0], "left", div_btn[0]);
	nfui_nfobject_set_data(sel_ch[0], "right", NULL);
	nfui_nfobject_set_data(sel_ch[0], "up", NULL);
	nfui_nfobject_set_data(sel_ch[0], "down", dir_btns[0]);
}
...
#if defined(_SUPPORT_DISCONTI_CH_SET)
		nfui_spin_button_set_index_no_expose((NFSPINBUTTON*)sel_ch[0], _get_sel_ch(g_output_ch, 0));
		nfui_signal_emit(ch_area, GDK_EXPOSE, TRUE);

		if (g_divType >  SCR_DIV_TYPE1) prvMakeChNavTable();
		else prvMakeChNavTable2();
#else
		nfui_nflabel_set_number((NFLABEL*)sel_ch[0], _get_sel_ch(g_output_ch, 0) + 1);
		nfui_signal_emit(ch_area, GDK_EXPOSE, TRUE);
#endif		
	}
#if defined(_SUPPORT_DISCONTI_CH_SET)
...
#if defined(_SUPPORT_DISCONTI_CH_SET)
		for(i=0; i<4; i++)
			nfui_spin_button_set_index_no_expose((NFSPINBUTTON*)sel_ch[i], _get_sel_ch(g_output_ch, i));
...
#else
		for (i = 0; i < 4; i++)
			nfui_nflabel_set_number((NFLABEL*)sel_ch[i], _get_sel_ch(g_output_ch, i) + 1);
...
#ifndef GUI_4CH_SUPPORT	
static gboolean post_div_btn9_event_handler(NFOBJECT *obj, GdkEvent *evt, gpointer data)
{
	if(evt->type == NFEVENT_RADIO_GET_FOCUS)
	{
		guint i;
...
#if defined(_SUPPORT_DISCONTI_CH_SET)
		for(i=0; i<8; i++)
			nfui_spin_button_set_index_no_expose((NFSPINBUTTON*)sel_ch[i], i);
...
static gboolean post_div_btn6_event_handler(NFOBJECT *obj, GdkEvent *evt, gpointer data)
{
	if(evt->type == NFEVENT_RADIO_GET_FOCUS)
	{
		guint i;
...
static void change_sel_ch(gint dir)
{
	gint i;
	gint cnt;
	gint ch;
...
#endif		
		case SCR_DIV_TYPE9: cnt = g_div9_ch_cnt; break;
...
#if defined(_SUPPORT_DISCONTI_CH_SET)
	for(i=0; i<cnt; i++) {
		ch = nfui_spin_button_get_index((NFSPINBUTTON*)sel_ch[i]);

		if(dir > 0) 
		{
			if(++ch >= _get_sel_ch(g_output_ch, g_max_ch_cnt)) 
				ch = _get_sel_ch(g_output_ch, 0);
		}
		else 
		{
			if(--ch < _get_sel_ch(g_output_ch, 0)) 
				ch = _get_sel_ch(g_output_ch, g_max_ch_cnt);
		}

		nfui_spin_button_set_index_no_expose((NFSPINBUTTON*)sel_ch[i], (gint)ch);
	}

	for(i=0; i<cnt; i++) 
		nfui_signal_emit(sel_ch[i], GDK_EXPOSE, TRUE);
#else
	for(i = 0; i < cnt; i++) {
		...
		{
			if(++ch > _get_sel_ch(g_output_ch, g_max_ch_cnt)) 
				ch = _get_sel_ch(g_output_ch, 0) + 1;
		}
		else 
		{
			if(--ch < _get_sel_ch(g_output_ch, 0) + 1) 
				ch = _get_sel_ch(g_output_ch, g_max_ch_cnt);
		}
...
static gboolean post_sel_ch_event_handler(NFOBJECT *obj, GdkEvent *evt, gpointer data)
{
...
static gboolean post_config_fixed_event_handler(NFOBJECT *obj, GdkEvent *evt, gpointer data)
{
	...
	if(evt->type == GDK_EXPOSE) 
	{
		...
		{
			case SCR_DIV_TYPE1:
				nfutil_draw_image(drawable, gc, IMG_SEQ_SETUP_BG_1, x+17, y+17, -1, -1, NFALIGN_LEFT, 0);
			break;
			case SCR_DIV_TYPE4:
				nfutil_draw_image(drawable, gc, IMG_SEQ_SETUP_BG_4, x+17, y+17, -1, -1, NFALIGN_LEFT, 0);
			break;
#ifndef GUI_4CH_SUPPORT
			case SCR_DIV_TYPE9:
				nfutil_draw_image(drawable, gc, IMG_SEQ_SETUP_BG_9, x+17, y+17, -1, -1, NFALIGN_LEFT, 0);				
			break;
#ifndef GUI_8CH_SUPPORT
			case SCR_DIV_TYPE16:
				nfutil_draw_image(drawable, gc, IMG_SEQ_SETUP_BG_16, x+17, y+17, -1, -1, NFALIGN_LEFT, 0);				
			break;
#ifndef GUI_16CH_SUPPORT
			case SCR_DIV_TYPE36:
				nfutil_draw_image(drawable, gc, IMG_SEQ_SETUP_BG_32, x+17, y+17, -1, -1, NFALIGN_LEFT, 0);				
			break;
#endif		
#endif		
#ifndef _NOT_SUPPORT_SPC_DIV
			case SCR_DIV_TYPE8:
				nfutil_draw_image(drawable, gc, IMG_SEQ_SETUP_BG_8, x+17, y+17, -1, -1, NFALIGN_LEFT, 0);				
			break;
			case SCR_DIV_TYPE6:
				nfutil_draw_image(drawable, gc, IMG_SEQ_SETUP_BG_6, x+17, y+17, -1, -1, NFALIGN_LEFT, 0);				
			break;
#endif		
#endif
		}
...
static gboolean post_okbutton_event_handler(NFOBJECT *obj, GdkEvent *evt, gpointer data)
{
	if(evt->type == GDK_BUTTON_RELEASE)
	{
...
#ifndef GUI_4CH_SUPPORT
		else if(g_curType == SCR_DIV_TYPE9)		cnt = g_div9_ch_cnt;
#ifndef GUI_8CH_SUPPORT
		...
		{
#if defined(_SUPPORT_DISCONTI_CH_SET)
			ch[i] = nfui_spin_button_get_index((NFSPINBUTTON*)sel_ch[i]);
#else		
...
guint SpotSetupDlg_Open(NFWINDOW *parent, SpotElementData *elem_data, gboolean add, guint split_ch, guint output_ch, gint menu)
{
	...
	GdkPixbuf *type_image[7][NFOBJECT_STATE_COUNT];
	...
	g_menu = menu;
	g_div9_ch_cnt = _get_div9_ch_cnt(_get_spot_port_max_ch_cnt(output_ch));
	g_max_ch_cnt = _get_spot_port_max_ch_cnt(output_ch);
    ...
    g_divType =  SCR_DIV_TYPE1;
	
    if(split_ch >= 4) g_divType =  SCR_DIV_TYPE4;	

#ifndef GUI_4CH_SUPPORT
    if(split_ch >= g_div9_ch_cnt) g_divType =  SCR_DIV_TYPE9;	
    #ifndef GUI_8CH_SUPPORT
        if(split_ch >= 16) g_divType =  SCR_DIV_TYPE16;
        #ifndef GUI_16CH_SUPPORT
            if(split_ch >= 32) g_divType =  SCR_DIV_TYPE36;
        #endif
    #endif
#endif
	...
	nfui_nfobject_set_size(fixed1, 212, 352);
	...
	nfui_nfobject_set_size(fixed2, 308, 352);
	...
	lbTemp = (NFOBJECT*)nfui_nflabel_new_with_pango_font("SPOT SETUP", nffont_get_pango_font(NFFONT_MEDIUM_SEMI), COLOR_IDX(205));
	...
	lbTemp = (NFOBJECT*)nfui_nflabel_new_with_pango_font("VIEW TYPE", nffont_get_pango_font(NFFONT_SMALL_SEMI), COLOR_IDX(292));
	...
	lbTemp = (NFOBJECT*)nfui_nflabel_new_with_pango_font("VIEW CONFIGURE", nffont_get_pango_font(NFFONT_SMALL_SEMI), COLOR_IDX(292));
	nfui_nflabel_set_align((NFLABEL*)lbTemp, NFALIGN_LEFT, 0);
...
//===> view type
	for(i = 0; i <= g_divType; i++)
	{
		nfui_get_pixbuf_size(type_image[i][0], &size_w, &size_h);
		...
		nfui_nfobject_set_size(div_btn[i], size_w, size_h);
		...
		if (i == DISP_TYPE8_IMG_IDX || i == DISP_TYPE6_IMG_IDX) {
			nfui_nfobject_disable(div_btn[i]);
			nfui_nfobject_hide(div_btn[i]);
		}
	}

#if defined(_SUPPORT_DISCONTI_CH_SET)
  	make_channel_index_string(strChannel);
	for(i=0; i<g_max_ch_cnt; i++)
	{
		sel_ch[i] = nfui_spinbutton_new(strChannel, g_max_ch_cnt, i);
		nfui_spinbutton_set_skin_type((NFSPINBUTTON*)sel_ch[i], NFSPINBUTTON_TYPE_POPUP_1);
	...
	for(i=0; i<g_max_ch_cnt; i++)
	{
		...
		nfui_nflabel_set_number((NFLABEL*)sel_ch[i], _get_sel_ch(output_ch, i) + 1);
...
// < > BUTTONS
	...
	nfui_nffixed_put((NFFIXED*)fixed2, obj, 84, 300); 
	...
	nfui_nfobject_set_size(obj, (guint)size_w, (guint)size_h);
	nfui_nfobject_show(obj);
	nfui_nffixed_put((NFFIXED*)fixed2, obj, 164, 300); 
	...
	else
	{
		...
		{
#if defined(_SUPPORT_DISCONTI_CH_SET)
			nfui_spin_button_set_index_no_expose((NFSPINBUTTON*)sel_ch[i], g_elem_data->conf[i]);
#else		
    ...
    if(split_ch >= 4) nfui_regi_post_event_callback(div_btn[1], post_div_btn4_event_handler);
#ifndef GUI_4CH_SUPPORT	
    if(split_ch >= g_div9_ch_cnt) nfui_regi_post_event_callback(div_btn[2], post_div_btn9_event_handler);
    #ifndef GUI_8CH_SUPPORT	
        if(split_ch >= 16) nfui_regi_post_event_callback(div_btn[3], post_div_btn16_event_handler);  
        #ifndef GUI_16CH_SUPPORT
            if(split_ch >= 32) nfui_regi_post_event_callback(div_btn[4], post_div_btn36_event_handler);  
        #endif
    #endif
#endif
    ...
    if(split_ch >= 4)
    {
		...
		nfui_nfobject_set_data(div_btn[0], "down", ss_btns[SSB_OK]);

		nfui_nfobject_set_data(div_btn[1], "left", div_btn[0]);
		nfui_nfobject_set_data(div_btn[1], "right", sel_ch[0]);
		nfui_nfobject_set_data(div_btn[1], "down", ss_btns[SSB_OK]);
    }
    if(split_ch >= g_div9_ch_cnt)
    {
		nfui_nfobject_set_data(div_btn[0], "right", div_btn[1]);
		...
		nfui_nfobject_set_data(div_btn[1], "left", div_btn[0]);
		nfui_nfobject_set_data(div_btn[1], "right", sel_ch[0]);
		nfui_nfobject_set_data(div_btn[1], "down", div_btn[2]);

		nfui_nfobject_set_data(div_btn[2], "right", sel_ch[0]);
		nfui_nfobject_set_data(div_btn[2], "up", div_btn[0]);
		nfui_nfobject_set_data(div_btn[2], "down", ss_btns[SSB_OK]);    
    }
    if(split_ch >= 16)
    {
		nfui_nfobject_set_data(div_btn[0], "right", div_btn[1]);
		...
		nfui_nfobject_set_data(div_btn[3], "down", ss_btns[SSB_OK]);
    }
    if (split_ch >= 32)
    {
		nfui_nfobject_set_data(div_btn[0], "right", div_btn[1]);
		...
		nfui_nfobject_set_data(div_btn[3], "right", sel_ch[0]);
		nfui_nfobject_set_data(div_btn[3], "up", div_btn[1]);
		nfui_nfobject_set_data(div_btn[3], "down", div_btn[4]);
		
		nfui_nfobject_set_data(div_btn[4], "right", sel_ch[0]);
		nfui_nfobject_set_data(div_btn[4], "up", div_btn[2]);
		nfui_nfobject_set_data(div_btn[4], "down", ss_btns[SSB_OK]);
    }
	...
	nfui_page_open(PGID_SPOT_SETUP, main_wnd, nfui_get_last_user());

	gtk_main();

	nfui_page_close(PGID_SPOT_SETUP, main_wnd);
