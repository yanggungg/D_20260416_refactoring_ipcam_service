/*
 * vw_vca_rev_component_rule_line.c
 *
...
static gboolean post_filter_color_event_handler(NFOBJECT *obj, GdkEvent *evt, gpointer data)
{
    ...
    {   
        ...
        gdk_color = UX_COLOR(component_data->line.filter_color_idx);
        ...
        component_data->line.filter_color_idx = color_idx;
        nfui_nfobject_modify_bg(obj, NFOBJECT_STATE_NORMAL, COLOR_IDX(color_idx));
        nfui_signal_emit(obj, GDK_EXPOSE, TRUE);
    }
    else if(evt->type == NFEVENT_VCAREV_COMPONENT_DATA_SYNC)
    {
        ...
        nfui_nfobject_modify_bg(obj, NFOBJECT_STATE_NORMAL, COLOR_IDX(component_data->line.filter_color_idx));
    }
...
static gboolean post_percentage_slider_event_handler(NFOBJECT *obj, GdkEvent *evt, gpointer data)
{
    if(evt->type == NFEVENT_CWSLIDER_CHANGED_RELEASE)
    {
        ...
        component_data->line.filter_color_percnt = value;
    }
    else if (evt->type == NFEVENT_VCAREV_COMPONENT_DATA_SYNC)
    {
        NFOBJECT *top;
        ...
        if(component_data->line.filter_color_percnt != value)
        {
            cw_slider_set_value((CWSLIDER*)obj, component_data->line.filter_color_percnt);
        }
    }
...
static gboolean post_color_check_event_handler(NFOBJECT *obj, GdkEvent *evt, gpointer data)
{
    if(evt->type == NFEVENT_CHECKBUTTON_CHANGED)
    {
        ...
        component_data = (VCA_COMPONENT_DATA_T*)nfui_nfobject_get_data((NFOBJECT*)top, VCA_COMPONENT_DATA);
        status = nfui_check_button_get_active((NFCHECKBUTTON*)obj);
        component_data->line.use_filter_color = status;
        ...
        if(component_data->line.use_filter_color)
        {
    ...
    else if (evt->type == NFEVENT_VCAREV_COMPONENT_DATA_SYNC)
    {
        ...
        top = nfui_nfobject_get_top(obj);
        component_data = (VCA_COMPONENT_DATA_T*)nfui_nfobject_get_data((NFOBJECT*)top, VCA_COMPONENT_DATA);
        nfui_check_button_set_active_no_expose((NFCHECKBUTTON*)obj, component_data->line.use_filter_color);
...
gint vw_vca_rev_rule_line_component_open(NFOBJECT *parent, guint opt)
{
    ...
    nfui_nfobject_set_size(fixed, DEFAULT_COMPONENT_WIDTH-20, 465-15);
    ...
    nfui_nfobject_modify_bg(obj, NFOBJECT_STATE_NORMAL, COLOR_IDX(200));
    nfui_nfobject_use_focus(obj, NFOBJECT_FOCUS_OFF);
    nfui_nfobject_set_size(obj, 154, 30);
    nfui_nfobject_show(obj);
    nfui_nffixed_put((NFFIXED*)fixed, obj,  pos_x, pos_y);
    pos_x += 155;
