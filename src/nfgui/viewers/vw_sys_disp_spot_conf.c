...
#define	CONF_WIDTH			(guint)(DISPLAY_IS_D1 ? 1288:1620)
...
static gint _get_spot_port_max_ch_cnt()
{
    gint i, cnt = 0;
...
static void prvDrawCell(NFOBJECT *obj, guint type, guint *chmap)
{
	...
	if(type == SCR_DIV_TYPE1)
	{
		...
		nfutil_draw_text_with_pango("SPOT_CONT", NULL, &UX_COLOR(737), drawable, gc, strBuf, x, y, w, h, nffont_get_pango_font(NFFONT_MEDIUM_SEMI), &UX_COLOR(739), NFALIGN_CENTER, 0);
	...
	else if(type == SCR_DIV_TYPE9)
	{
		for(i=0; i<9; i++)
		{
			...
			if(i >= _get_spot_port_max_ch_cnt()) break;
	...
	else if(type == SCR_DIV_TYPE36)
	{
		for(i=0; i<36; i++)
		{
			...
			sprintf(strBuf, "%d", chmap[i]+1);
			nfutil_draw_text_with_pango("SPOT_CONT", NULL, &UX_COLOR(737), drawable, gc, strBuf, x, y, w, h, nffont_get_pango_font(NFFONT_SMALL_SEMI), &UX_COLOR(739), NFALIGN_CENTER, 0);
		}
	}
#endif
#endif
...
static gboolean _refresh_spot_config_item(gpointer data)
{
	NFOBJECT *top;
	gint i;
...
static void _process_popup_ret_val(gint val)
{
	...
	if (val == 0)			// modify
	{
		if(cur_focus<0 || cur_focus>=32)
			return;
	
		ret = SpotSetupDlg_Open(g_curwnd, &g_elem_data[cur_focus], FALSE, g_split_ch, g_output_ch, g_menu);
		if(ret)	nfui_signal_emit(conf_fixed[cur_focus], GDK_EXPOSE, TRUE);
	}
	else if (val == 1)		// delete
	{
		if(*g_num_items<=0 || cur_focus<0 || cur_focus>=32)
			return;
	
		for(i=cur_focus; i<(*g_num_items); i++)
			g_memmove(&g_elem_data[i], &g_elem_data[i+1], sizeof(SpotElementData));

		(*g_num_items)--;
		cur_focus = -1;

		g_timeout_add(50, _refresh_spot_config_item, NULL);
	}
}
...
static gboolean pre_conf_fixed_event_handler(NFOBJECT *obj, GdkEvent *evt,  gpointer data)
{
	...
	else if(evt->type == GDK_BUTTON_RELEASE)
	{
		...
		ret_val = VW_Create_Spot_SubMenu(g_curwnd, x, y);
...
static gboolean post_add_btn_event_handler(NFOBJECT *obj, GdkEvent *evt, gpointer data)
{
	if(evt->type == GDK_BUTTON_RELEASE)
	{
		...
		if(*g_num_items >= 32)
		{
			nftool_mbox_auto(g_curwnd, 1, "NOTICE", "The number of items less than 32\nare available.");
			return FALSE;
		}

		ret = SpotSetupDlg_Open(g_curwnd, &g_elem_data[*g_num_items], TRUE, g_split_ch, g_output_ch, g_menu);
...
void SpotConf_Open(NFWINDOW *parent, SpotElementData *elem_data, guint *num_items, guint split_ch, guint output_ch, FROM_MENU_E menu)
{
	...
	for(i = 0; i < 32; i++)
	{
		...
		nfui_nffixed_put((NFFIXED*)main_fixed, conf_fixed[i], 30+200*(i%8), 30+200*(i/8));
	...
	nfui_page_open(PGID_SPOT_CONF, main_win, nfui_get_last_user());

	gtk_main();

	nfui_page_close(PGID_SPOT_CONF, main_win);
}
