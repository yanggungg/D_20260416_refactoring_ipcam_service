...
#include "vw_pass_help_popup.h"

#define HELP_WIN_H			(280)
...
#define HELP_STR3 "Must use more than 3 categories among the English capital letters, English lowercase letters, special characters or Numerics."
...
gint vw_pass_help_popup_open(NFWINDOW *parent, gint mx, gint my)
{
    NFOBJECT *win = NULL;
    NFOBJECT *fixed;
    NFOBJECT *obj;
	...
	gchar *strhelp[HELP_STR_CNT] = {HELP_STR1, HELP_STR2, HELP_STR3, HELP_STR4, HELP_STR5, HELP_STR6};
    gchar strTmp[10];

	GdkColor bg_color1 = UX_COLOR(200);


    for (i = 0; i < HELP_STR_CNT; i++)
    {
...
*/
	...
	win = (NFOBJECT*)nfui_nfwindow_new(parent, mx-win_w, my, (guint)win_w, (guint)HELP_WIN_H);
    ...
    for (i = 0; i < HELP_STR_CNT; i++)
    {
        ...
        {
            pos_x = 10;
            g_sprintf(strTmp, "%s", "1. ");
        }
        else
        {
            pos_x = 20;
            g_sprintf(strTmp, "(%d) ", i);
        }
	...
	nfui_page_open(PGID_PASS_HELP_POPUP, win, nfui_get_last_user());

	gtk_main();

	nfui_page_close(PGID_PASS_HELP_POPUP, win);	
}
