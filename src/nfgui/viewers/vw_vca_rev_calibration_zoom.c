...
#include "vw_vca_rev_calibration_zoom.h"
...
typedef struct _VCA_VMAP_CALLBACK
{
	VCA_VMAP_CB_FUNC 	cb_func;
	gpointer 			user_data;
} VCA_VMAP_CALLBACK;
...
static VCA_CLON g_vca_clon;

static VCA_MEVENT_CALLBACK g_mevent_cb[VCA_MEVENT_MAX] = {0, };
static VCA_VMAP_CALLBACK g_vmap_cb = {0, };
...
static gboolean _draw_vca_calb(gpointer data)
{      
	VCA_DP dp;
	VCA_CLON preClon, postClon;
	VCA_CLON tmpClon;	    	
    VCA_VMAP display_vmap;
	...
	preClon.dic_cnt = g_vca_clon.dic_cnt;
	preClon.pdics = g_vca_clon.pdics;
	postClon.dic_cnt = 0;
	postClon.pdics = 0;	
	vw_dit_display_get_vca_diclist(&postClon);	

	if ((vw_dit_display_compare_vca_diclist(preClon, postClon) == 1) && (g_force_update == 0) && (g_vmap_update == 0)) 
	{
	    vw_dit_display_free_vca_diclist(preClon);
	    g_vca_clon.dic_cnt = postClon.dic_cnt;
	    g_vca_clon.pdics = postClon.pdics;    	    
		return TRUE;
	}
    ...
    if (g_vmap_update)
    {
    	...
    	vw_dit_display_vca_erase(&dp, preClon, tmpClon);
        ...
        vw_dit_display_set_vca_vmap(&display_vmap);
        vw_dit_display_free_vca_diclist(preClon);
    }
    else
    {
...
{
    if (!g_draw_tid) g_draw_tid = g_timeout_add(100, _draw_vca_calb, 0);
    return FALSE;
}
...
static gboolean post_main_win_event_handler(NFOBJECT *obj, GdkEvent *evt, gpointer data)
{
	switch(evt->type) 
	{
        case GDK_DELETE:
        {
        	...
        	vw_dit_display_free_vca_diclist(g_vca_clon);
        	g_vca_clon.dic_cnt = 0;
        	g_vca_clon.pdics = 0;        	
...
{   
    ...
    switch (evt->type) 
    {
        ...
        case GDK_BUTTON_PRESS:
        {
            ...
            g_mevent_cb[VCA_MEVENT_LEFT_PRESS].cb_func(&scr_info, &mevt_pt, g_mevent_cb[VCA_MEVENT_LEFT_PRESS].user_data);
        ...
        case GDK_MOTION_NOTIFY:
        {
            ...
            if (g_select_icon == 1)
            {
                ...
                g_mevent_cb[VCA_MEVENT_DRAG].cb_func(&scr_info, &mevt_pt, g_mevent_cb[VCA_MEVENT_DRAG].user_data);
            }
            else
            {
...
gint _vca_rev_calibration_zoom_close()
{
...
gint _vca_rev_calibration_zoom_restart()
{
...
gint vw_vca_rev_calibration_zoom_open(NFOBJECT *parent, gint ch)
{
    NFOBJECT *win;
    NFOBJECT *fixed;   
    NFOBJECT *obj;
	...
	nfui_nfwindow_set_title((NFWINDOW*)win, "VCA ZOOM");
...
// ADD MAIN FIXED   
    ...
    vw_vca_rev_calibration_zoom_setup_open(win, ch); 
...
gint vw_vca_rev_calibration_zoom_attach_mevent(VCA_MEVENT_E mevt_type, VCA_MEVENT_CB_FUNC mevent_cb, gpointer user_data)
{
...
gint vw_vca_rev_calibration_zoom_set_vmap(VCA_VMAP_CB_FUNC vmap_cb, gpointer user_data)
{
