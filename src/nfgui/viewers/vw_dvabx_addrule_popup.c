...
#include "vw_dvabx_addrule_popup.h"
...
static NFOBJECT *g_counter_connect_button = 0;

static gint g_cur_ch = 0;
static guint g_draw_tid = 0;
static guint changed_data = 0;

static ITX_DVAZONE_CONF g_pre_zone_conf[16];
static ITX_DVAZONE_SHAPE g_pre_zone_shape[16];

static ITX_DVACNTR_CONF g_pre_cntr_conf[16];
static ITX_DVACNTR_SHAPE g_pre_cntr_shape[16];

static gint g_video_width = 0;
static gint g_rule_max_cnt = 0;
...
static gint _set_default_component_data(DVA_COMPONENT_DATA_T *component_data);
...
static gint _show_over_count_message(gint max_cnt)
{
    gchar strBuf[128];
    gchar strTmp[8];

    memset(strTmp, 0, sizeof(strTmp));
    g_sprintf(strTmp, "%d", max_cnt);

    memset(strBuf, 0x00, sizeof(strBuf));
    g_sprintf(strBuf, lookup_string("You can set up to %s rules."), strTmp);
    nftool_mbox(g_curwnd, "NOTICE", strBuf, NFTOOL_MB_OK);
    return 0;
}

static gint _add_dvaa_line(gint rule_idx, DVA_COMPONENT_DATA_T *component_data)
{
    DVAAID dvaaid;
    ITX_DVAZONE_CONF conf;

    dvaaid = dvaa_get_dvaaid(g_cur_ch);
    dvaa_itx_detector_add_zone_line_default_template(dvaaid, rule_idx);    
...
static gint _add_dvaa_area(gint rule_idx, DVA_COMPONENT_DATA_T *component_data)
{
    DVAAID dvaaid;
    ITX_DVAZONE_CONF conf;

    dvaaid = dvaa_get_dvaaid(g_cur_ch);
    dvaa_itx_detector_add_zone_area_default_template(dvaaid, rule_idx);    
...
static gint _add_dvaa_counter(gint rule_idx, DVA_COMPONENT_DATA_T *component_data)
{
    DVAAID dvaaid;
    ITX_DVACNTR_CONF conf;

    dvaaid = dvaa_get_dvaaid(g_cur_ch);
    dvaa_itx_detector_add_cntr_default_template(dvaaid, rule_idx);    
...
static gint _set_dvaa_line_shape(gint rule_idx, DVA_COMPONENT_DATA_T *component_data)
{
    DVAAID dvaaid;
    ...
    dvaa_itx_detector_get_zone_shape(dvaaid, rule_idx, &shape);
    strcpy(shape.name, component_data->line.name);
    shape.color_idx = component_data->line.display_color_idx;
    dvaa_itx_detector_set_zone_shape(dvaaid, rule_idx, &shape);
    return 0;
}

static gint _set_dvaa_line_conf(gint rule_idx, DVA_COMPONENT_DATA_T *component_data)
{
    DVAAID dvaaid;
    ITX_DVAZONE_CONF conf;
    int i;
    ...
    conf.all_detect_obj = component_data->line.all_detect_obj;

    memset(conf.interest_obj, 0x00, sizeof(conf.interest_obj));
    g_snprintf(conf.interest_obj, sizeof(conf.interest_obj)-1, "%s", component_data->line.interest_obj);
    ...
    conf.c_threshold = component_data->line.c_threshold;
 
    memset(conf.event_audio, 0x00, sizeof(conf.event_audio));

    for(i=0; i<8; i++)
    {
        g_snprintf(conf.event_audio[i], sizeof(conf.event_audio[i])-1, "%s", component_data->line.event_audio[i]);
    }

    dvaa_itx_detector_set_zone_conf(dvaaid, rule_idx, &conf);
    return 0;
}

static gint _get_dvaa_line_component_data(gint rule_idx, DVA_COMPONENT_DATA_T *component_data)
{
    DVAAID dvaaid;
    ITX_DVAZONE_CONF conf;
    ITX_DVAZONE_SHAPE shape;
    ...
    component_data->line.all_detect_obj = conf.all_detect_obj;

    memset(component_data->line.interest_obj, 0x00, sizeof(component_data->line.interest_obj));
    g_snprintf(component_data->line.interest_obj, sizeof(component_data->line.interest_obj)-1, "%s", conf.interest_obj);
    ...
    component_data->line.filter_min_size_w = conf.filter_width_from;
    component_data->line.filter_max_size_w = conf.filter_width_to;
    component_data->line.filter_min_size_h = conf.filter_height_from;
    component_data->line.filter_max_size_h = conf.filter_height_to;
    ...
    component_data->line.c_threshold = conf.c_threshold;

    memset(component_data->line.event_audio, 0x00, sizeof(component_data->line.event_audio));

    for (i=0; i<8; i++)
    {
        g_snprintf(component_data->line.event_audio[i], sizeof(component_data->line.event_audio[i])-1, "%s", conf.event_audio[i]);
    }
    return 0;
}

static gint _set_dvaa_area_shape(gint rule_idx, DVA_COMPONENT_DATA_T *component_data)
{
    DVAAID dvaaid;
    ITX_DVAZONE_SHAPE shape;
...
static gint _set_dvaa_area_conf(gint rule_idx, DVA_COMPONENT_DATA_T *component_data)
{
    DVAAID dvaaid;
    ITX_DVAZONE_CONF conf;
    int i;
    ...
    conf.all_detect_obj = component_data->area.all_detect_obj;
    
    memset(conf.interest_obj, 0x00, sizeof(conf.interest_obj));
    g_snprintf(conf.interest_obj, sizeof(conf.interest_obj)-1, "%s", component_data->area.interest_obj);

    conf.intrusion = component_data->area.intrusion;
    ...
    conf.c_threshold = component_data->area.c_threshold;

    memset(conf.event_audio, 0x00, sizeof(conf.event_audio));

    for (i=0; i<8; i++)
    {
        g_snprintf(conf.event_audio[i], sizeof(conf.event_audio[i])-1, "%s", component_data->area.event_audio[i]);
    }
...
static gint _get_dvaa_area_component_data(gint rule_idx, DVA_COMPONENT_DATA_T *component_data)
{
    DVAAID dvaaid;
    ITX_DVAZONE_CONF conf;
    ITX_DVAZONE_SHAPE shape;
    ...
    component_data->area.all_detect_obj = conf.all_detect_obj;

    memset(component_data->area.interest_obj, 0x00, sizeof(component_data->area.interest_obj));
    g_snprintf(component_data->area.interest_obj, sizeof(component_data->area.interest_obj)-1, "%s", conf.interest_obj);

    component_data->area.intrusion = conf.intrusion;
    ...
    component_data->area.filter_min_size_w = conf.filter_width_from;
    component_data->area.filter_max_size_w = conf.filter_width_to;
    component_data->area.filter_min_size_h = conf.filter_height_from;
    component_data->area.filter_max_size_h = conf.filter_height_to;
    ...
    component_data->area.c_threshold = conf.c_threshold;

    memset(component_data->area.event_audio, 0x00, sizeof(component_data->area.event_audio));

    for (i=0; i<8; i++)
    {
        g_snprintf(component_data->area.event_audio[i], sizeof(component_data->area.event_audio[i])-1, "%s", conf.event_audio[i]);
    }
    return 0;
}

static gint _set_dvaa_counter_shape(gint rule_idx, DVA_COMPONENT_DATA_T *component_data)
{
    DVAAID dvaaid;
    ...
    dvaa_itx_detector_get_cntr_shape(dvaaid, rule_idx, &shape);
    strcpy(shape.name, component_data->counter.name);
    shape.color_idx = component_data->counter.display_color_idx;
    dvaa_itx_detector_set_cntr_shape(dvaaid, rule_idx, &shape);
    return 0;
}

static gint _set_dvaa_counter_conf(gint rule_idx, DVA_COMPONENT_DATA_T *component_data)
{
    DVAAID dvaaid;
    ITX_DVACNTR_CONF conf;
    ...
    dvaa_itx_detector_set_cntr_conf(dvaaid, rule_idx, &conf);
    return 0;
}

static gint _get_dvaa_counter_component_data(gint rule_idx, DVA_COMPONENT_DATA_T *component_data)
{
    DVAAID dvaaid;
    ITX_DVACNTR_SHAPE shape;
    ITX_DVACNTR_CONF conf;
...
static gint _set_dvaa_counter_list(DVA_COMPONENT_DATA_T *component_data)
{
...
static gint _get_calb_result_component_data(DVAAID dvaaid, DVA_COMPONENT_DATA_T *component_data)
{
...
static gint _set_focus_dvaa_line(gint rule_idx)
{
    DVAAID dvaaid;
    ITX_DVAZONE_CONF conf;
...
static gint _set_focus_dvaa_area(gint rule_idx)
{
    DVAAID dvaaid;
    ITX_DVAZONE_CONF conf;
...
static gint _set_focus_dvaa_counter(gint rule_idx)
{
    DVAAID dvaaid;
    ITX_DVACNTR_CONF conf;
...
static gint _unset_focus_dvaa_line(gint rule_idx)
{
    DVAAID dvaaid;
    ITX_DVAZONE_CONF conf;
...
static gint _unset_focus_dvaa_area(gint rule_idx)
{
    DVAAID dvaaid;
    ITX_DVAZONE_CONF conf;
...
static gint _unset_focus_dvaa_counter(gint rule_idx)
{
    DVAAID dvaaid;
    ITX_DVACNTR_CONF conf;
...
static gint _rule_component_act_cb(gpointer user_data)
{
    ...
    if (component_data->rule_type == RTYPE_LINE) _set_dvaa_line_conf(component_data->zone_id, component_data);
    else if (component_data->rule_type == RTYPE_AREA) _set_dvaa_area_conf(component_data->zone_id, component_data);
    else if (component_data->rule_type == RTYPE_COUNTER) _set_dvaa_counter_conf(component_data->cntr_id, component_data);
...
static gint _line_component_name_cb(gpointer user_data)
{
    ...
    top = nfui_nfobject_get_top(obj);
    component_data = (DVA_COMPONENT_DATA_T*)nfui_nfobject_get_data((NFOBJECT*)top, DVA_COMPONENT_DATA);
    _set_dvaa_line_shape(component_data->zone_id, component_data);
...
static gint _line_component_connected_counter_cb(gpointer user_data)
{
    DVA_COMPONENT_DATA_T *component_data;
    NFOBJECT *top;
    NFOBJECT *obj;
    ...
    _set_dvaa_line_conf(component_data->zone_id, component_data);
    _unset_focus_dvaa_line(component_data->zone_id);

    cntr_idx = _get_new_counter_idx();

    if (cntr_idx == -1) {
        _show_over_count_message(g_rule_max_cnt);
        return -1;
    }
    ...
    component_data->rule_type = RTYPE_COUNTER; 
    component_data->counter.up_source = zone_id;
    ...
    _add_dvaa_counter(cntr_idx, component_data);
    _set_dvaa_counter_shape(cntr_idx, component_data);
    _set_dvaa_counter_conf(cntr_idx, component_data);
    _set_dvaa_counter_list(component_data);
    _set_focus_dvaa_counter(cntr_idx);

    vw_dvabx_rule_setup_component_sync_data(g_counter_fixed);
...
static gint _area_component_name_cb(gpointer user_data)
{
    ...
    top = nfui_nfobject_get_top(obj);
    component_data = (DVA_COMPONENT_DATA_T*)nfui_nfobject_get_data((NFOBJECT*)top, DVA_COMPONENT_DATA);
    _set_dvaa_area_shape(component_data->zone_id, component_data);
...
static gint _area_component_connected_counter_cb(gpointer user_data)
{
    DVA_COMPONENT_DATA_T *component_data;
    NFOBJECT *top;
    NFOBJECT *obj;
    ...
    if ((component_data->area.all_detect_obj == 0) && (strlen(component_data->area.interest_obj) == 0))
    {
        nftool_mbox(g_curwnd, "NOTICE", "Please select at least one object.", NFTOOL_MB_OK);
        return -1;
    }

    if ((component_data->area.intrusion == 0) && (component_data->area.enter == 0) && (component_data->area.exit == 0) && \ 
    ...
    _set_dvaa_area_conf(component_data->zone_id, component_data);
    _unset_focus_dvaa_area(component_data->zone_id);
...
static gint _counter_component_name_cb(gpointer user_data)
{
    ...
    top = nfui_nfobject_get_top(obj);
    component_data = (DVA_COMPONENT_DATA_T*)nfui_nfobject_get_data((NFOBJECT*)top, DVA_COMPONENT_DATA);
    _set_dvaa_counter_shape(component_data->cntr_id, component_data);
...
static gint _set_default_component_data(DVA_COMPONENT_DATA_T *component_data)
{
    ...
    component_data->line.all_detect_obj = 1;
    g_stpcpy(component_data->line.interest_obj, "person,car,bike");
    ...
    component_data->area.all_detect_obj = 1;
    g_stpcpy(component_data->area.interest_obj, "person,car,bike");
    component_data->area.intrusion = 0;
    ...
    component_data->area.stopped_val = 30;
    component_data->area.removed_val = 5;
    component_data->area.loitering_val = 30;
...
static gint _init_component_data(NFOBJECT *win, gint ch, gchar *algorithm)
{
    DVAAID dvaaid;
    DVA_COMPONENT_DATA_T *component_data;
    gint unit_change ;
    ...
    snprintf(component_data->algorithm_type, sizeof(component_data->algorithm_type), "%s", ALGOTYPE_DETECTOR);
    ...
    _set_default_component_data(component_data);    
    _get_calb_result_component_data(dvaaid, component_data);
...
static gint _init_component_action(NFOBJECT *win, gint ch)
{
    ...
    component_action->line_connected_counter_cb = _line_component_connected_counter_cb;  
    
    component_action->area_name_cb = _area_component_name_cb;  
    component_action->area_dis_color_cb = _area_component_dis_color_cb;  
    component_action->area_connected_counter_cb = _area_component_connected_counter_cb;  
...
static gboolean post_main_fixed_event_handler(NFOBJECT *obj, GdkEvent *evt, gpointer data)
{
    ...
    switch(evt->type)
    {
        case GDK_EXPOSE :
        {
            ...
            gdk_draw_rectangle(drawable, gc, TRUE, 15, 60+(WIN_HEIGHT-60-VIDEO_COMPONENT_HEIGHT)/2-1, g_video_width+2, VIDEO_COMPONENT_HEIGHT+2);

            gdk_gc_set_rgb_fg_color(gc, &UX_COLOR((604)));
            gdk_draw_rectangle(drawable, gc, TRUE, 20+g_video_width+10, 60, 1, size_h-80);
        }
        break;
        ...
        case INFY_DVAA_ITX_PRESS_ZONE_ID:
        {
            ...
            {
                if (component_data->zone_id != -1) _set_dvaa_line_conf(component_data->zone_id, component_data);
            }
            else if (component_data->rule_type == RTYPE_AREA) 
            {
                if (component_data->zone_id != -1) _set_dvaa_area_conf(component_data->zone_id, component_data);
            }
            else if (component_data->rule_type == RTYPE_COUNTER) 
            {
                if (component_data->cntr_id != -1) _set_dvaa_counter_conf(component_data->cntr_id, component_data);
            }
            ...
            if (conf.type == 0) 
            {
                _get_dvaa_line_component_data(rule_idx, component_data);
                vw_dvabx_rule_line_component_sync_data(g_ruleline_fixed);
            ...
            {
                _get_dvaa_area_component_data(rule_idx, component_data);
                vw_dvabx_rule_area_component_sync_data(g_rulearea_fixed);
        ...
        case INFY_DVAA_ITX_PRESS_CNTR_ID:
        {
            ...
            _get_dvaa_counter_component_data(rule_idx, component_data);
            _set_dvaa_counter_list(component_data);
            vw_dvabx_counter_component_sync_data(g_counter_fixed);
...
static gboolean post_nextbutton_event_handler(NFOBJECT *obj, GdkEvent *evt, gpointer data)
{
    if (evt->type == GDK_BUTTON_RELEASE)
    {
        ...
        if (component_data->rule_type == RTYPE_LINE) 
        {
            ...
            _add_dvaa_line(rule_idx, component_data);
            _set_dvaa_line_shape(rule_idx, component_data);
            _set_dvaa_line_conf(rule_idx, component_data);
            _set_focus_dvaa_line(rule_idx);
            vw_dvabx_rule_line_component_sync_data(g_ruleline_fixed);
        ...
        else if (component_data->rule_type == RTYPE_AREA) 
        {
            ...
            _add_dvaa_area(rule_idx, component_data);
            _set_dvaa_area_shape(rule_idx, component_data);
            _set_dvaa_area_conf(rule_idx, component_data);
            _set_focus_dvaa_area(rule_idx);            
            vw_dvabx_rule_area_component_sync_data(g_rulearea_fixed);
        ...
        else if (component_data->rule_type == RTYPE_COUNTER) 
        {
            ...
            _add_dvaa_counter(rule_idx, component_data);
            _set_dvaa_counter_shape(rule_idx, component_data);      
            _set_dvaa_counter_conf(rule_idx, component_data);
            _set_dvaa_counter_list(component_data);
            _set_focus_dvaa_counter(rule_idx);
            vw_dvabx_counter_component_sync_data(g_counter_fixed);

            next_fixed = g_counter_fixed;
        }

        if ((_get_zone_count() == g_rule_max_cnt) && (_get_counter_count() == g_rule_max_cnt))
        {
...
static gboolean post_closebutton_event_handler(NFOBJECT *obj, GdkEvent *evt, gpointer data)
{
    if (evt->type == GDK_BUTTON_RELEASE)
    {
    	...
    	if(ret != NFTOOL_MB_OK)
    	{
            ...
            for (i = 0; i < 16; i++)
            {
                dvaa_itx_detector_set_zone_shape(dvaaid, i, &g_pre_zone_shape[i]);
                dvaa_itx_detector_set_zone_conf(dvaaid, i, &g_pre_zone_conf[i]);
                dvaa_itx_detector_set_cntr_shape(dvaaid, i, &g_pre_cntr_shape[i]);
                dvaa_itx_detector_set_cntr_conf(dvaaid, i, &g_pre_cntr_conf[i]);
            }    
...
static gboolean post_ruleline_addbutton_event_handler(NFOBJECT *obj, GdkEvent *evt, gpointer data)
{
    if (evt->type == GDK_BUTTON_RELEASE)
    {
        ...
        _set_default_component_data(component_data);
        vw_dvabx_rule_setup_component_sync_data(g_rulesetup_fixed);
...
static gboolean post_counter_addbutton_event_handler(NFOBJECT *obj, GdkEvent *evt, gpointer data)
{
    if (evt->type == GDK_BUTTON_RELEASE)
    {
        ...
        _set_dvaa_counter_conf(component_data->cntr_id, component_data);
        _unset_focus_dvaa_counter(component_data->cntr_id);
...
gint vw_dvabx_addrule_popup_open(NFOBJECT *parent, gint ch, gchar *algorithm)
{
    NFOBJECT *win;
    NFOBJECT *fixed;   
    NFOBJECT *obj;
    ...
    if (scm_get_ipcam_ai_type(g_cur_ch) == CAM_AI_TYPE_CLAIR2) g_rule_max_cnt = 8;
    else g_rule_max_cnt = 16;

    memset(g_pre_zone_conf, 0x00, sizeof(ITX_DVAZONE_CONF) * 16);
    memset(g_pre_zone_shape, 0x00, sizeof(ITX_DVAZONE_SHAPE) * 16);
    memset(g_pre_cntr_conf, 0x00, sizeof(ITX_DVACNTR_CONF) * 16);
    memset(g_pre_cntr_shape, 0x00, sizeof(ITX_DVACNTR_SHAPE) * 16);

    dvaaid = dvaa_get_dvaaid(g_cur_ch);
    dvaa_itx_detector_get_rule_prop(dvaaid, &prop);

    if(prop.unit == 1)  set_unit_change_yard(1);
    else    set_unit_change_yard(0);

    for (i = 0; i < 16; i++)
    {
        dvaa_itx_detector_get_zone_conf(dvaaid, i, &g_pre_zone_conf[i]);
        dvaa_itx_detector_get_zone_shape(dvaaid, i, &g_pre_zone_shape[i]);
        dvaa_itx_detector_get_cntr_conf(dvaaid, i, &g_pre_cntr_conf[i]);
        dvaa_itx_detector_get_cntr_shape(dvaaid, i, &g_pre_cntr_shape[i]);

        if (g_pre_zone_conf[i].use_zone) act_zone |= (1 << i);
    }    

    scm_ipcam_get_max_stream_ratio(ch, &ratio_w, &ratio_h);
    if (ratio_w == 16) // max stream ratio -> 16:9
    {
        g_video_width = VIDEO_COMPONENT_16_9_W;
    }
    else // max stream ratio -> 4:3
    { 
        g_video_width = VIDEO_COMPONENT_4_3_W;
    }
    win_width = g_video_width+DEFAULT_COMPONENT_WIDTH+60;
    ...
    _init_component_data(win, ch, algorithm);
    ...
    uxm_reg_imsg_event(fixed, INFY_DVAA_ITX_PRESS_ZONE_ID);
    uxm_reg_imsg_event(fixed, INFY_DVAA_ITX_PRESS_CNTR_ID);
...
// VIDEO FIXED
    fixed = (NFOBJECT*)nfui_nffixed_new();
    nfui_nfobject_set_size(fixed, g_video_width, VIDEO_COMPONENT_HEIGHT);
...
// RULE SETUP FIXED
    ...
    nfui_nfobject_show(fixed);
    nfui_nffixed_put((NFFIXED*)main_fixed, fixed, 20+g_video_width+20, 60);
    ...
    if (act_zone != 0) opt |= (1 << OPT_RULE_SETUP_COUNTER);
    vw_dvabx_rule_setup_component_open(fixed, opt);
...
// RULE LINE FIXED
    ...
    opt |= (1 << OPT_LINE_CONNECTED_COUNTER);
    vw_dvabx_rule_line_component_open(fixed, opt);
...
// RULE AREA FIXED
    ...
    opt |= (1 << OPT_AREA_CONNECTED_COUNTER);
    vw_dvabx_rule_area_component_open(fixed, opt);
...
// COUNTER FIXED
    ...
    vw_dvabx_counter_component_open(fixed, opt);
