...
#define REC_ALARM_TAB_WIDTH             (300)
#define REC_ALARM_TAB_HEIGHT            (40)
...
void VW_Init_RecAlarm_Page(NFOBJECT *parent)
{
    ...
    nftab = nfui_nftab_new(2, (gchar**)strImage_h, REC_ALARM_TAB_WIDTH, REC_ALARM_TAB_HEIGHT, NFTAB_DIR_H, strTabTitle, colidx);
    ...
    VW_Init_RecAlarmParam_Page(tab_page[0]);
    VW_Init_RecAlarmSched_Page(tab_page[1]);

    nfui_nfobject_show(tab_page[0]);
}


gboolean VW_RecAlarm_tab_out_handler()
{
    mb_type ret;
    ChangedDataType type = REC_DATA_NOT_CHANGED;
    guint i,j;

    if((type = is_changed_alarm_data())) {
        ret = nftool_mbox(g_curwnd, "CONFIRM", "Configuration has been changed.\nDo you want to save?", NFTOOL_MB_OKCANCEL);

        if(ret == NFTOOL_MB_OK)
        {
            set_record_alarm_db(type);
            vw_send_notify_change_record_data();
        }
        else
        {
            get_record_alarm_db(VW_RecAlarmParam_get_sched_day(), 0);
            VW_RecAlarmParam_data_reloaded(TRUE);
        }
    }
