...
#include "vw_alarm_sensor_evt_camera.h"
...
static EA_AlmSenData g_asd[CAM_ALARM_IN];
static EA_AlmSenData g_oasd[CAM_ALARM_IN];
static PtzPresetData PresetData[GUI_CHANNEL_CNT];

static NFWINDOW *g_curwnd = 0;
static NFOBJECT *g_operObj[CAM_ALARM_IN];
static NFOBJECT *g_lcamObj[CAM_ALARM_IN];
static NFOBJECT *g_almObj[CAM_ALARM_IN];
static NFOBJECT *g_buzzObj[CAM_ALARM_IN];
static NFOBJECT *g_vpopObj[CAM_ALARM_IN];
static NFOBJECT *g_opopObj[CAM_ALARM_IN];
static NFOBJECT *g_mailObj[CAM_ALARM_IN];
static NFOBJECT *g_ftpObj[CAM_ALARM_IN];
static NFOBJECT *g_mobileObj[CAM_ALARM_IN];
static NFOBJECT *g_mobilepushObj[CAM_ALARM_IN];
static NFOBJECT *g_nameObj[CAM_ALARM_IN];
static NFOBJECT *g_presetObj[CAM_ALARM_IN];
static NFOBJECT *g_presetEditObj[CAM_ALARM_IN];
...
static void init_as_data()
{
	...
	memset(g_asd, 0x00, sizeof(EA_AlmSenData) * CAM_ALARM_IN);
	memset(g_oasd, 0x00, sizeof(EA_AlmSenData) * CAM_ALARM_IN);
	for(i = 0; i < CAM_ALARM_IN; i++)
		DAL_get_almSen_data(&g_asd[i], i);

	g_memmove(g_oasd, g_asd, sizeof(EA_AlmSenData) * CAM_ALARM_IN);
...
static void set_data_to_obj(gint expose)
{
	...
	for(i = 0; i < CAM_ALARM_IN; i++) {
        ...
        nfui_spin_button_set_index_no_expose((NFSPINBUTTON*)g_operObj[i], (guint)g_asd[i].oper);		
...
static gboolean post_lc_label_event_handler(NFOBJECT *obj, GdkEvent *evt, gpointer data)
{
	...
	if(evt->type == GDK_2BUTTON_PRESS || kpid == KEYPAD_ENTER) {
		...
		mask = (CAMERA_MASK_TYPE | g_asd[ch].linkCam);
		if(!VW_ChannelMask_Ctrl(g_curwnd, "LINKED CAMERA CHANNEL", (gint)off_x, (gint)(off_y + obj->height), &mask))
			return FALSE;

		if(mask != g_asd[ch].linkCam) {
			g_asd[ch].linkCam = mask;

			memset(str, 0x00, ALARM_STRING_LENGTH);
			conv_cam_data_to_string(g_asd[ch].linkCam, str);
...
static gboolean post_ao_label_event_handler(NFOBJECT *obj, GdkEvent *evt, gpointer data)
{
	...
	if(evt->type == GDK_2BUTTON_PRESS || kpid == KEYPAD_ENTER) {
		...
		mask = (ALARM_OUT_MASK_TYPE | g_asd[ch].almOut);
		if(!VW_ChannelMask_Ctrl(g_curwnd, "ALARM OUT CHANNEL", (gint)off_x, (gint)(off_y + obj->height), &mask))
			return FALSE;

		if(mask != g_asd[ch].almOut) {
			g_asd[ch].almOut = mask;

			memset(str, 0x00, ALARM_STRING_LENGTH);
			conv_alm_out_data_to_string(g_asd[ch].almOut, str);
...
static gboolean post_ao_btn_event_handler(NFOBJECT *obj, GdkEvent *evt, gpointer data)
{
	if(evt->type == GDK_BUTTON_PRESS) {
		...
		mask = (ALARM_OUT_MASK_TYPE | g_asd[ch].almOut);
		g_message("%s => %llu %llu %llu", __FUNCTION__, mask, ALARM_OUT_MASK_TYPE, g_asd[ch].almOut);
		if(!VW_ChannelMask_Ctrl(g_curwnd, "ALARM OUT CHANNEL", (gint)(off_x - 153), (gint)(off_y + obj->height), &mask))
			return FALSE;
...
static gboolean post_cancel_button_event_handler(NFOBJECT *obj, GdkEvent *evt, gpointer data)
{
	if(evt->type == GDK_BUTTON_RELEASE)
	{
		...
		g_memmove(g_asd, g_oasd, sizeof(EA_AlmSenData)*CAM_ALARM_IN);
...
static gboolean post_apply_button_event_handler(NFOBJECT *obj, GdkEvent *evt, gpointer data)
{
	if(evt->type == GDK_BUTTON_RELEASE)
	{
		...
		if(memcmp(g_oasd, g_asd, sizeof(EA_AlmSenData)*CAM_ALARM_IN)) {
			set_as_data();
...
void VW_Init_AlarmSensorEvt_Cam_Page(NFOBJECT *parent)
{
	...
	gchar buf[128];
	gchar *strParam[] = {"CAM", "NAME", "OPERATION"};
	...
	for(i = 0; i < CAM_ALARM_IN; i++) {
    	...
    	nfui_nffixed_put((NFFIXED*)g_page_fixed[page_num], g_operObj[i], action_pos_x, (row_num * 41));
...
// linked camera
	for(i = 0; i < CAM_ALARM_IN; i++)
	{
    	...
    	nfui_nffixed_put((NFFIXED*)g_page_fixed[page_num], obj, action_pos_x+action_w[ACTION_LCAMERA]-size_w, (row_num * 41));
	...
	action_pos_x += (action_w[ACTION_LCAMERA] + 2);
...
// alarm out
	for(i = 0; i < CAM_ALARM_IN; i++) {
		...
		if(ALARM_OUT_COUNT >= 2) {
        	...
        	nfui_nffixed_put((NFFIXED*)g_page_fixed[page_num], obj, action_pos_x+action_w[ACTION_AOUT]-size_w, (row_num * 41));
		}
	...
	action_pos_x += (action_w[ACTION_AOUT] + 2);
    ...
    for (i = 0; i < CAM_ALARM_IN; i++)
    {
    	...
    	nfui_nfobject_show(fixed_temp);
    	nfui_nffixed_put((NFFIXED*)g_page_fixed[page_num], fixed_temp, action_pos_x, (row_num * 41));
	...
	action_pos_x += (action_w[ACTION_PRESET] + 2);
#endif
...
gboolean VW_AlarmSensorEvt_Cam_tab_out_handler()
{
	mb_type ret;
