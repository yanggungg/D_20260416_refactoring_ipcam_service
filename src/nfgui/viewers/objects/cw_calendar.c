...
static int _get_rec_info(CWCALENDAR *cwwidget)
{
	gint i;
...
static void _prvDrawBorder(CWCALENDAR *cwwidget)
{
	...
	width = CWCLD(cwwidget)->w_width+1;
	height = CWCLD(cwwidget)->w_height+1;
...
static void _draw_days(CWCALENDAR *cwwidget, cairo_t *cr, GdkGC *gc, GdkDrawable *drawable)
{
	int i, j;
	guint x, y;
	...
	int xpos = x;
	int ypos = CW_CLD_DAYS_YPOS + cwwidget->col_height + y;
	
	int lastday = _get_month_lastday(CWCLD(cwwidget)->dyear, CWCLD(cwwidget)->dmonth);
	char text[5];
	
	gdouble text_xpos, text_ypos;
	
	CWCLD(cwwidget)->dfirst_wday = _get_first_wday(CWCLD(cwwidget)->dyear, CWCLD(cwwidget)->dmonth, 1);
	
	xpos = CWCLD(cwwidget)->dfirst_wday * (cwwidget->col_width);
	xpos += x;
	
	j = CWCLD(cwwidget)->dfirst_wday;
	...
	for(i = 1; i <= lastday ; i++)
	{
		...
		if( CWCLD(cwwidget)->cyear == CWCLD(cwwidget)->dyear &&
			CWCLD(cwwidget)->cmonth == CWCLD(cwwidget)->dmonth &&
			CWCLD(cwwidget)->cday == i )
    		select_day = 1;
    	else
    		select_day = 0;    	
...
static void _draw_calendar_paint(CWCALENDAR *cwwidget)
{
	GdkDrawable *drawable;
	cairo_t *cr;
	GdkGC *gc;
	...
	gdk_draw_rectangle(drawable, gc, TRUE,
							x, y, 
							CWCLD(cwwidget)->w_width, CWCLD(cwwidget)->w_height);
	...
	for( i = 0 ; i < 7 ; i++, values += (cwwidget->col_height) )
	{
		gdk_draw_line(drawable, gc, 
				x, y + values, 
				x + CWCLD(cwwidget)->w_width, y + values);
	} 
	...
	for( i = 0 ; i < 8 ; i++, values += (cwwidget->col_width) )
	{
		...
		gdk_draw_line(drawable, gc, 
				x + values, y, 
				x + values, y + CWCLD(cwwidget)->w_height);		
	}
...
static gint _flip_over_page(CWCALENDAR *cwwidget, guint year, guint month)
{
	...
	CWCLD(cwwidget)->dyear = year;
	CWCLD(cwwidget)->dmonth = month;
	cwwidget->dday = 1;
	_get_rec_info(cwwidget);	
	_draw_calendar_paint(cwwidget);

	return 0;
}

static gint _set_date(CWCALENDAR *cwwidget, guint year, guint month, guint day)
{
	cwwidget->cyear = year;
	cwwidget->cmonth = month;
	cwwidget->cday = day;
	cwwidget->cfirst_wday = _get_first_wday(CWCLD(cwwidget)->cyear, CWCLD(cwwidget)->cmonth, 1);
	
	cwwidget->dyear = CWCLD(cwwidget)->cyear;
	cwwidget->dmonth = CWCLD(cwwidget)->cmonth;
	cwwidget->dday = CWCLD(cwwidget)->cday;
	cwwidget->dfirst_wday = CWCLD(cwwidget)->cfirst_wday;
...
static void _button_press(CWCALENDAR *cwwidget, gint x, gint y)
{
	int i, j;
	int xpos = 0;
	int ypos = CW_CLD_DAYS_YPOS;
	int lastday = _get_month_lastday(CWCLD(cwwidget)->dyear, CWCLD(cwwidget)->dmonth);
	...
	for(i = 1; i <= lastday ; i++)
	{
			...
			y > ypos && y < (ypos + cwwidget->col_height) )
		{
			...
			CWCLD(cwwidget)->cmonth = CWCLD(cwwidget)->dmonth;
			CWCLD(cwwidget)->cyear = CWCLD(cwwidget)->dyear;
			
			CWCLD(cwwidget)->cday = i;
			CWCLD(cwwidget)->dday = i;
			_draw_calendar_paint(cwwidget);
...
static void _prvLeftButtonProc(CWCALENDAR *cwwidget)
{
	GDate *curDate;
...
static void _prvRightButtonProc(CWCALENDAR *cwwidget)
{
	GDate *curDate;
...
static void _prvUpButtonProc(CWCALENDAR *cwwidget)
{
	GDate *curDate;
...
static void _prvDownButtonProc(CWCALENDAR *cwwidget)
{
	GDate *curDate;
...
static gboolean _cw_cld_event_handler(CWCALENDAR *cwwidget, GdkEvent *event, gpointer data)
{
	switch(event->type)	
	{
		case GDK_EXPOSE :
...
CWCALENDAR *cw_cld_new(time_t time, guint col_width, guint col_height)
{
	...
	cwwidget = (CWCALENDAR*)imalloc(sizeof(CWCALENDAR));
	...
	cwwidget->object.default_event_handler = _cw_cld_event_handler;
...
{
	printf("CWCALENDAR: %d, %d, %d, %d, %d, %d, %d, %d, %d",
...
time_t cw_cld_set_prev_month(CWCALENDAR *cwwidget)
{
	...
	year = CWCLD(cwwidget)->dyear;
	mon = CWCLD(cwwidget)->dmonth;
	mon--;
	if(mon < 1)	{
		year--;
		mon = 12;
	...
	cwwidget->is_dmark = FALSE;
	CWCLD(cwwidget)->dmonth--;
	if(CWCLD(cwwidget)->dmonth < 1)	{
		CWCLD(cwwidget)->dyear--;
		CWCLD(cwwidget)->dmonth = 12;
	}
...
time_t cw_cld_set_next_month(CWCALENDAR *cwwidget)
{
	...
	cwwidget->is_dmark = FALSE;
	CWCLD(cwwidget)->dmonth++;
	if(CWCLD(cwwidget)->dmonth > 12) {
		CWCLD(cwwidget)->dyear++;
		CWCLD(cwwidget)->dmonth = 1;
	}
...
{
    int lastday;
	lastday = _get_month_lastday(CWCLD(cwwidget)->dyear, CWCLD(cwwidget)->dmonth);
...
int cw_cld_get_days_in_year(CWCALENDAR *cwwidget, gint year)
{
    ...
    for (i = 1; i < 13; i++)
    {
    	day_count += _get_month_lastday(CWCLD(cwwidget)->dyear, i);
    }
