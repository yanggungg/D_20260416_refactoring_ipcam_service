...
static ITX_LPRZONEID _get_highlight_zoneid(DVAAID dvaaid)
{
...
static int _set_zone_highlight_shape(DVAAID dvaaid, ITX_LPRZONEID zoneid, int mpt_x, int mpt_y)
{
	ITX_LPRZONE_SHAPE shape;
    int i, pt_idx;

	memset(&shape, 0x00, sizeof(ITX_LPRZONE_SHAPE));
	dvaa_itx_plateno_get_zone_shape(dvaaid, zoneid, &shape);
	...
	dvaa_itx_plateno_set_zone_shape(dvaaid, zoneid, &shape);
	return 0;
}

static int _set_zone_lowlight_shape(DVAAID dvaaid, ITX_LPRZONEID zoneid)
{
	ITX_LPRZONE_SHAPE shape;
    int i;
...
static int _set_zone_highlight_conf(DVAAID dvaaid, ITX_LPRZONEID zoneid)
{
	ITX_LPRZONE_CONF conf;

	memset(&conf, 0x00, sizeof(ITX_LPRZONE_CONF));
	dvaa_itx_plateno_get_zone_conf(dvaaid, zoneid, &conf);
	conf.focus = 1;
	dvaa_itx_plateno_set_zone_conf(dvaaid, zoneid, &conf);
	return 0;
}

static int _set_zone_lowlight_conf(DVAAID dvaaid, ITX_LPRZONEID zoneid)
{
	ITX_LPRZONE_CONF conf;
...
{
	ITX_LPRZONEID zoneid;
	ITX_LPRZONE_CONF conf;
	ITX_LPRZONE_SHAPE shape;
	int i, pt_idx;

	zoneid = dvaa_itx_plateno_find_zone(dvaaid, mpt_x, mpt_y);
...
static int _proc_select_zone_highlight(DVAAID dvaaid, ITX_LPRZONEID zoneid)
{
