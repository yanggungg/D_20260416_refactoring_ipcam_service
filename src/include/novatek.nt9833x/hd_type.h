...
typedef struct _HD_BUFINFO {
	UINTPTR phy_addr;                    ///< physical start address of buffer (page aligned)
...
#define HD_DAL_VIDEOPROC_RESERVED_BASE  0x2B00 ///< for reserved HD_DAL_VIDEOPROC
...
typedef enum _HD_VIDEO_PXLFMT {
	...
	HD_VIDEO_PXLFMT_YUV422_UYVY =       HD_VIDEO_PXLFMT_YUV422_ONE, ///< 1 plane, pixel=UYVY(w,h), packed format with Y2U1V1
	HD_VIDEO_PXLFMT_YUV422_VYUY =       0x51101422, ///< 1 plane, pixel=VYUY(w,h), packed format with Y2U1V1
	HD_VIDEO_PXLFMT_YUV422_YUYV =       0x51102422, ///< 1 plane, pixel=YUYV(w,h), packed format with Y2U1V1
	HD_VIDEO_PXLFMT_YUV422_YVYU =       0x51103422, ///< 1 plane, pixel=YVYU(w,h), packed format with Y2U1V1
	...
	HD_VIDEO_PXLFMT_YUV420_MB3 =        0x510cd420, ///< 1 plane, 1 plane, pixel=Y(w,h), UV(w/2,h/2), semi-planer format 8x4   
	...
	HD_VIDEO_PXLFMT_YUV420_NVX4 =       0x610c4420, ///< novatek-yuv-compress-4 of YUV420 for h264/h265 LLC_8x4
	HD_VIDEO_PXLFMT_YUV422_NVX3 =       0x61103422, ///< novatek-yuv-compress-3 of YUV422
	HD_VIDEO_PXLFMT_YUV420_NVX5 =       0x610c5420, ///< novatek-yuv-compress-5 of YUV420 for VPE
...
#define HD_VIDEO_PIX_PACK_LSB           0x0800
...
typedef struct _HD_VIDEO_FRAME {
	...
	UINTPTR phy_addr[HD_VIDEO_MAX_PLANE];        ///< physical address of each plane
	UINT32 poc_info;                    ///< poc_info for user trigger
	UINT32 reserved[7];                 ///< reserved info
	UINTPTR blk;                        ///< memory block
} HD_VIDEO_FRAME;
...
typedef struct _HD_VIDEO_BS_DATA {
	...
	UINTPTR phy_addr;                    ///< Physical address of encoded data
...
/**
	video bitstream data descriptor.
*/
typedef struct _HD_VIDEOENC_BS3 {
	UINT32 sign;                        ///< [r]signature = MAKEFOURCC('V','S','T','M')
	HD_METADATA* p_next;                ///< [r]pointer to next meta
	HD_VIDEO_CODEC vcodec_format;       ///< [r]Encoded format of video frame
	UINT32 pack_num;                    ///< [r]Pack number in video frame
	UINT64 timestamp;                   ///< [r]Encoded timestamp (unit: microsecond)
	HD_FRAME_TYPE frame_type;           ///< [r]Video Frame type
	HD_SVC_LAYER_TYPE svc_layer_type;   ///< [r]SVC temporal layer ID
	HD_VIDEO_BS_DATA *video_pack;       ///< [w]Pack array of encoded data, user should alloc this buffer. Buffer size = sizeof(HD_VIDEO_BS_DATA) * video_pack_num
	UINT32 video_pack_num;              ///< [w]Pack array number, user provide how many number of HD_VIDEO_BS_DATA are valid in video_pack buffer.
	HD_VIDEO_BS_PSNR_INFO psnr_info;    ///< [r]The PSNR information
	HD_VIDEO_BS_BLOCK_PARTITION_INFO blk_info; ///< [r]The block partition information
	HD_VIDEO_EVBR_STATE evbr_state;     ///< [r]The evbr state value (significant when rate-control is EVBR)
	UINT32 qp;                          ///< [r]The qp value
	UINT32 ddr_id;                      ///< [r]ddr id
	INT32  blk;                         ///< [r]memory block
	UINT32 motion_ratio;                ///< [r]Encode motion ratio
} HD_VIDEOENC_BS3;
...
typedef struct _HD_VIDEODEC_BS {
	...
	UINT32 ddr_id;                      ///< ddr id
	UINTPTR phy_addr;                    ///< physical address of bitstream data
...
typedef struct _HD_AUDIO_FRAME {
	...
	UINTPTR phy_addr[HD_AUDIO_MAX_CH];   ///< physical address of each channel
...
typedef struct _HD_OSG_STAMP_BUF {
	...
	UINTPTR p_addr;                     ///< buffer's physical address
...
typedef struct _HD_OSG_STAMP_IMG {
	...
	UINTPTR p_addr;                     ///< image's bitmap content
	UINT8 ddr_id;                       ///< p_addr's ddr_id
